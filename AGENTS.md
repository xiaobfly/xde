...
| 函数 | 语义 |
|------|------|
| `int xde_disasm(const uint8_t *opcode, struct xde_instr *diza)` | 64 位模式解码（`src/xde.c:1258`） |
| `int xde_disasm_ex(const uint8_t *opcode, struct xde_instr *diza, unsigned mode)` | 指定 16/32/64（`src/xde.c:1253`） |
| `int xde_disasm_buf(const uint8_t *opcode, unsigned max_len, struct xde_instr *diza, unsigned mode)` | 额外限制读取上限（`src/xde.c:799`） |
| `int xde_asm(uint8_t *opcode, const struct xde_instr *diza)` | 结构 → 字节；`return xde_asm_buf(opcode, XDE_MAXLEN, diza);` 的包装（`src/xde.c:1343`） |
| `int xde_asm_buf(uint8_t *opcode, unsigned max_len, const struct xde_instr *diza)` | 结构 → 字节，额外限制写入上限；装不下返回 0（`src/xde.c:1292`） |
...
```
consumer
  └─ xde_disasm / xde_disasm_ex            (src/xde.c:1258 / :1253, 只差默认参数)
       └─ xde_disasm_buf(ptr, max_len, d, mode)   ← 唯一真正的入口 (src/xde.c:799-1251)
            ├─ xde_attr[map][opcode]      (查表, :1103)  ← src/xdetbl.c 生成
            ├─ xde_group[gid][modrm.reg]  (二次查表, :1131)
            └─ parse_modrm (:673) → apply_modrm_usage (:294)
                                  → apply_usage_special (:146)
                                  → apply_implicit_gp (:583)
  └─ xde_asm(out, d) → xde_asm_buf(out, XDE_MAXLEN, d)   (src/xde.c:1343 / :1292-1341, 纯字节重组)
```
...
| # | 阶段 | 行 |
|---|------|-----|
| 1 | 入参守卫：空指针 → 0；`mode ∉ {16,32,64}` → 0；`max_len` 0/超限一律夹到 15 | `:810-817` |
| 2 | `memset` 清零 + 预置 `mode` / `defaddr` / `defdata` | `:819-822` |
| 3 | 游标初始化 `beg`/`p`/`end` | `:824-826` |
| 4 | `C_BAD` 启发式 | `:828-832` |
| 5 | 遗留前缀循环 | `:834-888` |
| 6 | 取下一个字节 | `:890-892` |
| 7 | REX（`40-4F`，仅 64 位） | `:894-907` |
| 8 | REX2（`D5`，仅 64 位） | `:909-940` |
| 9 | EVEX（`62`） | `:942-991` |
| 10 | VEX（`C4`/`C5`） | `:993-1051` |
| 11 | XOP（`8F`） | `:1053-1086` |
| 12 | 遗留 opcode + `enc = XDE_ENC_LEGACY` | `:1088-1091` |
| 13 | 汇合点标签 `got_opcode:` | `:1093` |
| 14 | map 越界检查 + `attr = xde_attr[map][mop]` | `:1101-1103` |
| 15 | 三种拒绝：`XA_INVALID` → 0；`XA_I64 && mode==64` → 0；`XA_O64 && mode!=64` → 0 | `:1105-1110` |
| 16 | `apply_attr_flags`：`XA_*` → `C_*` 映射（**`C_REL` 的唯一来源**，收尾不再重复置位） | `:1112`（实现 `:123-144`） |
| 17 | `XA_GROUP` 强制 `XA_MODRM`，并就地补置 `C_MODRM`（`apply_attr_flags` 已经跑过） | `:1114-1120` |
| 18 | peek ModR/M，取 `reg = (mpeek >> 3) & 7` | `:1122-1127` |
| 19 | group 二次查表：`xde_group[gid][reg]` 再跑一次 `apply_attr_flags` | `:1128-1135` |
| 20 | 硬编码特例：移位组 `C0 C1 D0-D3` 写 FL（`RCL`/`RCR` 另读 FL、`D2`/`D3` 另读 CL）／`C6 C7 8F` 的 `C_BAD`（`F8` 例外 = `XBEGIN`/`XABORT`）／`F6`/`F7` 的标志与 `MUL`/`DIV` 累加器规则 | `:1136-1196` |
| 21 | `parse_modrm` + `apply_modrm_usage` | `:1198-1201` |
| 22 | MOFFS 路径（非 ModR/M 的 `A0-A3` 等） | `:1202-1215` |
| 23 | `apply_usage_special` / `apply_implicit_gp` | `:1217-1218` |
| 24 | `XA_UNDEF` → `src_set = dst_set = XSET_UNDEF`、`src_set2 = dst_set2 = XSET2_ALL`（**赋值，非 OR**） | `:1220-1225` |
| 25 | `imm_bytes` 定长 → 拷贝到 `data_b` + `C_DATA*` | `:1227-1240` |
| 26 | 收尾：`len = cur.p - opcode`；`0` 或 `>15` → 0；置 `len`；返回 | `:1242-1250` |

**3DNow 没有特例分支**：表把尾随 opcode 字节建模成 `XA_IMM_IB`，`XA_3DNOW` 只负责置 `C_3DNOW`（收尾注释 `:1242-1243`，现在是准确描述而非待办）。

**移位组与组 3 都写标志**：legacy map 的 `C0`/`C1`/`D0-D3`（八个操作）一律 `dst_set |= XSET_FL`；`reg == 2 || 3`（`RCL`/`RCR`）另加 `src_set |= XSET_FL`（读 CF），`D2`/`D3` 另加 `src_set |= XSET_CL`。`F6`/`F7` 在 `reg != 2` 时 `dst_set |= XSET_FL`（`NOT`(/2) 不写标志），`/4`-`/7` 的累加器规则不变（`:1136-1196`）。这与 ALU 路径一致（`:463`/`:467`/`:607`/`:641`/`:647`）：凡写标志都要在 `dst_set` 里出现 `XSET_FL`。

**`rex` 口径统一**：`apply_modrm_usage`（`:297`）与 `apply_implicit_gp`（`:586`）都用 `(diza->rex != 0) || (diza->enc != XDE_ENC_LEGACY)`，8 位寄存器命名不会因走哪一趟而不同。`gp_set` 对 `sz ∉ {1,2,4,8}` 返回 `XSET_OTHER`（`:89`），不会冒充 64 位。
...
1. 首两字节构成的 16 位小端字等于 `0x0000` 或 `0xFFFF`（`:828-832`）；
2. 同一类遗留前缀**重复出现**（`66`/`67`/段/`F2F3`/`F0`，`:849`/`:862`/`:870`/`:877`/`:884`）；
3. 表属性 `XA_BAD`（映射见 `:130`；语义 = 「**任何模式下都不可用的编码**」，模式受限的形式改由 `XA_I64`/`XA_O64`/`XA_F64`/`XA_D64` 表达，见 Known Gaps）；
4. `C6`/`C7`/`8F` 在 legacy map 下 `reg != 0` **且该 ModR/M 字节不等于 `0xF8`**（`:1151-1153`）——`C7 F8`/`C6 F8` 是合法的 `XBEGIN`/`XABORT`，其余 `/7` 编码（`C7 FA`/`C6 FA`/`C7 38`）才算坏。判据必须**字节精确**：写成 `reg != 0 && reg != 7` 会把所有非法 `/7` 一并豁免。
5. `0F AE` mod=3 的非法子形式：无 `F3` 的 `/0-/3` 与 `/4`（表里记的是内存形式，mod=3 时这两档没有定义；判据 `:1154-1161`）；
6. `0F 18 /0-/3` 与 `0F 0D /0-/1` 的 mod=3 **保留形式**（SDM 只定义 `m8`，`mod=11b` 不成立；判据 `:1162-1169`）；
7. **REX 紧邻 VEX/EVEX/XOP/REX2 引导字节**（SDM 视作 `#UD`；判据 `:1094-1100`，`rex_seen` 在 `:903` 置位）——编码侧因 `nvex != 0` 永不发 REX，故这类输入不可能字节级往返（见 Known Gaps「编码器契约与「已判 `C_BAD` 不保证往返」边界」条）。

**前缀语义**：`66` 翻转 `defdata` 2↔4（`:846-847`）；`67` 在 64 位翻转 `defaddr` 8↔4、其余模式 2↔4（`:855-860`）；段前缀存 `p_seg`、`F2/F3` 存 `p_rep`、`F0` 存 `p_lock`。**每类只保留最后见到的字节**，**尺寸效应与之一致：同组重复前缀只在最后一次生效**（`if (!twice)`，`:846`/`:855`）——否则 `p_66`/`p_67` 已置而 `defdata`/`defaddr` 退回默认值，struct 自相矛盾，`xde_asm` 写出的字节无法解码回自身长度；重复本身仍标 `C_BAD`。段前缀本来就是「后者替换前者」的口径（`2E 26 06` → `26 06`）。注意前缀循环在 REX 判定**之前**跑完且 REX 只判一次，所以 `48 66 90` 会把 `48` 当 REX、再把 `66` 当 opcode——解码侧不拒绝非规范前缀顺序，但**重编码会按 SDM 组序规范化**（见 Known Gaps）。
...
| 字节 | 两种解释 | 判定门槛 | 行 |
|------|----------|----------|-----|
| `62` | EVEX / BOUND | `peek(1..3)` 全成功 **且** `(b2 & 0x04)` **且**（`mode == 64` 或 `(b1 & 0xC0) == 0xC0`）→ 否则 BOUND | `:944-945` |
| `C4` `C5` | VEX2/VEX3 / LES/LDS | `mode == 64` 或 `(b1 & 0xC0) == 0xC0`（`C4` 还需第三字节）→ 否则 LES/LDS | `:993-996` |
| `8F` | XOP / POP r/m | `(b1 & 0x1F) >= 8` **且**（`mode == 64` 或 `(b1 & 0xC0) == 0xC0`）→ 否则 `8F /0` = POP r/m | `:1057` |
| `D5` | REX2 (APX) / AAD | 仅 64 位且 `b == 0xD5` → 否则按 AAD 走 legacy | `:909-940` |
...
门槛失败即落到 `parse_legacy_opcode`（定义 `:763-797`，调用 `:1088-1089`）。这就是 README 那句「`C4`/`C5`/`62`/`8F` 只有在后随字节符合前缀形式时才是 VEX/EVEX/XOP」的实现。
...
| XOP | `XDE_ENC_XOP` | 3 | `8F, b1, b2` | `b1 & 0x1F`（→ map 8-10） | `C_XOP`；清 `p_66`/`p_rep`（`:1077-1078`）；只写 `opcode`/`map`（`:1083-1084`） |

**`opcode2`/`opcode3` 的写入规则**：只有 `map ∈ {XDE_MAP_0F, XDE_MAP_0F38, XDE_MAP_0F3A}` 时才写（VEX `:1040-1048`、EVEX `:979-987`）；EVEX 的 map 4-7、VEX 的 map 7（`VEX7`）、XOP 的 map 8/9/A 都不写。XOP 与 VEX/EVEX 同一规则，不是缺口——`opcode`/`opcode2`/`opcode3` 只描述 `0F`/`0F 38`/`0F 3A` 这条链。

**`opcode2`/`opcode3` 的语义（按 map 决定谁是真 opcode）**：`map == XDE_MAP_0F` 时 `opcode2` **就是**真 opcode（`0F xx` 里的 `xx`）；`map == XDE_MAP_0F38`/`XDE_MAP_0F3A` 时 `opcode2` 存的是**转义字节** `0x38`/`0x3A`，真 opcode 落在 `opcode3`（写入顺序见 `parse_legacy_opcode` `:763-797`，VEX/EVEX 路径同构）。所以判 0F38/0F3A 指令必须读 `opcode3`——读者若在 `apply_modrm_usage` 里照 `opcode2` 判断，永远只会看到 `0x38`/`0x3A`（上一轮 MOVBE 缺陷的一部分就是这么漏的）。
...
`XA_*` 到 `C_*` 的映射在 `apply_attr_flags`（`src/xde.c:123-144`）：17 条一对一 OR，**只增不减**（读-改-写 `diza->flag`）。`XA_VVVV_GPR` 与 group 位不经此函数（`XA_VVVV_GPR` 在 `:422`/`:434`/`:443`/`:481`/`:714` 直接读，`XA_GRP_ID` 只在 `:1129` 读）。
...
尺寸→标志：1 → `C_DATA1`，2 → `C_DATA2`，**3 → `C_DATA1 \| C_DATA2`**，4 → `C_DATA4`，8 → `C_DATA8`，**6 → `C_DATA4 \| C_DATA2`**（`:1234-1239`）。

### 对象集推导（三趟）

1. `apply_modrm_usage`（`:294-581`）——ModR/M 的 reg/r/m 字段。`mod == 3` 走寄存器分支，否则内存分支。`rex` 启发式在 `:297`：`rex != 0 || enc != LEGACY`（即 VEX/EVEX/XOP/REX2 一律当作「有 REX」，影响 8 位寄存器命名）。reg 字段带扩展位组成 `regx = rex_r4<<4 | rex_r<<3 | reg`（`:304-305`），r/m 侧同理用 `rex_b4`（`:473-474`）；`int setcc = (map == 0F && c == 0x0F && c2 ∈ 90..9F)` 在 `:307`。`mod == 3` 的寄存器分支里，`dst` 白名单 = ALU / `MOV r/m` / 移位 / `F6`/`F7` / `FE`/`FF` / `80-83` / **`C6`/`C7`** / **`SETcc`** / **带 `XA_VVVV_GPR` 的 VEX/EVEX/XOP**（`:535-541`，原因见下段），而 `src` 赋值排除 `0x8D` **以及 MOV 存储形式 `0x88`/`0x89`/`0xC6`/`0xC7` 与 `SETcc`**（`:529-530`）——这几类只写 r/m，不读；内存分支同样处理（`src` 排除 `:553-556`、`dst` 白名单 `:559-566`）。**分组 opcode 另有一层 reg 字段分派**：`mov_crdr`（`:335-336`）、`rdrand`（`:340-341`）、`fence`（`:349-350`）、`fsgsbase`（`:351-353`）、`endbr`（`:355-356`）、`nop_ea`（`:370-372`）、`prefetch_ea`（`:373-376`）、`mem_store`（`:383-389`）八个判定先在 `:391-392` 折成 `reg_not_dst`，`mod == 3` 分支在 `:485-511` 按它们分派并让通用规则让位，内存分支在 `:553-558` 与 `:559` 用 `nop_ea`/`prefetch_ea`/`mem_store` 改写「有内存操作数就有寄存器读写」的默认假设。
...
3. **MOVBE 载入形式的 reg 目的缺失**：`0F 38 F0` 不在 load-form 的 opcode 名单里（该名单只枚举 `8A`/`8B`/`8D` 与 0F map 的若干），于是 `movbe eax,[rax]` 的 `dst` 为空。新增 `reg_dst`（`:312-314`，`map == XDE_MAP_0F38 && (opcode3 == 0xF0 || (opcode3 == 0xF1 && p_rep != 0))`）并加进 load 条件（`:434`）。
4. **MOVBE 存储形式的 reg 源与内存目的缺失**：`reg_src`（`:315-316`，`opcode3 == 0xF1 && p_rep == 0`）加进 store-form 的 src 条件（`:451`）与内存分支的 dst 白名单（`:559`）。注意 `0F 38 F1` 的 reg 角色取决于前缀：无前缀是 MOVBE（reg=源），带 `F2`/`F3` 是 CRC32（reg=目的）——`CRC32` 的 `dst` 修前也不对（被 REP 规则塞进 `RCX`，见第 2 条）。
...
1. **`0F 20-23` MOV CR/DR 的 src/dst 整个反了（最重）**：新增 `mov_crdr`（`:335-336`），`mod == 3` 分支按 `/20 /21`（CR/DR → r/m）与 `/22 /23`（r/m → CR/DR）分派（`:485-495`），CR/DR 侧折 `OTHER`。修前 `mov eax,cr0` 报 `src={RAX}`、`dst={}`（EAX 明明是目的却记成源，`CR0` 整个丢掉）；现为 `src=OTHER`、`dst=RAX`，`mov cr0,eax` 补上 `dst=OTHER`。mutation M1（`mov_crdr = 0`）→ 9 条断言失败。
2. **`0F C7 /6`/`/7` mod=3 = RDRAND/RDSEED 的假 `OTHER`**：新增 `rdrand`（`:340-341`），只写 `dst` = r/m GPR、没有源（`:496-500`）。mutation M2 → 8 条失败。
3. **`0F AE` mod=3**：LFENCE/MFENCE/SFENCE（`:349-350`，`reg >= 5`）**无操作数**，两侧清空；`F3 0F AE /0-/3`（`:351-353`）是 RDFSBASE/RDGSBASE（写 r/m）/ WRFSBASE/WRGSBASE（读 r/m、写 FS/GS base = `OTHER`），按 `reg <= 1` 分派（`:501-511`）。mutation M3（`fence = 0`）→ 6 条、M4（`fsgsbase = 0`）→ 8 条失败。
4. **`0F AE`/`0F C7` 内存**写**形式的 `dst` 漏 `MEM`**：新增白名单 `mem_store`（`:383-389`）并加进内存分支的 dst 条件（`:559`）。修前 `fxsave`/`stmxcsr`/`xsave`/`xsaveopt`/`clwb`/`cmpxchg8b`/`cmpxchg16b`/`xrstors`/`xsavec`/`xsaves` 的 `dst` 为空（`src` 反倒有 `MEM`），现 `dst` 含 `MEM`；**只读对照**（`fxrstor`/`ldmxcsr`/`xrstor`/`vmptrld`/`vmptrst`）保持无 `MEM`。mutation M8 → 10 条失败。
...
6. **`0F B2`/`B4`/`B5`（LSS/LFS/LGS）缺 GPR 目的**：0F-map 的目的白名单里有 `B6`/`B7`/`BE`/`BF` 却漏了这三个（`:436-439` 的 `c2` 列表）→ `lss eax,[rax]` 的 `dst` 现含 `RAX`（+`OTHER`），与 MOVBE 载入形式同类。
7. **`C7 F8`（XBEGIN）/ `C6 F8`（XABORT）被误标 `C_BAD`**（两者都是合法指令）：成因是 `C6`/`C7`/`8F` 的 `reg != 0` 判据，现改用**字节精确**判据 `mpeek != 0xF8`（`:1151-1153`）；`reg != 0 && reg != 7` 那种写法会把 `C7 FA`/`C6 FA`/`C7 38` 等非法 `/7` 编码一并豁免（mutation M10b：`C7 FA`/`C6 FA` 的「仍应 bad」断言失败）。mutation M10（恢复 `reg != 0`）→ 2 条失败。
...
1. **`0F 1E`（无 `F3`）是 NOP Ev**（与 `0F 1F` 同类）：mod≠3 与 mod=3 都**不访问操作数**。新增 `nop1e`（判据 `map0F && LEGACY && c2 == 0x1E && p_rep == 0`，`:361-362`），加入 `reg_not_dst`（`:391-392`）、mod=3 的空操作数分支与内存分支的 `src` 排除（`!nop1e`，`:553`）。mutation → 6 条失败（断言在 `tests/xde_test.c:1560-1570`）。
2. **`F3 0F 1E /0`（RDSSPD/RDSSPQ）**：r/m 是**目的 GPR**、SSP 折 `OTHER`——`rdssp`（`:363-364`）在 mod=3 分支 `src |= OTHER`、`dst |= rset`（`:521-525`）。mutation → 4 条失败。
...
5. **`0F AE` mod=3 的非法子形式 → `C_BAD`**：无 `F3` 的 `/0-/3` 与 `/4` 在表里仍是内存形式的编码，mod=3 时没有任何定义，原先当普通 `OTHER|OTHER` 指令报出；`xde_disasm_buf` 新增**字节精确**判据 `opcode == 0x0F && map == XDE_MAP_0F && opcode2 == 0xAE && (mpeek >> 6) == 3 && reg <= 4 && p_rep != 0xF3`（其中的 `p_rep!=0xF3` 就是 `F3` 豁免条件；`:1154-1161`）。mutation → 5 条失败；另加 9 条「不得误伤」对照（`lfence`/`mfence`/`sfence`/四个 FS/GS base 形式/`incsspd`/`fxsave` 内存形式都不带 `C_BAD`，`tests/xde_test.c:1361-1374`）。
6. **NOP Ev 的 mod=3 形式**（`{0F,1F,C0}`/`{0F,19,C0}`/`{0F,1D,C0}`/`{0F,18,E0}`）与 mod≠3 一致清空：`nop_ea`（`:370-372`）去掉 `mod != 3`，mod=3 分支与 `fence`/`endbr`/`nop1e` 共用空操作数分支。mutation（重新插回 `mod != 3 &&`）→ 8 条失败（断言在 `tests/xde_test.c:1525-1532`）。
7. **`F3 0F AE /5`（INCSSPD/INCSSPQ）**：读 r/m GPR、写 SSP（折 `OTHER`）。新增 `incssp`（`:347-348`，`map0F && LEGACY && c2 == 0xAE && mod == 3 && reg == 5 && p_rep == 0xF3`），`fence` 判据改成 `reg >= 5 && !incssp`（`:349-350`），加入 `reg_not_dst`（`:391-392`）与 mod=3 分支（`:515-520`）；`C_BAD` 保持 0。mutation A（去掉 `!incssp`）→ 4 条、B（`dst |= OTHER|rset`）→ 2 条；断言在 `tests/xde_test.c:1376-1389`。
...
**最新一轮两批（批 A 已提交 `fdce0f3`，批 B 随本文档一起提交）——4 项 0F 系统组隐式寄存器/标志 + 2 项编解码往返缺陷，外加 1 项经核实拒绝**——同样按「先写断言看到失败 → 改 → 通过 → mutation 回退即失败」走：

批 A（`apply_modrm_usage` 的谓词表 + `xde_disasm_buf` 一处新 `C_BAD` 判据，**未动表**）：

1. **`RDRAND`/`RDSEED`（`0F C7 /6`/`/7` mod=3）写 CF** → `dst_set |= XSET_FL`（`:499`；与 `add`/`inc`/`mul` 同类。`SETcc`/`BT` 是**读** FL，归 `src`）。mutation：去掉 FL → 3 条失败。
2. **`CMPXCHG8B`/`CMPXCHG16B`（`0F C7 /1` mod≠3）**：比较并替换 `EDX:EAX`（`REX.W` 下 `RDX:RAX`）并写 ZF → 新增 `cmpxchg8b` 谓词（`:398-399`），`src`/`dst` 各含寄存器对、`dst |= XSET_FL`（`:571-577`）。**宽度由 `REX.W` 决定而非操作数尺寸**（8B 形式在 64 位仍用 32 位半寄存器，有断言钉住）。mutation：去寄存器对 → 4 条、整块删 → 11 条失败。
3. **`XSAVE`/`XSAVEOPT`/`XRSTOR`（`0F AE /4 /5 /6`）与 `XSAVEC`/`XSAVES`/`XRSTORS`（`0F C7 /3 /4 /5`）读状态组件掩码 `EDX:EAX`** → 新增 `xsave_mask` 谓词（`:404-409`），`src |= EAX|EDX`（`:579-580`）。**对照（不得置位）**：FXSAVE/FXRSTOR、LDMXCSR/STMXCSR、CLFLUSH/**CLWB**（`66` 前缀须排除 ⇒ 谓词里 `reg == 6 && p_66 == 0`）、VMPTRLD/VMPTRST。mutation：去 EDX → 6 条、整条删 → 10 条失败。
4. **`0F 18 /0-/3` 与 `0F 0D /0-/1` 的 mod=3 是保留形式**（SDM 只定义 `m8`）→ `C_BAD`（`:1162-1169`）；`/4-/7` 的 mod=3 NOP 形式与内存形式保持干净。mutation：范围收窄 → 2 条失败。

**经核实拒绝的一项（务必别照原计划改）**：原计划给 `F3 0F AE` 的 `/4`、`/6`、`/7` 置 `C_BAD`，经 SDM + objdump 核实三条**都有定义**——`/4` = **PTWRITE r32/m32**、`/6` = **UMONITOR r16/32/64**（WAITPKG）、`/7` = **SFENCE**（`F3` 冗余）——加 `C_BAD` 会错，故**未改**；其对象集缺口的记录见 Known Gaps「PTWRITE / UMONITOR」条。
...
5. **重复 `66`/`67` 的尺寸效应与自身字节不自洽**：原实现遇同组重复前缀时置 `C_BAD` 并**取消**该前缀的尺寸效应（留默认 `datasize`/`defaddr`）**却仍填 `p_66`/`p_67`** ⇒ struct 自相矛盾（`67 67 00 06` 写出 3 字节、重解码 5）。改为**最后者生效**（SDM：同组只有最后一个前缀有效；**段前缀本就是该口径**，`2E 26 06` → `26 06`），判据形如 `if (!twice) diza->defdata = …` / `if (!twice) { … defaddr … }`（`:846-847` / `:855-860`），`C_BAD` 保留。**有意后果**：部分重复前缀输入的 `len` 会变（如 `66` 生效后 Jcc 走 rel16 而非 rel32）——实测**没有任何既有断言因此变化**（643 − 621 = 22 = 恰为本轮新增）。
6. **REX 紧邻 VEX/EVEX/XOP/REX2 引导字节**（`40 C5 04 08` 这类）：SDM 视作非法 `#UD`，此前被**静默接受**；而编码侧因 `nvex != 0` 永不发 REX ⇒ 丢字节（`66 48 D5 …` 还会变立即数宽度）。按已定口径置 `C_BAD`（判据在 `got_opcode`：`rex_seen && enc != XDE_ENC_LEGACY`，`:1094-1100`；`rex_seen` 在 `:903` 置位），并配 3 条「不得误伤」对照（合法 `48 31 C0`、无 REX 的 `C5 04 08`、`vaddps`）。**全空间往返扫描与残差口径**见 Known Gaps「编码器契约与「已判 `C_BAD` 不保证往返」边界」条。

**REX2 的 r/m 是 GPR，不是向量寄存器**：`parse_modrm` 的 SIB 分支（`:713-715`）与 `apply_modrm_usage` 的内存分支（`:567-568`）在判断「是否按向量寄存器记 `XSET_OTHER`」时都会排除 `XDE_ENC_REX2`；`mod == 3` 分支同样排除（`:479-484`）。另外 `parse_modrm` 的两个「无基址」判定——SIB 的 `mod == 0 && base == 5`（`:711`）与 `mod == 0 && rm == 5`（`:730`）——在 `rex_b4` 置位时不再成立，此时它指的是真寄存器 r21，不是 disp32。
...
**不做校验、不解码**；`xde_asm_buf`（`src/xde.c:1292-1341`）只多做一次容量检查，`xde_asm`（`:1343-1346`）是 `return xde_asm_buf(opcode, XDE_MAXLEN, diza);` 的薄包装。固定顺序拼接：

```
max_len 夹取：0 或 > XDE_MAXLEN 一律按 15                        (:1299-1300)
→ 计数夹到数组容量：nvex ≤ 4 / naddr ≤ 8 / ndata ≤ 8            (:1303-1305)
→ asm_size() 算所需字节数，> max_len 则 return 0                 (:1264-1290, :1307-1308)
p_lock → p_rep → p_seg → p_66 → p_67    （SDM 组序 1→2→3→4，:1315-1319）
...
**容量安全**：`addrsize`/`datasize`/`nvex` 是 `uint8_t`，被写坏时解码侧的计数会越过 `addr_b[8]`/`data_b[8]`/`vex[4]`（真 UB）。`xde_asm_buf` 先把这三个计数夹到数组容量（`:1303-1305`），再用 `asm_size()`（`:1264-1290`）算出所需字节数，`> max_len` 就返回 `0`（`:1307-1308`）；`max_len == 0` 或 `> XDE_MAXLEN` 一律按 `XDE_MAXLEN` 处理（`:1299-1300`），与 `xde_disasm_buf` 的 `max_len` 处理同构。所以解码成功的指令（`len ≤ 15`）经 `xde_asm` 永远编得出来。

`p_66` 现在与其它遗留前缀一起在 `:1318` 统一发射（没有 nvex 专属分支）：这是为 REX2 准备的——REX2 是唯一**保留** `p_66`/`p_rep` 的编码类（VEX/EVEX/XOP 在解码时就清掉，`:1009-1010`/`:1032-1033`/`:972-973`/`:1077-1078`），而它们的 `vex[]` 头自带 pp 字段，不走 `p_66`；所以 `66 D5 …` 现在能字节级往返。
...
- 返回 `0` 只有两种情形：`!opcode || !diza`（`:1297-1298`），或所需字节数 `> max_len`（`:1307-1308`）；解码成功的指令不可能编出 0 字节；
...
| `src/xde.c` | 解码器 + 编码器（1346 行），全部核心逻辑 | `xde_disasm_buf:799`, `xde_asm_buf:1292`, `xde_asm:1343`, 13 个 static 助手 |
...
| `tests/xde_test.c` | 唯一测试文件（1795 行） | 8 个 `expect_*` 助手, `main:211` |
...
退出码 `0` = 全通过，`1` = 有失败。`main` 是 `int main(void)`，**没有任何 CLI 参数**（无 filter / verbose / 单用例选择），每次运行都执行全部 643 项检查。
...
- **注释风格**：行内 `//`，多用于标注特例原因，例：`// 32-bit GP writes zero-extend in 64-bit mode.`（`:411`）、`// NOT (/2) writes no flags`（`:1172`）、`// prefetches do read memory`（`:558`）、`// segment override`（`:554`）。**全树没有任何 `TODO`/`FIXME`/`XXX`/`HACK`/`NOTE` 标记**（`src/` 已核）。
...
| `src/xde.c:799-1251` | `xde_disasm_buf` —— 改解码行为从这里读起 |
| `src/xde.c:1292-1341` | `xde_asm_buf` —— 字节重组 + 容量检查（`xde_asm` 是它的包装，`:1343-1346`） |
| `src/xde.c:1264-1290` | `asm_size` —— 估算编码所需字节数，与 `xde_asm_buf` 的落地循环必须同步 |
| `src/xde.c:944-945` / `:993-996` / `:1057` | 三处编码类歧义门槛 |
| `src/xde.c:123-144` | `XA_*` → `C_*` 映射表（新增属性位必改） |
| `src/xde.c:146` / `:294` / `:583` | 三趟对象集推导 |
...
**自研极简 harness，无第三方框架，无 `ASSERT`/`CHECK`/`TEST` 宏**。全部逻辑在 `tests/xde_test.c`（1795 行）的单个 `int main(void)`（`:211-1795`）中：119 个匿名 `{ }` 块，每块一个 `static const uint8_t` 向量紧跟 `expect_*` 调用。**零文件 IO、零 fixture、零 golden 文件**。
...
| `expect_set(name, mode, b, n, sel, bit, want)` | `:167-188` | 要求解码长度 == `n`，再断言单个对象集位的存在/不存在。`sel` 0/1/2/3 = `src_set`/`dst_set`/`src_set2`/`dst_set2`，`want` 1 = 置位、0 = 未置位；成功打 `set<n> 0x… set|clear`（377 次） |
| `expect_flag(name, mode, b, n, bit, want)` | `:191-209` | 要求解码长度 == `n`，再断言单个 flag 位。`bit` 取单个 `C_*` 常量，`want` 1 = 置位、0 = 未置位；成功打 `flag 0x… set|clear`（146 次） |

### 覆盖矩阵（643 项检查）

643 = 613 次「经典」助手调用（`expect_len` 46 + `expect_enc` 15 + `expect_fail` 23 + `expect_roundtrip` 6 + `expect_set` 377 + `expect_flag` 146）+ 30 项其余检查（`expect_sizes` 2 + `expect_selflen` 4 + 24 项内联/打印器块——`C_RIPREL`、Object sets 打印器、闸门/asm 区段、规范前缀、`sete al` 非 undef、打印器最坏情况、Coverage 区段 ud2、Jcc/LOOP 打印器、`0F C7`/`0F AE` 保留编码等，逐项见覆盖矩阵各行）。**本轮（批 A + 批 B）新增 61 项**：批 A 的 39 项落在 `// System groups…` 区段，批 B 的 22 项落在 `// Canonical…` 区段的两个新块（重复前缀折叠 14 + REX 紧邻引导字节 8）；以上计数都是对 1795 行文件的实测值（助手自身的定义行不计入调用）。
...
5. 若新属性需要影响 `flag` 或对象集，还要改 `src/xde.c` 的 `apply_attr_flags`（`:123-144`）或三趟对象集推导（`:146` / `:294` / `:583`）；
6. 加测试用例并 `build.bat`。
...
- **REX2 保留 `p_66`/`p_rep`（刻意行为，不是缺口）**（对比 VEX `:1009-1010`/`:1032-1033`、EVEX `:972-973`、XOP `:1077-1078` 都清）——`66` 是 REX2 合法的遗留前缀，不是多余前缀。编码侧 `p_66` 与其它遗留前缀一起在 `:1318` 统一发射（无 nvex 专属分支），所以 `66 D5 …` 现在能字节级往返；改这里要解码/编码两侧一起动。
- **`C_ADDR8` 只来自 MOFFS（原先那条不可达分支已删）**：`parse_modrm` 里 `disp` 只会是 1/2/4（`:750-759`，末行注释写明），原来永不触发的 `else → C_ADDR8` 已删除；8 字节地址只经 MOFFS 路径（`:1210`）。新增位移宽度前先确认这条不变量还成立。
...
- **`0F 00`/`0F 01` 系统组的对象集断言恒真、不可证伪**（`XA_UNDEF`）：`src/xde.c:1220-1225` 对整集**赋值** `XSET_UNDEF` + `XSET2_ALL`，这两个 opcode 上的任何集合断言都恒真；要修必须在 `:1220` 之后按 opcode/reg/mod 白名单回填真实集合，而 group 行全 0、`XA_*` 32 位已满 → 风险/收益最差。本轮只补了 `sldt`/`smsw` 的 `C_UNDEF` 锚点（`tests/xde_test.c:1601-1608`）。
...
- **未定义编码只记录、不建模**：`0F 0D /2-/7` mod≠3、`F3 0F 1E /1..6`、`F3 0F 1E /0` 的 mod≠3、`F2 0F 1E C0`、`F3 0F AE /4` mod=3——UD/未定义编码，无可辩驳的目标语义 → 不写断言（`F3 0F AE /4` 因 `F3` 豁免而无 `C_BAD`；`0F AE` mod=3 的其余非法子形式此后那批已加 `C_BAD`，见 `src/xde.c:1154-1161`）。
- **隐式操作数只剩 `XBEGIN`/`XABORT` 未建模**：RDRAND/RDSEED 的 CF、CMPXCHG8B/16B 的 `EDX:EAX` + ZF、XSAVE 族/XRSTOR(S) 的 `EDX:EAX` 已在**批 A** 补齐（`cmpxchg8b`/`xsave_mask` 两个谓词，`:398-399`/`:404-409` 定义、`:573-580` 使用）；`C7 F8`（XBEGIN）/`C6 F8`（XABORT）的隐式项仍未建模——这两条只钉了显式 r/m 与 `MEM`。
- **PTWRITE / UMONITOR 的对象集未建模（批 A 记录，非本轮缺陷）**：批 A 原计划把 `F3 0F AE` 的 `/4`、`/6`、`/7` 一并标 `C_BAD`，经 SDM + objdump 核实三条**都有定义** —— `/4` = **PTWRITE**（r/m 是**只读 GPR 源**）、`/6` = **UMONITOR**（WAITPKG；mod 必须为 11B，r/m 只读 GPR）、`/7` = **SFENCE**（`F3` 冗余）—— 故**未改**（加 `C_BAD` 会错）。**衍生缺口**：PTWRITE 的 r/m 现被折成 `OTHER` 且**源丢失**；UMONITOR 被 `fence` 判据（`reg >= 5 && !incssp`）当成**无操作数**吞掉。
- **`0F B2`/`B4`/`B5`（LSS/LFS/LGS）的表侧 `XA_BAD` 假阳性已在本轮修掉**：生成器 `tools/gen_tables.py:351/353/354` 从 `XA_MODRM|XA_BAD`（`0x00000201`）改成只有 `XA_MODRM`（`0x1`），重跑后 `src/xdetbl.c:63`（0F map 的 `B0-B7` 行）为 `0x00000081, 0x00000001, 0x00000001, 0x00000001, 0x00000001, 0x00000001, 0x00000001, 0x00000001`。这三个 opcode 在 16/32/64 位都合法，解码侧本就按正常载入处理（`simd_0f` 名单与 dst 白名单都含它们），所以表侧随即与解码侧一致；三条 `C_BAD` 未置位断言钉住（`tests/xde_test.c:1688-1690`）。
