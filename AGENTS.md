...
**返回值契约**：解码返回指令长度，`0` = 失败（截断 / 该模式下非法 / undefined）。编码返回写入字节数，`xde_asm_buf` 在结构体所需字节数 `> max_len` 时返回 `0`（`xde_asm` 给的是 `XDE_MAXLEN`，所以除空指针外永远成功）。没有错误码枚举、没有 errno、没有 out-param 状态。**注意 `max_len == 0` 不是「零字节上限」而**是「用默认 `XDE_MAXLEN`(15)」（`src/xde.c:1066-1069`，`xde_asm_buf` 同构 `:1712-1713`）——传 0 不会失败，而是按 15 解码 / 编码；`max_len == 16` 也被夹到 15（同一处 `> XDE_MAXLEN` 判定，两边同构），所以「传 16 就多写一字节」的期待不成立；要限长必须传显式上界（批 B v2 的截断扫描因此在 `len == 1`（即 `len-1 == 0`）的 **8,437,552** 例上单列跳过）。
...
consumer
  ...
  └─ xde_asm(out, d) → xde_asm_buf(out, XDE_MAXLEN, d)   (src/xde.c:1756 / `:1705-1754`, 纯字节重组)
```
...
| # | 阶段 | 行 |
|---|------|-----|
| 1 | 入参守卫：空指针 → 0；`mode ∉ {16,32,64}` → 0；`max_len` 0/超限一律夹到 15 | `:1062-1069` |
| 2 | `memset` 清零 + 预置 `mode` / `defaddr` / `defdata` | `:1071-1074` |
| 3 | 游标初始化 `beg`/`p`/`end` | `:1076-1078` |
| 4 | `C_BAD` 启发式 | `:1080-1084` |
| 5 | 遗留前缀循环 | `:1086-1140` |
| 6 | 取下一个字节 | `:1142-1144` |
| 7 | REX（`40-4F`，仅 64 位） | `:1146-1159` |
| 8 | REX2（`D5`，仅 64 位） | `:1161-1192` |
| 9 | EVEX（`62`） | `:1194-1243` |
| 10 | VEX（`C4`/`C5`） | `:1245-1303` |
| 11 | XOP（`8F`） | `:1305-1338` |
| 12 | 遗留 opcode + `enc = XDE_ENC_LEGACY` | `:1340-1343` |
| 13 | 汇合点标签 `got_opcode:` | `:1345` |
| 14 | map 越界检查 + `attr = xde_attr[map][mop]` | `:1353-1355` |
| 15 | 三种拒绝：`XA_INVALID` → 0；`XA_I64 && mode==64` → 0；`XA_O64 && mode!=64` → 0 | `:1357-1362` |
| 16 | `apply_attr_flags`：`XA_*` → `C_*` 映射（**`C_REL` 的唯一来源**，收尾不再重复置位） | `:1364`（实现 `:123-144`） |
| 17 | `XA_GROUP` 强制 `XA_MODRM`，并就地补置 `C_MODRM`（`apply_attr_flags` 已经跑过） | `:1366-1372` |
| 18 | peek ModR/M，取 `reg = (mpeek >> 3) & 7` | `:1396-1401` |
| 19 | group 二次查表：`xde_group[gid][reg]` 再跑一次 `apply_attr_flags` | `:1402-1409` |
| 20 | 强制前缀检查：`legacy_enc` 且 `map == XDE_MAP_0F` 时，`xde_pfx_0f[opcode2]` 里没有当前前缀位（bit0 裸 / bit1 `66` / bit2 `F2` / bit3 `F3`，`enum XDE_PFX_*`）→ `C_BAD`，表项为 0 的 `0F 7A`/`0F 7B` 在任何前缀下都置位（见「`C_BAD` 的来源」第 20 条）；跑在 `XA_MODRM` 组**之前**——它不需要 r/m 字节，而 VEX/EVEX/XOP 的前缀在头里、从不到达这里 | `:1388-1394` |
| 21 | 硬编码特例：移位组 `C0 C1 D0-D3` 写 FL（`RCL`/`RCR` 另读 FL、`D2`/`D3` 另读 CL）／`C6 C7 8F` 的 `C_BAD`（`F8` 例外 = `XBEGIN`/`XABORT`）／**`C_BAD` 普查的六个判据（`0F 00 /6 /7`、`0F 01` 内存 `/5` 与 mod=3 保留子槽、`FF /3`/`/5` mod=3、`62` mod=3、x87 `D8-DF`，见「`C_BAD` 的来源」第 8-13 条）**／**本轮 mod 结构性判据（`8D`、`0F 13`/`0F 17`/`0F 2B`、`0F 50` 与三组顺带 opcode，见第 14-19 条）**／`F6`/`F7` 的标志与 `MUL`/`DIV` 累加器规则 | `:1410-1595` |
| 22 | `parse_modrm` + `apply_modrm_usage` | `:1597-1608` |
| 23 | MOFFS 路径（非 ModR/M 的 `A0-A3` 等） | `:1609-1622` |
| 24 | `apply_usage_special` / `apply_implicit_gp` | `:1624-1625` |
| 25 | `XA_UNDEF` → `src_set = dst_set = XSET_UNDEF`、`src_set2 = dst_set2 = XSET2_ALL`（**赋值，非 OR**）；随后 `undef_sys_operands` 按 ModR/M 回填 `0F 00`/`0F 01`/`0F 02`/`0F 03` 已建模形式的真实集合并清 `C_UNDEF`，未回填者保持整集 + `C_UNDEF` | `:1627-1638`（实现 `:308-428`，`ea_set` 暂存 `:1603-1606`，调用 `:1636`） |
| 26 | `imm_bytes` 定长 → 拷贝到 `data_b` + `C_DATA*` | `:1640-1653` |
| 27 | 收尾：`len = cur.p - opcode`；`0` 或 `>15` → 0；置 `len`；返回 | `:1655-1663` |

**3DNow 没有特例分支**：表把尾随 opcode 字节建模成 `XA_IMM_IB`，`XA_3DNOW` 只负责置 `C_3DNOW`（收尾注释 `:1655-1656`，现在是准确描述而非待办）。

**移位组与组 3 都写标志**：legacy map 的 `C0`/`C1`/`D0-D3`（八个操作）一律 `dst_set |= XSET_FL`；`reg == 2 || 3`（`RCL`/`RCR`）另加 `src_set |= XSET_FL`（读 CF），`D2`/`D3` 另加 `src_set |= XSET_CL`。`F6`/`F7` 在 `reg != 2` 时 `dst_set |= XSET_FL`（`NOT`(/2) 不写标志），`/4`-`/7` 的累加器规则不变（`:1410-1595`）。这与 ALU 路径一致（`:628`/`:632`/`:784`/`:818`/`:824`）：凡写标志都要在 `dst_set` 里出现 `XSET_FL`。
...
**`C_BAD` 的来源**（共 20 类；第 1-7 类是原有判据，第 8-13 类是 `C_BAD` 普查新增的六个 `0F`/`FF`/`62`/x87 判据，第 14-19 类是本轮 mod 结构性判据——三个显式项加三组顺带 opcode，第 20 类是本轮（强制前缀）判据，全部落在 `xde_disasm_buf` 的 `got_opcode` 段）：

1. 首两字节构成的 16 位小端字等于 `0x0000` 或 `0xFFFF`（`:1080-1084`）；
2. 同一类遗留前缀**重复出现**（`66`/`67`/段/`F2F3`/`F0`，`:1101`/`:1114`/`:1122`/`:1129`/`:1136`）；
3. 表属性 `XA_BAD`（映射见 `:130`；语义 = 「**任何模式下都不可用的编码**」，模式受限的形式改由 `XA_I64`/`XA_O64`/`XA_F64`/`XA_D64` 表达，见 Known Gaps）；
4. `C6`/`C7`/`8F` 在 legacy map 下 `reg != 0` **且该 ModR/M 字节不等于 `0xF8`**（`:1425-1427`）——`C7 F8`/`C6 F8` 是合法的 `XBEGIN`/`XABORT`，其余 `/7` 编码（`C7 FA`/`C6 FA`/`C7 38`）才算坏。判据必须**字节精确**：写成 `reg != 0 && reg != 7` 会把所有非法 `/7` 一并豁免。
5. `0F AE` mod=3 的非法子形式：无 `F3` 的 `/0-/3` 与 `/4`（表里记的是内存形式，mod=3 时这两档没有定义；（`:1428-1435`）；
6. `0F 18 /0-/3` 与 `0F 0D /0-/1` 的 mod=3 **保留形式**（SDM 只定义 `m8`，`mod=11b` 不成立；（`:1436-1443`）；
7. **REX 紧邻 VEX/EVEX/XOP/REX2 引导字节**（SDM 视作 `#UD`；判据 `:1346-1352`，`rex_seen` 在 `:1155` 置位）——编码侧因 `nvex != 0` 永不发 REX，故这类输入不可能字节级往返（见 Known Gaps「编码器契约与「已判 `C_BAD` 不保证往返」边界」条）。
8. `0F 00` 组的 `/6`/`/7`（**全 mod** 共 64 槽；`:1449-1451`）——SDM 给该组只定义 `/0-/5`（SLDT/STR、LLDT/LTR、VERR/VERW），寄存器形式与内存形式都没有 `/6`/`/7`；REX2 也走这条（`legacy_enc` 在 `:1378`，覆盖写进 `opcode2` 的形式）。
9. `0F 01` **内存形式**的 `/5`（24 槽；`:1465-1467`）——内存表是 SGDT/SIDT、LGDT/LIDT、SMSW、LMSW、INVLPG，`/5` 空着；mod=3 的 `/5`（`E8`-`EF`）另有自己的表，不受影响。
10. `0F 01` **mod=3 的保留子槽**（12 槽；`:1468-1472`）——`C6 C7 CC CD CE D2 D3 E9 EA EB EC ED`；同一张表里 `C0-C5`（ENCLV…PCONFIG）、`C8-CB`/`CF`（MONITOR/MWAIT/CLAC/STAC/ENCLS）、`D0/D1/D4-D7`（XGETBV/XSETBV/VMFUNC/XEND/XTEST/ENCLU）、`D8-DF`（SVM）、`E0-E7`（SMSW）、`EE`/`EF`（RDPKRU/WRPKRU）、`F0-F7`（LMSW）、`F8-FF`（SWAPGS/RDTSCP/MONITORX…）都是真指令，**不得误标**。
11. `FF /3`/`/5` 的 **mod=3**（16 槽；`:1479-1481`）——far `CALL`/far `JMP` 只定义内存形式的 `m16:16/m16:32/m16:64`，mod=3 无编码；`/2`/`/4`（near `CALL`/`JMP r/m`）与 `/3`/`/5` 的内存形式保持干净，`/7` 本来就由表侧 `XA_BAD` 覆盖。
12. `62`（BOUND）的 **mod=3**（64 槽；`:1486-1488`）——BOUND 的两个操作数都从内存读；判据只看 `XDE_ENC_LEGACY`，所以 `62` 在 32 位被当 EVEX 引导时不受影响（`XDE_ENC_EVEX` 不置 `C_BAD`），64 位下 BOUND 早已被 `XA_I64` 拒。
13. **x87 逃逸表（`D8-DF`）的空白槽**（251 槽；`:1559-1568`）——两张掩码表：`x87_resm[8]`（`:872-874`，内存形式的 `/reg` 位掩码，SDM 空白的是 `D9 /1`、`DB /4`、`DB /6`、`DD /5`，共 96 槽）与 `x87_res3[8]`（`:866-871`，mod=3 的 `bit(reg*8+rm)` 掩码，155 槽），合计 96 + 155 = 251。**有意排除 12 槽**：`DB E0/E1/E4/E5`（8087/287 的 FENI/FDISI/FSETPM/FRSTPM）与 `DF C0-C7`（FFREEP）在 SDM 里同样空白，但 binutils 仍给它们助记符，标 `C_BAD` 会是假阳性 ⇒ 表里留 0，并有 `not bad` 对照断言钉住（见 Known Gaps「应标 `C_BAD` 却未标：431 槽」条的两条告诫）。

14. **`LEA`（`8D /r`）的 mod=3**（每模式 64 槽；`:1493-1495`）——SDM 把第二操作数写成内存操作数（算地址），mod=3 不是它的任何编码。判据只看 `map == XDE_MAP_LEGACY`，而 REX2 的 `8D` 也落在 legacy map（`legacy_enc` 在 `:1378` 覆盖 REX2），所以两个编码类一并命中；`lea [eax]` 的三种模式不误标。
15. **`0F 13`/`0F 17`/`0F 2B` 的 mod=3**（`:1502-1505`）——MOVLPS/MOVHPS 只定义存储进内存的 m64（`66` 的 MOVLPD/MOVHPD 与 REX2 形式同 r/m），MOVNTPS 只写 m128；**`0F 12`/`0F 16` 的载入形式不在此列**：它们的 mod=3 是 MOVHLPS/MOVLHPS，合法。
16. **`0F 50`（MOVMSKPS/MOVMSKPD）的 mod≠3**（`:1509-1511`）——r/m 必须取 xmm 寄存器，其它 mod 不是它的编码；`66` 形式同判据。
17. **第二组「r/m 只定义在内存」的 0F opcode 的 mod=3**（`:1522-1529`，8 条规则）——`66 0F 12`/`66 0F 16`（MOVLPD/MOVHPD，门 `p_66 && !p_rep`）、`0F B2`/`B4`/`B5`（LSS/LFS/LGS）、`0F C3`（MOVNTI）、`0F E7`（MOVNTQ，`66` 形式为 MOVNTDQ，门 `!p_rep`）、`F2 0F F0`（LDDQU，门 `p_rep == 0xF2`）；门槛的作用是**保住合法形式**——`MOVHLPS`/`MOVLHPS`/`MOVSLDUP`/`MOVSHDUP`/`MOVDDUP` 都有寄存器 r/m，不得误标。
18. **第二组「r/m 只定义在寄存器」的 0F opcode 的 mod≠3**（`:1534-1538`，3 条规则）——`0F D7`/`66 0F D7`（PMOVMSKB）、`0F F7`/`66 0F F7`（MASKMOVQ/MASKMOVDQU，门 `!p_rep`）、`F2 0F D6`（MOVDQ2Q，门 `p_rep == 0xF2`）；**裸 `0F D6` 不是 MOVQ 的形式**——SDM 把 mm 存储写成 `0F 7F`、把 xmm 存储写成 `66 0F D6`，所以裸字节串不命名任何指令（现由第 20 条的前缀判据置位，`66 0F D6` 仍合法），而 `MOVQ r/m` 的寄存器 r/m 形式只在 `66`/`F3` 下存在，**不得误标**。
19. **`0F 38` 的 memory-only 组（mod=3）**（`:1544-1555`，10 条规则）——`66 0F 38 2A`（MOVNTDQA）、`66 0F 38 80/81/82`（INVEPT/INVVPID/INVPCID）、`0F 38 F0`/`F1`（MOVBE，门 `!p_rep`，从而 `F2` 读作 CRC32 时保持合法）、`66 0F 38 F5`（WRUSSD/WRUSSQ）、`0F 38 F6`（WRSSD/WRSSQ，门 `!p_66 && !p_rep`）、`66 0F 38 F8`（MOVDIR64B）与 `F2 0F 38 F8`（ENQCMD/ENQCMDS）、`0F 38 F9`（MOVDIRI）。

20. **`0F` map 的强制前缀合法性**（`:1388-1394`；表 `xde_pfx_0f[256]` 在 `:889-922`，`enum { XDE_PFX_NONE = 0x01, XDE_PFX_66 = 0x02, XDE_PFX_F2 = 0x04, XDE_PFX_F3 = 0x08 }` 在 `:883-888`）——SSE 族把强制前缀当成 opcode 的一部分，所以同一个转义 opcode 在别的前缀下不命名任何指令：`66 0F 6C` 是 PUNPCKLQDQ、`F3 0F B8` 是 POPCNT，而裸 `0F 6C`/`F2 0F 6C`/`F3 0F 6C` 与裸 `0F B8`/`66 0F B8`/`F2 0F B8` 都不是指令；`0F 09` 在 `66`/`F2` 下无定义（`F3 0F 09` 是另一条 WBNOINVD）。表里每位一个前缀类、**0 表示该 opcode 在任何前缀下都没有 legacy 形式**（只有 `0F 7A`/`0F 7B`），判据是 `(xde_pfx_0f[opcode2] & 实际前缀位) == 0`。它只覆盖 `map == XDE_MAP_0F` 且 `legacy_enc`：`0F 38`/`0F 3A` 的转义表与 VEX/EVEX 共用，属 P1 的白名单范围；VEX/EVEX/XOP 的头里自带前缀字节，从不到达这条判据。

第 14-16 条是三个显式判据，第 17-19 条是普查候选（44 条）里经 objdump 逐条复核（**合法**形式须被 objdump 命名助记符、**非法**形式须报 `(bad)`）后保留下来的顺带项，共 **21 条 (map, opcode[, 前缀]) 规则**；三组各自带一个 `not bad` 对照块，把守护的合法形式钉住（第 17-19 条的门就是为此存在）。
...
**前缀语义**：`66` 翻转 `defdata` 2↔4（`:1098-1099`）；`67` 在 64 位翻转 `defaddr` 8↔4、其余模式 2↔4（`:1107-1112`）；段前缀存 `p_seg`、`F2/F3` 存 `p_rep`、`F0` 存 `p_lock`。**每类只保留最后见到的字节**，**尺寸效应与之一致：同组重复前缀只在最后一次生效**（`if (!twice)`，`:1098`/`:1107`）——否则 `p_66`/`p_67` 已置而 `defdata`/`defaddr` 退回默认值，struct 自相矛盾，`xde_asm` 写出的字节无法解码回自身长度；重复本身仍标 `C_BAD`。段前缀本来就是「后者替换前者」的口径（`2E 26 06` → `26 06`）。注意前缀循环在 REX 判定**之前**跑完且 REX 只判一次，所以 `48 66 90` 会把 `48` 当 REX、再把 `66` 当 opcode——解码侧不拒绝非规范前缀顺序，但**重编码会按 SDM 组序规范化**（见 Known Gaps）。
...
| 字节 | 两种解释 | 判定门槛 | 行 |
|------|----------|----------|-----|
| `62` | EVEX / BOUND | `peek(1..3)` 全成功 **且** `(b2 & 0x04)` **且**（`mode == 64` 或 `(b1 & 0xC0) == 0xC0`）→ 否则 BOUND（走 legacy 时 mod=3 一律 `C_BAD`，见「`C_BAD` 的来源」第 12 条；EVEX 分派不受影响） | `:1196-1197` |
| `C4` `C5` | VEX2/VEX3 / LES/LDS | `mode == 64` 或 `(b1 & 0xC0) == 0xC0`（`C4` 还需第三字节）→ 否则 LES/LDS | `:1245-1248` |
| `8F` | XOP / POP r/m | `(b1 & 0x1F) >= 8` **且**（`mode == 64` 或 `(b1 & 0xC0) == 0xC0`）→ 否则 `8F /0` = POP r/m | `:1309` |
| `D5` | REX2 (APX) / AAD | 仅 64 位且 `b == 0xD5` → 否则按 AAD 走 legacy | `:1161-1192` |
...
门槛失败即落到 `parse_legacy_opcode`（定义 `:1014-1048`，调用 `:1340-1341`）。这就是 README 那句「`C4`/`C5`/`62`/`8F` 只有在后随字节符合前缀形式时才是 VEX/EVEX/XOP」的实现。
...
| XOP | `XDE_ENC_XOP` | 3 | `8F, b1, b2` | `b1 & 0x1F`（→ map 8-10） | `C_XOP`；清 `p_66`/`p_rep`（`:1329-1330`）；只写 `opcode`/`map`（`:1335-1336`） |

**`opcode2`/`opcode3` 的写入规则**：只有 `map ∈ {XDE_MAP_0F, XDE_MAP_0F38, XDE_MAP_0F3A}` 时才写（VEX `:1292-1300`、EVEX `:1231-1239`）；EVEX 的 map 4-7、VEX 的 map 7（`VEX7`）、XOP 的 map 8/9/A 都不写。XOP 与 VEX/EVEX 同一规则，不是缺口——`opcode`/`opcode2`/`opcode3` 只描述 `0F`/`0F 38`/`0F 3A` 这条链。

**`opcode2`/`opcode3` 的语义（按 map 决定谁是真 opcode）**：`map == XDE_MAP_0F` 时 `opcode2` **就是**真 opcode（`0F xx` 里的 `xx`）；`map == XDE_MAP_0F38`/`XDE_MAP_0F3A` 时 `opcode2` 存的是**转义字节** `0x38`/`0x3A`，真 opcode 落在 `opcode3`（写入顺序见 `parse_legacy_opcode` `:1014-1048`，VEX/EVEX 路径同构）。所以判 0F38/0F3A 指令必须读 `opcode3`——读者若在 `apply_modrm_usage` 里照 `opcode2` 判断，永远只会看到 `0x38`/`0x3A`（上一轮 MOVBE 缺陷的一部分就是这么漏的）。
...
`XA_*` 到 `C_*` 的映射在 `apply_attr_flags`（`src/xde.c:123-144`）：17 条一对一 OR，**只增不减**（读-改-写 `diza->flag`）。`XA_VVVV_GPR` 与 group 位不经此函数（`XA_VVVV_GPR` 在 `:587`/`:599`/`:608`/`:646`/`:965` 直接读，`XA_GRP_ID` 只在 `:1403` 读）。

**解码器自己的 `static const` 掩码表**（都在 `src/xde.c` 里，不进 `src/xdetbl.c`）：`x87_res3[8]`（`:866-871`）与 `x87_resm[8]`（`:872-874`）记 x87 逃逸表的保留槽；`xde_pfx_0f[256]`（`:889-922`，注释 `:876-882`）每位一个强制前缀类——`enum { XDE_PFX_NONE = 0x01, XDE_PFX_66 = 0x02, XDE_PFX_F2 = 0x04, XDE_PFX_F3 = 0x08 }`（`:883-888`），**0 = 该 `0F` opcode 在任何前缀下都没有 legacy 形式**（只有 `0F 7A`/`0F 7B`）。判「这个 `0F xx` + 前缀组合 xde 会不会拒」先查这张表，别从判据反推。
...
尺寸→标志：1 → `C_DATA1`，2 → `C_DATA2`，**3 → `C_DATA1 \| C_DATA2`**，4 → `C_DATA4`，8 → `C_DATA8`，**6 → `C_DATA4 \| C_DATA2`**（`:1647-1652`）。
...
7. **`C7 F8`（XBEGIN）/ `C6 F8`（XABORT）被误标 `C_BAD`**（两者都是合法指令）：成因是 `C6`/`C7`/`8F` 的 `reg != 0` 判据，现改用**字节精确**判据 `mpeek != 0xF8`（`:1425-1427`）；`reg != 0 && reg != 7` 那种写法会把 `C7 FA`/`C6 FA`/`C7 38` 等非法 `/7` 编码一并豁免（mutation M10b：`C7 FA`/`C6 FA` 的「仍应 bad」断言失败）。mutation M10（恢复 `reg != 0`）→ 2 条失败。
...
1. **`0F 1E`（无 `F3`）是 NOP Ev**（与 `0F 1F` 同类）：mod≠3 与 mod=3 都**不访问操作数**。新增 `nop1e`（判据 `map0F && LEGACY && c2 == 0x1E && p_rep == 0`，`:524-525`），加入 `reg_not_dst`（`:555-556`）、mod=3 的空操作数分支与内存分支的 `src` 排除（`!nop1e`，`:718`）。mutation → 6 条失败（断言在 `tests/xde_test.c:2034-2044`）。
...
5. **`0F AE` mod=3 的非法子形式 → `C_BAD`**：无 `F3` 的 `/0-/3` 与 `/4` 在表里仍是内存形式的编码，mod=3 时没有任何定义，原先当普通 `OTHER|OTHER` 指令报出；`xde_disasm_buf` 新增**字节精确**判据 `opcode == 0x0F && map == XDE_MAP_0F && opcode2 == 0xAE && (mpeek >> 6) == 3 && reg <= 4 && p_rep != 0xF3`（其中的 `p_rep!=0xF3` 就是 `F3` 豁免条件；`:1428-1435`）。mutation → 5 条失败；另加 9 条「不得误伤」对照（`lfence`/`mfence`/`sfence`/四个 FS/GS base 形式/`incsspd`/`fxsave` 内存形式都不带 `C_BAD`，`tests/xde_test.c:1551-1564`）。
6. **NOP Ev 的 mod=3 形式**（`{0F,1F,C0}`/`{0F,19,C0}`/`{0F,1D,C0}`/`{0F,18,E0}`）与 mod≠3 一致清空：`nop_ea`（`:533-535`）去掉 `mod != 3`，mod=3 分支与 `fence`/`endbr`/`nop1e` 共用空操作数分支。mutation（重新插回 `mod != 3 &&`）→ 8 条失败（断言在 `tests/xde_test.c:1792-1799`）。
7. **`F3 0F AE /5`（INCSSPD/INCSSPQ）**：读 r/m GPR、写 SSP（折 `OTHER`）。新增 `incssp`（`:483-484`，`map0F && LEGACY && c2 == 0xAE && mod == 3 && reg == 5 && p_rep == 0xF3`），`fence` 判据改成 `reg >= 5 && !incssp`（`:499-500`），加入 `reg_not_dst`（`:555-556`）与 mod=3 分支（`:680-685`）；`C_BAD` 保持 0。mutation A（去掉 `!incssp`）→ 4 条、B（`dst |= OTHER|rset`）→ 2 条；断言在 `tests/xde_test.c:1643-1656`。
...
4. **`0F 18 /0-/3` 与 `0F 0D /0-/1` 的 mod=3 是保留形式**（SDM 只定义 `m8`）→ `C_BAD`（`:1436-1443`）；`/4-/7` 的 mod=3 NOP 形式与内存形式保持干净。mutation：范围收窄 → 2 条失败。
...
5. **重复 `66`/`67` 的尺寸效应与自身字节不自洽**：原实现遇同组重复前缀时置 `C_BAD` 并**取消**该前缀的尺寸效应（留默认 `datasize`/`defaddr`）**却仍填 `p_66`/`p_67`** ⇒ struct 自相矛盾（`67 67 00 06` 写出 3 字节、重解码 5）。改为**最后者生效**（SDM：同组只有最后一个前缀有效；**段前缀本就是该口径**，`2E 26 06` → `26 06`），判据形如 `if (!twice) diza->defdata = …` / `if (!twice) { … defaddr … }`（`:1098-1099` / `:1107-1112`），`C_BAD` 保留。**有意后果**：部分重复前缀输入的 `len` 会变（如 `66` 生效后 Jcc 走 rel16 而非 rel32）——实测**没有任何既有断言因此变化**（新增恰为该轮的 22 条）。
6. **REX 紧邻 VEX/EVEX/XOP/REX2 引导字节**（`40 C5 04 08` 这类）：SDM 视作非法 `#UD`，此前被**静默接受**；而编码侧因 `nvex != 0` 永不发 REX ⇒ 丢字节（`66 48 D5 …` 还会变立即数宽度）。按已定口径置 `C_BAD`（判据在 `got_opcode`：`rex_seen && enc != XDE_ENC_LEGACY`，`:1346-1352`；`rex_seen` 在 `:1155` 置位），并配 3 条「不得误伤」对照（合法 `48 31 C0`、无 REX 的 `C5 04 08`、`vaddps`）。**全空间往返扫描与残差口径**见 Known Gaps「编码器契约与「已判 `C_BAD` 不保证往返」边界」条。
...
1. **`F3 0F AE /4` = PTWRITE**（r/m 是**只读** GPR 源或**只读内存**，**无目的操作数**）：新增 `ptwrite` 谓词（`:506-507`）、加进 `reg_not_dst`（`:555-556`）——`mod == 3` 形式因此走通用 r/m 读路径（不另设分支）、reg 字段不再冒充目的；内存形式另从 `mem_store`（`:549` 的 `(reg == 4 && !ptwrite)`）与 `xsave_mask`（`:572`）**排除**（不是写，也不读 `EDX:EAX`）。编码 `{F3,0F,AE,E0}`、`{F3,48,0F,AE,E0}`、`{F3,0F,AE,20}`，断言在 `tests/xde_test.c:1567` 起的块。
2. **`F3 0F AE /6` mod=3 = UMONITOR**（WAITPKG；r/m 只读 GPR）：新增 `umonitor`（`:491-492`），与同字段同 mod 的 `umwait`（`F2`，`:493-494`）、`tpause`（`66`，`:495-497`）折成 `waitpkg`（`:498`），`fence` 判据加 `&& !waitpkg`（`:499-500`）——否则 WAITPKG 三个会被**无操作数**的 MFENCE 吞掉；`waitpkg` 与 `ptwrite` 一并进 `reg_not_dst`。断言在 `tests/xde_test.c:1592` 起的块。
3. **`F2 0F AE /6` = UMWAIT、`66 0F AE /6` = TPAUSE**：除 r/m 只读外还读 `EDX:EAX`（唤醒期限）、写 `FL`（CF 报告唤醒原因）——`:750-753`。
4. **`F3 0F AE /6` mod≠3 = CLRSSBSY**（CET；清 shadow stack token 的 busy 标志）：`clrssbsy`（`:512-513`）——m64 读且写（由 `mem_store` 的 `reg == 6` 覆盖）、`dst_set |= XSET_FL`（`:756-757`）、`xsave_mask` 用 `!clrssbsy` 排除（`:573`）。断言在 `tests/xde_test.c:1629-1630` 起的块。
...
**不得误伤（最新一轮新加对照）**：`{F3,0F,AE,F8}` 的冗余 `F3` 仍是**无操作数**的 SFENCE（`tests/xde_test.c:1523-1529`）；既有的 `{0F,AE,E8/F0/F8}`（LFENCE/MFENCE/SFENCE）、`{F3,0F,AE,C0}`（RDFSBASE）、`{F3,0F,AE,E8}`（INCSSPD）对照（`tests/xde_test.c:1551-1564`）保持不动。
...
1. **`8D`（LEA）的 mod=3 → `C_BAD`**（`:1493-1495`）：10 条断言——`8D C0` 在 16/32/64、`8D FF`、`48 8D C0` 与 REX2 的 `D5 40 8D C0` **置位**，`8D 00`/`48 8D 00` 的内存形式三种模式**不置位**；断言在 `tests/xde_test.c:1818-1886`。mutation（删该判据）→ **恰 6 条**失败。
2. **`0F 13`/`0F 17`/`0F 2B` 的 mod=3 → `C_BAD`**（`:1502-1505`）：16 条断言——三个 opcode 各三模式、`66 0F 13`、REX2 的 `D5 80 13 C0` 置位，五个内存形式不置位。mutation（删该判据）→ **该组 11 条**失败。
3. **`0F 50`（MOVMSKPS/MOVMSKPD）的 mod≠3 → `C_BAD`**（`:1509-1511`）：9 条断言——裸/`66` 的 mod=0 与 `/7` mod=0 置位，`0F 50 C0`/`66 0F 50 C0` 的寄存器形式不置位。
4. **三组顺带 opcode**（`:1522-1529`「r/m 只在内存」、`:1534-1538`「r/m 只在寄存器」、`:1544-1555` 的 `0F 38` 组）：64 条断言——`66 0F 12/16` 11、LSS/LFS/LGS 与 MOVNTI/MOVNTQ/MOVNTDQ/LDDQU 16、PMOVMSKB/MASKMOVQ/MASKMOVDQU/MOVDQ2Q 18、`0F 38` 组 19，含守护合法形式的门（MOVHLPS/MOVLHPS/MOVSLDUP/MOVSHDUP/MOVDDUP、`crc32 r/m8`/`r/m32`、`0F D6` 的裸/`66` MOVQ r/m）；断言在 `tests/xde_test.c:1887-2024`。
...
**不做校验、不解码**；`xde_asm_buf`（`src/xde.c:1705-1754`）只多做一次容量检查，`xde_asm`（`:1756-1759`）是 `return xde_asm_buf(opcode, XDE_MAXLEN, diza);` 的薄包装。固定顺序拼接：

```
max_len 夹取：0 或 > XDE_MAXLEN 一律按 15                        (:1712-1713)
→ 计数夹到数组容量：nvex ≤ 4 / naddr ≤ 8 / ndata ≤ 8            (:1716-1718)
→ asm_size() 算所需字节数，> max_len 则 return 0                 (:1677-1703, :1720-1721)
p_lock → p_rep → p_seg → p_66 → p_67    （SDM 组序 1→2→3→4(:1728-1732)
...
**容量安全**：`addrsize`/`datasize`/`nvex` 是 `uint8_t`，被写坏时解码侧的计数会越过 `addr_b[8]`/`data_b[8]`/`vex[4]`（真 UB）。`xde_asm_buf` 先把这三个计数夹到数组容量（`:1716-1718`），再用 `asm_size()`（`:1677-1703`）算出所需字节数，`> max_len` 就返回 `0`（`:1720-1721`）；`max_len == 0` 或 `> XDE_MAXLEN` 一律按 `XDE_MAXLEN` 处理（`:1712-1713`），与 `xde_disasm_buf` 的 `max_len` 处理同构。所以解码成功的指令（`len ≤ 15`）经 `xde_asm` 永远编得出来（不变量与 13 项同步清单见 Known Gaps「`asm_size()` 是写出序列的第二份文本实现」条）。

`p_66` 现在与其它遗留前缀一起在 `:1731` 统一发射（没有 nvex 专属分支）：这是为 REX2 准备的——REX2 是唯一**保留** `p_66`/`p_rep` 的编码类（VEX/EVEX/XOP 在解码时就清掉，`:1261-1262`/`:1284-1285`/`:1224-1225`/`:1329-1330`），而它们的 `vex[]` 头自带 pp 字段，不走 `p_66`；所以 `66 D5 …` 现在能字节级往返。
...
- 返回 `0` 只有两种情形：`!opcode || !diza`（`:1710-1711`），或所需字节数 `> max_len`（`:1720-1721`）；解码成功的指令不可能编出 0 字节；
...
| `src/xde.c` | 解码器 + 编码器（1759 行），全部核心逻辑 | `xde_disasm_buf:1050`, `xde_asm_buf:1705`, `xde_asm:1756`, `asm_size:1677`, `undef_sys_operands:339`, 14 个 static 助手 + 三张 `static const` 掩码表：`x87_res3`/`x87_resm`（`:866-874`）与 `xde_pfx_0f`（`:889-922`） |
...
| `tests/xde_test.c` | 唯一测试文件（2640 行） | 9 个 `expect_*` 助手（含 `expect_seteq:194`）, `main:237` |
...
退出码 `0` = 全通过，`1` = 有失败。`main` 是 `int main(void)`，**没有任何 CLI 参数**（无 filter / verbose / 单用例选择），每次运行都执行全部 1044 项检查。
...
| `src/xde.c:1050-1664` | `xde_disasm_buf` —— 改解码行为从这里读起 |
| `src/xde.c:1705-1754` | `xde_asm_buf` —— 字节重组 + 容量检查（`xde_asm` 是它的包装，`:1756-1759`） |
| `src/xde.c:1677-1703` | `asm_size` —— 估算编码所需字节数，与 `xde_asm_buf` 的落地循环必须同步 |
| `src/xde.c:1196-1197` / `:1245-1248` / `:1309` | 三处编码类歧义门槛 |
...
**自研极简 harness，无第三方框架，无 `ASSERT`/`CHECK`/`TEST` 宏**。全部逻辑在 `tests/xde_test.c`（2640 行）的单个 `int main(void)`（`:237-2640`）中：127 个匿名 `{ }` 块，每块一个 `static const uint8_t` 向量紧跟 `expect_*` 调用。**零文件 IO、零 fixture、零 golden 文件**。
...
| `expect_flag(name, mode, b, n, bit, want)` | `:217-235` | 要求解码长度 == `n`，再断言单个 flag 位。`bit` 取单个 `C_*` 常量，`want` 1 = 置位、0 = 未置位；成功打 `flag 0x… set|clear`（442 次） |

### 覆盖矩阵（1044 项检查）

1044 = 1020 次助手调用（`expect_len` 46 + `expect_enc` 16 + `expect_fail` 23 + `expect_roundtrip` 6 + `expect_set` 416 + `expect_seteq` 65 + `expect_flag` 442 + `expect_sizes` 2 + `expect_selflen` 4）+ 24 项其余检查（24 项内联/打印器块——`C_RIPREL`、Object sets 打印器、闸门/asm 区段、规范前缀、`sete al` 非 undef、打印器最坏情况、Coverage 区段 ud2、Jcc/LOOP 打印器、`0F C7`/`0F AE` 保留编码等，逐项见覆盖矩阵各行）。**上一轮（批 A + 批 B）新增 61 项**：批 A 的 39 项落在 `// System groups…` 区段，批 B 的 22 项落在 `// Canonical…` 区段的两个新块（重复前缀折叠 14 + REX 紧邻引导字节 8）；**最新一轮新增 44 项**（批 A：PTWRITE 12 + WAITPKG 23 + CLRSSBSY 6 + SFENCE 对照 3，全在 `// System groups…` 区段；批 B 是只读扫描、不加断言）；**UNDEF 回填那轮新增 69 条断言、删旧锚点 2 条**——62 条 `expect_seteq` + 7 条 `C_UNDEF` 的 `expect_flag`，全在 `// System groups…` 区段的新块（`:2082-2209`），删掉的是旧的 `0F 00`/`0F 01` 整集 `C_UNDEF` 锚点 ⇒ 净增 67 项；**`C_BAD` 普查 + `CPUID`/`XABORT` 那轮再新增 124 项**——119 条 `expect_flag`（`62`(BOUND) 7 + `0F 00`/`0F 01` 保留槽 41 + x87 62 + `FF /3`/`/5` 9；其中 `C_BAD` 置位 68 条、未置位 51 条）+ 3 条 `expect_seteq`（`C6 F8` 的 `dst`/`src` + `C6 C0` 的 `dst`）+ 1 条 `expect_set`（`CPUID` 的 `ECX`）+ 1 条 `expect_enc`（32 位 EVEX 对照，证明 BOUND 的 `C_BAD` 不影响 `62` 的 EVEX 分派）⇒ 该轮净增 124 项；**随后那轮（mod 结构性判据）再净增 99 条 `expect_flag`**；**P2 轮（强制前缀判据）再新增 69 条 `expect_flag`、删 2 条**——新块 67 条全在 `// SSE mandatory prefixes…` 区段（`tests/xde_test.c:1326-1463`，29 条置位 / 38 条未置位），`// System groups…` 区段另加 2 条 `0F D6` 置位断言、同时删掉 2 条把裸 `0F D6` 当作合法 MOVQ 的旧断言 ⇒ 该区段计数不变 ⇒ **1044**（`expect_flag` 375 → 442，其余助手调用数不变）；`### 覆盖矩阵` 标题与 CLI 说明已同步改为 1044。以上计数都是对 **2640** 行文件的实测值（助手自身的定义行不计入调用）。
...
本轮（分组 opcode 口径）**没有动任何表**：改动全在 `src/xde.c` 的 `apply_modrm_usage`（新增 `mov_crdr`/`rdrand`/`fence`/`fsgsbase`/`endbr`/`nop_ea`/`prefetch_ea`/`mem_store` 八个 opcode+mod+reg 判定，并让通用规则让位）与 `xde_disasm_buf` 的 `C6`/`C7`/`8F` `C_BAD` 判据（加 `mpeek != 0xF8`），所以 `tools/gen_tables.py`、`src/xdetbl.c`、`src/xdetbl.h` 三者**逐字节未变**；`src/xde.c` 里带 `XA_VVVV_GPR` 的分支与 `simd_0f` 名单也一并保持原样。改这类「reg 字段是选择器而非寄存器」的组时，**三层都要看**：表属性、`apply_modrm_usage` 的 `mod == 3` / 内存两条分支、以及 `xde_disasm_buf` 里的组特例。此后两批同样**没有动任何表**：新增 `nop1e`/`incssp` 两个判定、去掉 `nop_ea` 的 mod 条件、给 `mem_store` 的 `0F C7` 列表加上 `/7`，并在 `xde_disasm_buf` 新增 `0F AE` mod=3 的 `C_BAD` 判据（`:1495-1502`）——`tools/gen_tables.py`、`src/xdetbl.c`、`src/xdetbl.h` 仍**逐字节未变**。**上一轮（批 A 提交 `fdce0f3` + 批 B）同样零表改动**：全部落在 `src/xde.c` 的 `apply_modrm_usage` 谓词表与 `xde_disasm_buf`（前缀循环的 `if (!twice)` + `got_opcode` 的 `rex_seen` 判据），`src/xdetbl.c`/`src/xdetbl.h`/`tools/gen_tables.py` 仍逐字节未变。**最新一轮（批 A）同样零表改动**：仍只动 `src/xde.c` 的 `apply_modrm_usage` 谓词表（`umonitor`/`umwait`/`tpause`/`waitpkg`/`ptwrite`/`clrssbsy`），`src/xdetbl.c`/`src/xdetbl.h`/`tools/gen_tables.py` 继续逐字节未变。**本轮（`0F 00`/`0F 01`/`0F 02`/`0F 03` 系统组回填）同样零表改动**：新增 `src/xde.c` 的 `undef_sys_operands`（`:308-428`）、它的调用与 `C_UNDEF` 清理（`:1627-1638`）以及寻址寄存器的 `ea_set` 暂存（`:1060`、`:1603-1606`）——表上的 `XA_UNDEF` 位**原样保留**，回填只在解码侧做，所以 `src/xdetbl.c`/`src/xdetbl.h`/`tools/gen_tables.py` 仍逐字节未变。

**本轮（`C_BAD` 定口径 + 表侧清理）是第一次按「`XA_BAD` = 该编码在任何模式下都不可用」改 legacy / 0F map**：`tools/gen_tables.py` 删掉 60 个 `XA_BAD` 槽（35 个三模式全合法的 legacy 槽 + 22 个带 `XA_I64` 的 legacy 槽 + `m1` 的 `0F B2`/`B4`/`B5`），并在 `:20-21` 给 `XA_BAD` 补注释「not a usable encoding in any mode; mode-limited forms use XA_I64/XA_O64/XA_F64/XA_D64 instead」；`src/xdetbl.c` 由重跑生成器产出（24 行改动、24 插 24 删，**字节数 40140 不变** ⇒ 只翻转属性位，`src/xdetbl.c` **未手工编辑**）；`src/xdetbl.h` 未动（无新常量，`check_header()` 通过）；`src/xde.c` 未动（64 位拒绝仍走 `:1359` 的 `XA_I64` 分支，与 `XA_BAD` 无关）。删后 `XA_BAD` 余 **28 槽** = `m0[0xD6]`（SALC，`XA_BAD|XA_OPSZ8`）+ `m0[0xF1]`（INT1/ICEBP，`XA_BAD|XA_UNDEF`）+ 26 个 group 槽。生成器幂等实测：连跑两次逐字节一致（md5 `3fc495d3a3ce7180179bd5e8ddd261f0`、40140 字节、退出码 0）。双向 mutation 实测：把 `XA_BAD` 加回 `m0[0xCC]`（INT3）→ 重跑生成器后**恰好 1 条**失败（`int3 not bad`）；删掉 `group[XG_5][7]` 的 `XA_BAD` → **恰好 1 条**失败（`FF /7 still bad`）；恢复后 md5 复原、`0 failure(s)`。测试侧同步加 90 条断言（见「覆盖缺口」与覆盖矩阵里 `tests/xde_test.c:2433-2620` 那一行）。
...
| `xde_disasm_buf(ptr, 0, …)` 没报错却按 15 字节解码 | `max_len == 0` 是「用默认 `XDE_MAXLEN`」而不是「零字节上限」（`src/xde.c:1066-1069`；`xde_asm_buf` 同构，`:1712-1713`）。要限长请传显式上界；批 B v2 的截断扫描因此把 `len == 1` 的 **8,437,552** 例单列跳过。 |

## Known Gaps / Cautions

以下是**代码可证**的已知缺口（作者未用 TODO 标注）：

- **REX2 保留 `p_66`/`p_rep`（刻意行为，不是缺口）**（对比 VEX `:1261-1262`/`:1284-1285`、EVEX `:1224-1225`、XOP `:1329-1330` 都清）——`66` 是 REX2 合法的遗留前缀，不是多余前缀。编码侧 `p_66` 与其它遗留前缀一起在 `:1731` 统一发射（无 nvex 专属分支），所以 `66 D5 …` 现在能字节级往返；改这里要解码/编码两侧一起动。
- **`C_ADDR8` 只来自 MOFFS（原先那条不可达分支已删）**：`parse_modrm` 里 `disp` 只会是 1/2/4（`:1001-1010`，末行注释写明），原来永不触发的 `else → C_ADDR8` 已删除；8 字节地址只经 MOFFS 路径（`:1617`）。新增位移宽度前先确认这条不变量还成立。
...
- **未定义编码只记录、不建模**：`0F 0D /2-/7` mod≠3、`F3 0F 1E /1..6`、`F3 0F 1E /0` 的 mod≠3、`F2 0F 1E C0`——UD/未定义编码，无可辩驳的目标语义 → 不写断言（`F3 0F AE /4` 的 mod=3 形式**最新一轮已按 PTWRITE 建模**，不再算「未定义」，见上条；`F3 0F AE /4` 因 `F3` 豁免而无 `C_BAD`；`0F AE` mod=3 的其余非法子形式此后那批已加 `C_BAD`，见 `src/xde.c:1495-1502`）。
...
- **objdump↔xde 分歧普查（带必需前缀重跑；快照 HEAD `1d7d37b`；只读，未改仓库任何文件）**：普查把「必需前缀」当成 opcode 选择子的 **80 个** opcode 规则补进来（`66`/`F2`/`F3` 作选择子者，如 `66 0F 6C/6D` 族、`F2 0F F0`、`F3 0F B8`、`66`+`F2 0F 7C/7D/D0`、`F3 0F 3A F0` 等），规范化形式两边即一致，残余只剩**裸前缀/错前缀**形式被 xde 接受。分类（槽数，三列 = m64 / m32 / m16）：
...
- **下一轮优先修清单（按槽数降序；P2 已完成，其余未修）**：
  - **P1** legacy `0F 38`/`0F 3A` 白名单掩码（m64 **501,504** / m32·16 355,584，复现 `0F 38 0C C0`）；
  - **P2 ✅ 已完成（本轮，提交 `5aa9840`）**：`0F` map 的必需前缀检查——判据在 `src/xde.c:1388-1394`、表是 `xde_pfx_0f[256]`（`:889-922`），见「`C_BAD` 的来源」第 20 条。**只覆盖 `0F` map**：原清单里那 80 个 opcode 中属于 `0F 38`/`0F 3A` 的部分没进这条判据（那两张转义表与 VEX/EVEX 共用，属 **P1** 的白名单范围）。**P2 轮重跑（64 位；10 种前缀 prologue × 4 张 map × 256 opcode × 256 ModR/M + 2,560 个 3DNow = 2,624,000 个候选，去重后 2,614,006 个唯一字节串；第二解码器 objdump 2.36.1，第三解码器 capstone 5.0.7 作旁证）**：`B`（objdump 判非法、xde 干净接受）**760,174 → 691,747**（修掉 **68,427**），`C` 8,695、`A` 21,573、`OK` 1,132,140、`E` 670,546 **均不变**，`D`（两边都非法）30,872 → 99,299；残余 `B` 691,747 按 map 分：**0F38 484,799** / **0F3A 182,469** / 0F 23,471 / legacy 单字节 952（全是 REX2/APX）/ XOP 9 56 ⇒ 剩余优先项是 **P1**（legacy `0F 38`/`0F 3A` 白名单，槽数最大）与 **P5**（操作数形式）。复现：`0F 6C C0`（缺 `66`）现 `C_BAD=1`，`66 0F 6C C0`（PUNPCKLQDQ）`C_BAD=0`；
  - **P3** 其它未定义编码（`0F 7A`/`7B`、`0F 77`、冗余前缀族，52,854 / 48,534，复现 `0F 7A C0`、`F2 0F 13 C0`）；
  - **P4** 3DNow 选择子掩码（**232** 个未定义选择子 vs objdump 认可的 24 个，`x87_res3` 风格，复现 `0F 0F C0 00`）；
  - **P5** 操作数形式谓词的 48 组（36,275；头部 `0F 73`/`72`/`71` 12,624、`0F A6`/`A7` 12,072、`0F D7` 4,608、`0F F7`/`C5`/`50` 各 3,456、`0F 1A`/`1B` 6,588、`0F C7` 2,440、`8D`/`0F B2`/`B4`/`B5`/`0F 2B` 各 1,536、`0F 38 F0`/`F1` 2,304、`0F 38 F6`/`F9` 1,920、`0F E7`/`D6`/`78`/`79`/`13`/`17`/`0D` 各 1,152、`0F C3` 960、`0F AE` 897、`0F 18` 768、`0F 3A F0` 765）。其中与 mod 结构性规则相关的 `8D`、`0F 50`、`0F 13`/`17`、`0F 2B`、`0F B2`/`B4`/`B5`、`0F C3`、`0F D6`/`D7`/`E7`/`F7`、`0F 38 F0`/`F1`/`F6`/`F9` 已由本轮批 A 的判据覆盖（P5 的槽数是 `1d7d37b` 快照值，未按修复后重跑核减）。
- **P2 轮之后的有意残余（第二/第三解码器互相冲突 ⇒ 不标 `C_BAD`）**：`0F 18 /4-/7` 的 mod=3（objdump 2.36.1 判 `(bad)`，capstone 5.0.7 / LLVM 解作 `nop eax`，**224 例**）与 `66`/`F2 0F 01` 的 mod=3（objdump 判 `(bad)`，capstone 判合法，**15 例**）——SDM 对这两个形状没有定义，两个解码器各执一词，按「不制造假阳性」的口径**有意保留**（`0F 18 /4-/7` 的 mod=3 是 NOP，见上文「`0F 18 /0-/3` 与 `0F 0D /0-/1` 的 mod=3 是保留形式，不是 NOP」条）。
- **objdump 2.36.1 的工具缺口（APX / REX2）**：残余 `B` 里 legacy 单字节那 **952** 例全是 REX2/APX 形式，objdump 2.36.1 不认识这些编码、一律报 `(bad)` ⇒ 这是**工具缺口**，不计入 P1-P5，也不据此给它们标 `C_BAD`。
- **不要动清单（普查发现但按现有口径不修）**：`B2` 的裸前缀行伪影；`C` 类（`0F B9` UD1、`F1` ICEBP、重复前缀、REX 紧邻引导字节——已在本文件声明为**有意**）；`A` 类（32/16 位的 SYSCALL/SYSRET、MOV TR）；`D5` REX2 与合法的 `0F 38 F6 00`、`F3 0F 3A F0 C0`；截断残余（3,267 例中 2,890 一致）属 objdump 工具缺口；`0F 18 /4-/7` mod=3 与 `66`/`F2 0F 01` mod=3 属上面两条记的第二解码器冲突。
- **上一轮普查的副产物已由本轮批 A 收尾**：上一轮记的「前后两次字节计数差值 **296**、`LEA 8D` 的 mod=3 是明确的下一候选」已在本轮批 A 处理（`8D`、`0F 13`/`17`/`2B`、`0F 50` 与三组顺带 opcode，见上文「本轮（mod 结构性违规普查）」段与「`C_BAD` 的来源」第 14-19 条）；其余候选转入上面的 P1-P5。
