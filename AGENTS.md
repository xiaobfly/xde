...
| 函数 | 语义 |
|------|------|
| `int xde_disasm(const uint8_t *opcode, struct xde_instr *diza)` | 64 位模式解码（`src/xde.c:1607`） |
| `int xde_disasm_ex(const uint8_t *opcode, struct xde_instr *diza, unsigned mode)` | 指定 16/32/64（`src/xde.c:1602`） |
| `int xde_disasm_buf(const uint8_t *opcode, unsigned max_len, struct xde_instr *diza, unsigned mode)` | 额外限制读取上限（`src/xde.c:1002`） |
| `int xde_asm(uint8_t *opcode, const struct xde_instr *diza)` | 结构 → 字节；`return xde_asm_buf(opcode, XDE_MAXLEN, diza);` 的包装（`src/xde.c:1692`） |
| `int xde_asm_buf(uint8_t *opcode, unsigned max_len, const struct xde_instr *diza)` | 结构 → 字节，额外限制写入上限；装不下返回 0（`src/xde.c:1641`） |
...
**返回值契约**：解码返回指令长度，`0` = 失败（截断 / 该模式下非法 / undefined）。编码返回写入字节数，`xde_asm_buf` 在结构体所需字节数 `> max_len` 时返回 `0`（`xde_asm` 给的是 `XDE_MAXLEN`，所以除空指针外永远成功）。没有错误码枚举、没有 errno、没有 out-param 状态。**注意 `max_len == 0` 不是「零字节上限」而**是「用默认 `XDE_MAXLEN`(15)」（`src/xde.c:1018-1021`，`xde_asm_buf` 同构 `:1648-1649`）——传 0 不会失败，而是按 15 解码 / 编码；`max_len == 16` 也被夹到 15（同一处 `> XDE_MAXLEN` 判定，两边同构），所以「传 16 就多写一字节」的期待不成立；要限长必须传显式上界（批 B v2 的截断扫描因此在 `len == 1`（即 `len-1 == 0`）的 **8,437,552** 例上单列跳过）。
...
```
consumer
  └─ xde_disasm / xde_disasm_ex            (src/xde.c:1607 / :1602, 只差默认参数)
       └─ xde_disasm_buf(ptr, max_len, d, mode)   ← 唯一真正的入口 (src/xde.c:1002-1600)
  ...
  └─ xde_asm(out, d) → xde_asm_buf(out, XDE_MAXLEN, d)   (src/xde.c:1692 / `:1641-1690`, 纯字节重组)
```
...
| 20 | 硬编码特例：移位组 `C0 C1 D0-D3` 写 FL（`RCL`/`RCR` 另读 FL、`D2`/`D3` 另读 CL）／`C6 C7 8F` 的 `C_BAD`（`F8` 例外 = `XBEGIN`/`XABORT`）／**`C_BAD` 普查的六个判据（`0F 00 /6 /7`、`0F 01` 内存 `/5` 与 mod=3 保留子槽、`FF /3`/`/5` mod=3、`62` mod=3、x87 `D8-DF`，见「`C_BAD` 的来源」第 8-13 条）**／**本轮 mod 结构性判据（`8D`、`0F 13`/`0F 17`/`0F 2B`、`0F 50` 与三组顺带 opcode，见第 14-19 条）**／`F6`/`F7` 的标志与 `MUL`/`DIV` 累加器规则 | `:1346-1531` |
| 21 | `parse_modrm` + `apply_modrm_usage` | `:1533-1544` |
| 22 | MOFFS 路径（非 ModR/M 的 `A0-A3` 等） | `:1545-1558` |
| 23 | `apply_usage_special` / `apply_implicit_gp` | `:1560-1561` |
| 24 | `XA_UNDEF` → `src_set = dst_set = XSET_UNDEF`、`src_set2 = dst_set2 = XSET2_ALL`（**赋值，非 OR**）；随后 `undef_sys_operands` 按 ModR/M 回填 `0F 00`/`0F 01`/`0F 02`/`0F 03` 已建模形式的真实集合并清 `C_UNDEF`，未回填者保持整集 + `C_UNDEF` | `:1563-1574`（实现 `:308-428`，`ea_set` 暂存 `:1539-1542`，调用 `:1572`） |
| 25 | `imm_bytes` 定长 → 拷贝到 `data_b` + `C_DATA*` | `:1576-1589` |
| 26 | 收尾：`len = cur.p - opcode`；`0` 或 `>15` → 0；置 `len`；返回 | `:1591-1599` |

**3DNow 没有特例分支**：表把尾随 opcode 字节建模成 `XA_IMM_IB`，`XA_3DNOW` 只负责置 `C_3DNOW`（收尾注释 `:1591-1592`，现在是准确描述而非待办）。

**移位组与组 3 都写标志**：legacy map 的 `C0`/`C1`/`D0-D3`（八个操作）一律 `dst_set |= XSET_FL`；`reg == 2 || 3`（`RCL`/`RCR`）另加 `src_set |= XSET_FL`（读 CF），`D2`/`D3` 另加 `src_set |= XSET_CL`。`F6`/`F7` 在 `reg != 2` 时 `dst_set |= XSET_FL`（`NOT`(/2) 不写标志），`/4`-`/7` 的累加器规则不变（`:1346-1531`）。这与 ALU 路径一致（`:628`/`:632`/`:784`/`:818`/`:824`）：凡写标志都要在 `dst_set` 里出现 `XSET_FL`。
...
**`C_BAD` 的来源**（共 19 类；第 1-7 类是原有判据，第 8-13 类是 `C_BAD` 普查新增的六个 `0F`/`FF`/`62`/x87 判据，第 14-19 类是本轮 mod 结构性判据——三个显式项加三组顺带 opcode，全部落在 `xde_disasm_buf` 的 `got_opcode` 段）：
...
13. **x87 逃逸表（`D8-DF`）的空白槽**（251 槽；`:1495-1504`）——两张掩码表：`x87_resm[8]`（`:872-874`，内存形式的 `/reg` 位掩码，SDM 空白的是 `D9 /1`、`DB /4`、`DB /6`、`DD /5`，共 96 槽）与 `x87_res3[8]`（`:866-871`，mod=3 的 `bit(reg*8+rm)` 掩码，155 槽），合计 96 + 155 = 251。**有意排除 12 槽**：`DB E0/E1/E4/E5`（8087/287 的 FENI/FDISI/FSETPM/FRSTPM）与 `DF C0-C7`（FFREEP）在 SDM 里同样空白，但 binutils 仍给它们助记符，标 `C_BAD` 会是假阳性 ⇒ 表里留 0，并有 `not bad` 对照断言钉住（见 Known Gaps「应标 `C_BAD` 却未标：431 槽」条的两条告诫）。

14. **`LEA`（`8D /r`）的 mod=3**（每模式 64 槽；`:1429-1431`）——SDM 把第二操作数写成内存操作数（算地址），mod=3 不是它的任何编码。判据只看 `map == XDE_MAP_LEGACY`，而 REX2 的 `8D` 也落在 legacy map（`legacy_enc` 在 `:1337` 覆盖 REX2），所以两个编码类一并命中；`lea [eax]` 的三种模式不误标。
15. **`0F 13`/`0F 17`/`0F 2B` 的 mod=3**（`:1438-1441`）——MOVLPS/MOVHPS 只定义存储进内存的 m64（`66` 的 MOVLPD/MOVHPD 与 REX2 形式同 r/m），MOVNTPS 只写 m128；**`0F 12`/`0F 16` 的载入形式不在此列**：它们的 mod=3 是 MOVHLPS/MOVLHPS，合法。
16. **`0F 50`（MOVMSKPS/MOVMSKPD）的 mod≠3**（`:1445-1447`）——r/m 必须取 xmm 寄存器，其它 mod 不是它的编码；`66` 形式同判据。
17. **第二组「r/m 只定义在内存」的 0F opcode 的 mod=3**（`:1458-1465`，8 条规则）——`66 0F 12`/`66 0F 16`（MOVLPD/MOVHPD，门 `p_66 && !p_rep`）、`0F B2`/`B4`/`B5`（LSS/LFS/LGS）、`0F C3`（MOVNTI）、`0F E7`（MOVNTQ，`66` 形式为 MOVNTDQ，门 `!p_rep`）、`F2 0F F0`（LDDQU，门 `p_rep == 0xF2`）；门槛的作用是**保住合法形式**——`MOVHLPS`/`MOVLHPS`/`MOVSLDUP`/`MOVSHDUP`/`MOVDDUP` 都有寄存器 r/m，不得误标。
18. **第二组「r/m 只定义在寄存器」的 0F opcode 的 mod≠3**（`:1470-1474`，3 条规则）——`0F D7`/`66 0F D7`（PMOVMSKB）、`0F F7`/`66 0F F7`（MASKMOVQ/MASKMOVDQU，门 `!p_rep`）、`F2 0F D6`（MOVDQ2Q，门 `p_rep == 0xF2`）；`0F D6` 的裸/`66` 形式是 MOVQ r/m、mod=3 合法，**不得误标**。
19. **`0F 38` 的 memory-only 组（mod=3）**（`:1480-1491`，10 条规则）——`66 0F 38 2A`（MOVNTDQA）、`66 0F 38 80/81/82`（INVEPT/INVVPID/INVPCID）、`0F 38 F0`/`F1`（MOVBE，门 `!p_rep`，从而 `F2` 读作 CRC32 时保持合法）、`66 0F 38 F5`（WRUSSD/WRUSSQ）、`0F 38 F6`（WRSSD/WRSSQ，门 `!p_66 && !p_rep`）、`66 0F 38 F8`（MOVDIR64B）与 `F2 0F 38 F8`（ENQCMD/ENQCMDS）、`0F 38 F9`（MOVDIRI）。
...
尺寸→标志：1 → `C_DATA1`，2 → `C_DATA2`，**3 → `C_DATA1 \| C_DATA2`**，4 → `C_DATA4`，8 → `C_DATA8`，**6 → `C_DATA4 \| C_DATA2`**（`:1583-1588`）。
...
1. **`0F 1E`（无 `F3`）是 NOP Ev**（与 `0F 1F` 同类）：mod≠3 与 mod=3 都**不访问操作数**。新增 `nop1e`（判据 `map0F && LEGACY && c2 == 0x1E && p_rep == 0`，`:524-525`），加入 `reg_not_dst`（`:555-556`）、mod=3 的空操作数分支与内存分支的 `src` 排除（`!nop1e`，`:718`）。mutation → 6 条失败（断言在 `tests/xde_test.c:1892-1902`）。
...
1. **`8D`（LEA）的 mod=3 → `C_BAD`**（`:1429-1431`）：10 条断言——`8D C0` 在 16/32/64、`8D FF`、`48 8D C0` 与 REX2 的 `D5 40 8D C0` **置位**，`8D 00`/`48 8D 00` 的内存形式三种模式**不置位**；断言在 `tests/xde_test.c:1679-1747`。mutation（删该判据）→ **恰 6 条**失败。
2. **`0F 13`/`0F 17`/`0F 2B` 的 mod=3 → `C_BAD`**（`:1438-1441`）：16 条断言——三个 opcode 各三模式、`66 0F 13`、REX2 的 `D5 80 13 C0` 置位，五个内存形式不置位。mutation（删该判据）→ **该组 11 条**失败。
3. **`0F 50`（MOVMSKPS/MOVMSKPD）的 mod≠3 → `C_BAD`**（`:1445-1447`）：9 条断言——裸/`66` 的 mod=0 与 `/7` mod=0 置位，`0F 50 C0`/`66 0F 50 C0` 的寄存器形式不置位。
4. **三组顺带 opcode**（`:1458-1465`「r/m 只在内存」、`:1470-1474`「r/m 只在寄存器」、`:1480-1491` 的 `0F 38` 组）：64 条断言——`66 0F 12/16` 11、LSS/LFS/LGS 与 MOVNTI/MOVNTQ/MOVNTDQ/LDDQU 16、PMOVMSKB/MASKMOVQ/MASKMOVDQU/MOVDQ2Q 18、`0F 38` 组 19，含守护合法形式的门（MOVHLPS/MOVLHPS/MOVSLDUP/MOVSHDUP/MOVDDUP、`crc32 r/m8`/`r/m32`、`0F D6` 的裸/`66` MOVQ r/m）；断言在 `tests/xde_test.c:1748-1882`。
...
**不做校验、不解码**；`xde_asm_buf`（`src/xde.c:1641-1690`）只多做一次容量检查，`xde_asm`（`:1692-1695`）是 `return xde_asm_buf(opcode, XDE_MAXLEN, diza);` 的薄包装。固定顺序拼接：

```
max_len 夹取：0 或 > XDE_MAXLEN 一律按 15                        (:1648-1649)
→ 计数夹到数组容量：nvex ≤ 4 / naddr ≤ 8 / ndata ≤ 8            (:1652-1654)
→ asm_size() 算所需字节数，> max_len 则 return 0                 (:1613-1639, :1656-1657)
p_lock → p_rep → p_seg → p_66 → p_67    （SDM 组序 1→2→3→4(:1664-1668)
...
**容量安全**：`addrsize`/`datasize`/`nvex` 是 `uint8_t`，被写坏时解码侧的计数会越过 `addr_b[8]`/`data_b[8]`/`vex[4]`（真 UB）。`xde_asm_buf` 先把这三个计数夹到数组容量（`:1652-1654`），再用 `asm_size()`（`:1613-1639`）算出所需字节数，`> max_len` 就返回 `0`（`:1656-1657`）；`max_len == 0` 或 `> XDE_MAXLEN` 一律按 `XDE_MAXLEN` 处理（`:1648-1649`），与 `xde_disasm_buf` 的 `max_len` 处理同构。所以解码成功的指令（`len ≤ 15`）经 `xde_asm` 永远编得出来（不变量与 13 项同步清单见 Known Gaps「`asm_size()` 是写出序列的第二份文本实现」条）。

`p_66` 现在与其它遗留前缀一起在 `:1667` 统一发射（没有 nvex 专属分支）：这是为 REX2 准备的——REX2 是唯一**保留** `p_66`/`p_rep` 的编码类（VEX/EVEX/XOP 在解码时就清掉，`:1213-1214`/`:1236-1237`/`:1176-1177`/`:1281-1282`），而它们的 `vex[]` 头自带 pp 字段，不走 `p_66`；所以 `66 D5 …` 现在能字节级往返。
...
- 返回 `0` 只有两种情形：`!opcode || !diza`（`:1646-1647`），或所需字节数 `> max_len`（`:1656-1657`）；解码成功的指令不可能编出 0 字节；
...
| `src/xde.c` | 解码器 + 编码器（1695 行），全部核心逻辑 | `xde_disasm_buf:1002`, `xde_asm_buf:1641`, `xde_asm:1692`, `asm_size:1613`, `undef_sys_operands:339`, 14 个 static 助手 + `x87_res3`/`x87_resm` 两张 x87 保留槽掩码表（`:866-874`） |
...
| `tests/xde_test.c` | 唯一测试文件（2498 行） | 9 个 `expect_*` 助手（含 `expect_seteq:194`）, `main:237` |
...
退出码 `0` = 全通过，`1` = 有失败。`main` 是 `int main(void)`，**没有任何 CLI 参数**（无 filter / verbose / 单用例选择），每次运行都执行全部 977 项检查。
...
| `src/xde.c:1002-1600` | `xde_disasm_buf` —— 改解码行为从这里读起 |
| `src/xde.c:1641-1690` | `xde_asm_buf` —— 字节重组 + 容量检查（`xde_asm` 是它的包装，`:1692-1695`） |
| `src/xde.c:1613-1639` | `asm_size` —— 估算编码所需字节数，与 `xde_asm_buf` 的落地循环必须同步 |
...
**自研极简 harness，无第三方框架，无 `ASSERT`/`CHECK`/`TEST` 宏**。全部逻辑在 `tests/xde_test.c`（2498 行）的单个 `int main(void)`（`:237-2498`）中：126 个匿名 `{ }` 块，每块一个 `static const uint8_t` 向量紧跟 `expect_*` 调用。**零文件 IO、零 fixture、零 golden 文件**。
...
| `expect_flag(name, mode, b, n, bit, want)` | `:217-235` | 要求解码长度 == `n`，再断言单个 flag 位。`bit` 取单个 `C_*` 常量，`want` 1 = 置位、0 = 未置位；成功打 `flag 0x… set|clear`（375 次） |

### 覆盖矩阵（977 项检查）

977 = 953 次助手调用（`expect_len` 46 + `expect_enc` 16 + `expect_fail` 23 + `expect_roundtrip` 6 + `expect_set` 416 + `expect_seteq` 65 + `expect_flag` 375 + `expect_sizes` 2 + `expect_selflen` 4）+ 24 项其余检查（24 项内联/打印器块——`C_RIPREL`、Object sets 打印器、闸门/asm 区段、规范前缀、`sete al` 非 undef、打印器最坏情况、Coverage 区段 ud2、Jcc/LOOP 打印器、`0F C7`/`0F AE` 保留编码等，逐项见覆盖矩阵各行）。**上一轮（批 A + 批 B）新增 61 项**：批 A 的 39 项落在 `// System groups…` 区段，批 B 的 22 项落在 `// Canonical…` 区段的两个新块（重复前缀折叠 14 + REX 紧邻引导字节 8）；**最新一轮新增 44 项**（批 A：PTWRITE 12 + WAITPKG 23 + CLRSSBSY 6 + SFENCE 对照 3，全在 `// System groups…` 区段；批 B 是只读扫描、不加断言）；**UNDEF 回填那轮新增 69 条断言、删旧锚点 2 条**——62 条 `expect_seteq` + 7 条 `C_UNDEF` 的 `expect_flag`，全在 `// System groups…` 区段的新块（`:1940-2067`），删掉的是旧的 `0F 00`/`0F 01` 整集 `C_UNDEF` 锚点 ⇒ 净增 67 项；**`C_BAD` 普查 + `CPUID`/`XABORT` 那轮再新增 124 项**——119 条 `expect_flag`（`62`(BOUND) 7 + `0F 00`/`0F 01` 保留槽 41 + x87 62 + `FF /3`/`/5` 9；其中 `C_BAD` 置位 68 条、未置位 51 条）+ 3 条 `expect_seteq`（`C6 F8` 的 `dst`/`src` + `C6 C0` 的 `dst`）+ 1 条 `expect_set`（`CPUID` 的 `ECX`）+ 1 条 `expect_enc`（32 位 EVEX 对照，证明 BOUND 的 `C_BAD` 不影响 `62` 的 EVEX 分派）⇒ **977**（上一轮净增 124、本轮 mod 结构性判据再 +99 条 `expect_flag`）；`### 覆盖矩阵` 标题与 CLI 说明已同步改为 977。以上计数都是对 **2498** 行文件的实测值（助手自身的定义行不计入调用）。
...
本轮（分组 opcode 口径）**没有动任何表**：改动全在 `src/xde.c` 的 `apply_modrm_usage`（新增 `mov_crdr`/`rdrand`/`fence`/`fsgsbase`/`endbr`/`nop_ea`/`prefetch_ea`/`mem_store` 八个 opcode+mod+reg 判定，并让通用规则让位）与 `xde_disasm_buf` 的 `C6`/`C7`/`8F` `C_BAD` 判据（加 `mpeek != 0xF8`），所以 `tools/gen_tables.py`、`src/xdetbl.c`、`src/xdetbl.h` 三者**逐字节未变**；`src/xde.c` 里带 `XA_VVVV_GPR` 的分支与 `simd_0f` 名单也一并保持原样。改这类「reg 字段是选择器而非寄存器」的组时，**三层都要看**：表属性、`apply_modrm_usage` 的 `mod == 3` / 内存两条分支、以及 `xde_disasm_buf` 里的组特例。此后两批同样**没有动任何表**：新增 `nop1e`/`incssp` 两个判定、去掉 `nop_ea` 的 mod 条件、给 `mem_store` 的 `0F C7` 列表加上 `/7`，并在 `xde_disasm_buf` 新增 `0F AE` mod=3 的 `C_BAD` 判据（`:1431-1438`）——`tools/gen_tables.py`、`src/xdetbl.c`、`src/xdetbl.h` 仍**逐字节未变**。**上一轮（批 A 提交 `fdce0f3` + 批 B）同样零表改动**：全部落在 `src/xde.c` 的 `apply_modrm_usage` 谓词表与 `xde_disasm_buf`（前缀循环的 `if (!twice)` + `got_opcode` 的 `rex_seen` 判据），`src/xdetbl.c`/`src/xdetbl.h`/`tools/gen_tables.py` 仍逐字节未变。**最新一轮（批 A）同样零表改动**：仍只动 `src/xde.c` 的 `apply_modrm_usage` 谓词表（`umonitor`/`umwait`/`tpause`/`waitpkg`/`ptwrite`/`clrssbsy`），`src/xdetbl.c`/`src/xdetbl.h`/`tools/gen_tables.py` 继续逐字节未变。**本轮（`0F 00`/`0F 01`/`0F 02`/`0F 03` 系统组回填）同样零表改动**：新增 `src/xde.c` 的 `undef_sys_operands`（`:308-428`）、它的调用与 `C_UNDEF` 清理（`:1563-1574`）以及寻址寄存器的 `ea_set` 暂存（`:1012`、`:1539-1542`）——表上的 `XA_UNDEF` 位**原样保留**，回填只在解码侧做，所以 `src/xdetbl.c`/`src/xdetbl.h`/`tools/gen_tables.py` 仍逐字节未变。
...
| `xde_disasm_buf(ptr, 0, …)` 没报错却按 15 字节解码 | `max_len == 0` 是「用默认 `XDE_MAXLEN`」而不是「零字节上限」（`src/xde.c:1018-1021`；`xde_asm_buf` 同构，`:1648-1649`）。要限长请传显式上界；批 B v2 的截断扫描因此把 `len == 1` 的 **8,437,552** 例单列跳过。 |

## Known Gaps / Cautions

以下是**代码可证**的已知缺口（作者未用 TODO 标注）：

- **REX2 保留 `p_66`/`p_rep`（刻意行为，不是缺口）**（对比 VEX `:1213-1214`/`:1236-1237`、EVEX `:1176-1177`、XOP `:1281-1282` 都清）——`66` 是 REX2 合法的遗留前缀，不是多余前缀。编码侧 `p_66` 与其它遗留前缀一起在 `:1667` 统一发射（无 nvex 专属分支），所以 `66 D5 …` 现在能字节级往返；改这里要解码/编码两侧一起动。
- **`C_ADDR8` 只来自 MOFFS（原先那条不可达分支已删）**：`parse_modrm` 里 `disp` 只会是 1/2/4（`:953-962`，末行注释写明），原来永不触发的 `else → C_ADDR8` 已删除；8 字节地址只经 MOFFS 路径（`:1553`）。新增位移宽度前先确认这条不变量还成立。
...
- **编码侧已按 SDM 组序规范化前缀**：`xde_asm_buf` 现在按 SDM 组序发射遗留前缀——lock/rep（组1）→ segment（组2）→ `66`（组3）→ `67`（组4）（`:1664-1668`），随后才是 REX 或 `vex[]` + opcode；`asm_size()`（`:1613-1639`）的计数顺序同步调整（字节总数不变）。解码侧仍不限制前缀顺序，所以非规范序输入（如 `67 66 90`、`64 F3 A4`）能解码，但**重编码会规范化**为 `66 67 90`、`F3 64 A4`——`xde_asm` 的输出不再逐字节等于非规范输入，往返测试因此只用规范序输入，或显式断言规范化结果。**批 B 起同组重复前缀也折叠为「最后者生效」**：`66 66 …`/`67 67 …` 只发一个前缀，输出必然短于输入（这类输入改用 `expect_selflen` 钉自洽性，`expect_roundtrip` 表达不了）。
...
- **未定义编码只记录、不建模**：`0F 0D /2-/7` mod≠3、`F3 0F 1E /1..6`、`F3 0F 1E /0` 的 mod≠3、`F2 0F 1E C0`——UD/未定义编码，无可辩驳的目标语义 → 不写断言（`F3 0F AE /4` 的 mod=3 形式**最新一轮已按 PTWRITE 建模**，不再算「未定义」，见上条；`F3 0F AE /4` 因 `F3` 豁免而无 `C_BAD`；`0F AE` mod=3 的其余非法子形式此后那批已加 `C_BAD`，见 `src/xde.c:1431-1438`）。
...
| `E` | 两边都拒绝 | 692,119 | 425,285 | 425,285 |
  ...
  `B` 的子类（m64）：操作数形式违规 **36,275**、必需前缀缺失/错 **180,992**、legacy `0F 38`/`0F 3A` **501,504**、其它未定义被接受 52,854、裸前缀伪影 1,409。**3DNow**：objdump 认可 24 个选择子，xde 接受 **232** 个未定义选择子（复现 `0F 0F C0 00`）。**截断**：3,267 例中 2,890 例两边一致（**91%**），残余是 objdump 工具缺口（APX REX2、ICEBP、`0F 05`/`0F 07` 模式不敏感）。
- **下一轮优先修清单（按槽数降序；均未修）**：
  - **P1** legacy `0F 38`/`0F 3A` 白名单掩码（m64 **501,504** / m32·16 355,584，复现 `0F 38 0C C0`）；
  - **P2** **80 个** opcode 的必需前缀检查（180,992 / 119,552，复现 `0F 6C C0`）；
...
- **不要动清单（普查发现但按现有口径不修）**：`B2` 的裸前缀行伪影；`C` 类（`0F B9` UD1、`F1` ICEBP、重复前缀、REX 紧邻引导字节——已在本文件声明为**有意**）；`A` 类（32/16 位的 SYSCALL/SYSRET、MOV TR）；`D5` REX2 与合法的 `0F 38 F6 00`、`F3 0F 3A F0 C0`；截断残余（3,267 例中 2,890 一致）属 objdump 工具缺口。
