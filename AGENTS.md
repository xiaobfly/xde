...
**API 一览**（`include/xde.h:272-282`，全部 `__cdecl`，无 export/visibility 宏）：

| 函数 | 语义 |
|------|------|
| `int xde_disasm(const uint8_t *opcode, struct xde_instr *diza)` | 64 位模式解码（`src/xde.c:996`） |
| `int xde_disasm_ex(const uint8_t *opcode, struct xde_instr *diza, unsigned mode)` | 指定 16/32/64（`src/xde.c:991`） |
| `int xde_disasm_buf(const uint8_t *opcode, unsigned max_len, struct xde_instr *diza, unsigned mode)` | 额外限制读取上限（`src/xde.c:582`） |
| `int xde_asm(uint8_t *opcode, const struct xde_instr *diza)` | 结构 → 字节（`src/xde.c:1001`） |
...
| `void xde_sprintset2(char *output, uint64_t set2)` | 第二对象集字 → 串，缓冲区 ≥256 字节（声明 `include/xde.h:282`，实现 `src/xde_text.c:107`） |

**返回值契约**：解码返回指令长度，`0` = 失败（截断 / 该模式下非法 / undefined）。没有错误码枚举、没有 errno、没有 out-param 状态。

用法（`README.md:55-63` 原文）：
...
```
consumer
  └─ xde_disasm / xde_disasm_ex            (src/xde.c:996 / :991, 只差默认参数)
       └─ xde_disasm_buf(ptr, max_len, d, mode)   ← 唯一真正的入口 (src/xde.c:582-989)
            ├─ xde_attr[map][opcode]      (查表, :870)  ← src/xdetbl.c 生成
            ├─ xde_group[gid][modrm.reg]  (二次查表, :896)
            └─ parse_modrm (:455) → apply_modrm_usage (:250)
                                  → apply_usage_special (:139)
                                  → apply_implicit_gp (:365)
  └─ xde_asm(out, d)                       (src/xde.c:1001-1036, 纯字节重组)
```
...
| # | 阶段 | 行 |
|---|------|-----|
| 1 | 入参守卫：空指针 → 0；`mode ∉ {16,32,64}` → 0；`max_len` 0/超限一律夹到 15 | `:593-600` |
| 2 | `memset` 清零 + 预置 `mode` / `defaddr` / `defdata` | `:602-605` |
| 3 | 游标初始化 `beg`/`p`/`end` | `:607-609` |
| 4 | `C_BAD` 启发式 | `:611-615` |
| 5 | 遗留前缀循环 | `:617-664` |
| 6 | 取下一个字节 | `:667-668` |
| 7 | REX（`40-4F`，仅 64 位） | `:670-682` |
| 8 | REX2（`D5`，仅 64 位） | `:684-715` |
| 9 | EVEX（`62`） | `:717-766` |
| 10 | VEX（`C4`/`C5`） | `:768-826` |
| 11 | XOP（`8F`） | `:828-860` |
| 12 | 遗留 opcode + `enc = XDE_ENC_LEGACY` | `:862-865` |
| 13 | 汇合点标签 `got_opcode:` | `:867` |
| 14 | map 越界检查 + `attr = xde_attr[map][mop]` | `:868-870` |
| 15 | 三种拒绝：`XA_INVALID` → 0；`XA_I64 && mode==64` → 0；`XA_O64 && mode!=64` → 0 | `:872-877` |
| 16 | `apply_attr_flags`：`XA_*` → `C_*` 映射 | `:879`（实现 `:116-137`） |
| 17 | `XA_GROUP` 强制按 ModR/M 处理 | `:882-885` |
| 18 | peek ModR/M，取 `reg = (mpeek >> 3) & 7` | `:887-892` |
| 19 | group 二次查表：`xde_group[gid][reg]` 再跑一次 `apply_attr_flags` | `:893-899` |
| 20 | 硬编码特例（移位计数 / `C6 C7 8F` / `F6` / `F7`） | `:901-933` |
| 21 | `parse_modrm` + `apply_modrm_usage` | `:936-939` |
| 22 | MOFFS 路径（非 ModR/M 的 `A0-A3` 等） | `:940-953` |
| 23 | `apply_usage_special` / `apply_implicit_gp` | `:955-956` |
| 24 | `XA_UNDEF` → `src_set = dst_set = XSET_UNDEF`、`src_set2 = dst_set2 = XSET2_ALL`（**赋值，非 OR**） | `:958-963` |
| 25 | `imm_bytes` 定长 → 拷贝到 `data_b` + `C_DATA*` | `:965-978` |
| 26 | 收尾：`len = cur.p - opcode`；`0` 或 `>15` → 0；置 `len`、`C_REL`；返回 | `:980-988` |
...
1. 首两字节构成的 16 位小端字等于 `0x0000` 或 `0xFFFF`（`:611-615`）；
2. 同一类遗留前缀**重复出现**（`66`/`67`/段/`F2F3`/`F0`，`:627`/`:638`/`:646`/`:653`/`:660`）；
3. 表属性 `XA_BAD`（映射见 `:123`）；
4. `C6`/`C7`/`8F` 在 legacy map 下 `reg != 0`（`:909-911`）。

**前缀语义**：`66` 翻转 `defdata` 2↔4（`:625`）；`67` 在 64 位翻转 `defaddr` 8↔4、其余模式 2↔4（`:632-636`）；段前缀存 `p_seg`、`F2/F3` 存 `p_rep`、`F0` 存 `p_lock`。**每类只保留最后见到的字节**。注意前缀循环在 REX 判定**之前**跑完且 REX 只判一次，所以 `48 66 90` 会把 `48` 当 REX、再把 `66` 当 opcode——非规范前缀顺序不被拒绝。
...
| 字节 | 两种解释 | 判定门槛 | 行 |
|------|----------|----------|-----|
| `62` | EVEX / BOUND | `peek(1..3)` 全成功 **且** `(b2 & 0x04)` **且**（`mode == 64` 或 `(b1 & 0xC0) == 0xC0`）→ 否则 BOUND | `:717-720` |
| `C4` `C5` | VEX2/VEX3 / LES/LDS | `mode == 64` 或 `(b1 & 0xC0) == 0xC0`（`C4` 还需第三字节）→ 否则 LES/LDS | `:768-771`, `:788-789` |
| `8F` | XOP / POP r/m | `(b1 & 0x1F) >= 8` → 否则 `8F /0` = POP r/m。**没有 mod 测试**（见 Known Gaps） | `:828-831` |
| `D5` | REX2 (APX) / AAD | 仅 64 位且 `b == 0xD5` → 否则按 AAD 走 legacy | `:684-715` |

门槛失败即落到 `parse_legacy_opcode`（`:862`）。这就是 README 那句「`C4`/`C5`/`62`/`8F` 只有在后随字节符合前缀形式时才是 VEX/EVEX/XOP」的实现。
...
`XA_*` 到 `C_*` 的映射在 `apply_attr_flags`（`src/xde.c:116-137`）：17 条一对一 OR，**只增不减**（读-改-写 `diza->flag`）。`XA_VVVV_GPR` 与 group 位不经此函数（`XA_VVVV_GPR` 在 `:273`/`:288`/`:325`/`:496` 直接读，`XA_GRP_ID` 只在 `:894` 读）。

立即数尺寸表（`imm_bytes`，`src/xde.c:94-114`）：
...
尺寸→标志：1 → `C_DATA1`，2 → `C_DATA2`，**3 → `C_DATA1 \| C_DATA2`**，4 → `C_DATA4`，8 → `C_DATA8`，**6 → `C_DATA4 \| C_DATA2`**（`:972-977`）。

### 对象集推导（三趟）

1. `apply_modrm_usage`（`:250-363`）——ModR/M 的 reg/r/m 字段。`mod == 3` 走寄存器分支，否则内存分支。`rex` 启发式在 `:253`：`rex != 0 || enc != LEGACY`（即 VEX/EVEX/XOP/REX2 一律当作「有 REX」，影响 8 位寄存器命名）。reg 字段带扩展位组成 `regx = rex_r4<<4 | rex_r<<3 | reg`（`:260-261`），r/m 侧同理用 `rex_b4`（`:318-319`）。
2. `apply_usage_special`（`:139-248`）——隐式操作数：REP/串操作（`A4-A7`/`AA-AF`/`6C-6F`/`AC-AD`）、IO（`E4-E7`/`EC-EF`）、`SAHF/LAHF`、`CBW/CWD`、`AAA/AAS`、`AAM/AAD`、`PUSHA/POPA`、`PUSH/POP sreg`、`XLAT`、`ENTER/LEAVE`、`MOV` 段寄存器，以及 0F map 的 `CPUID`/`SHLD/SHRD`/`LSS` 等。**其 `attr` 形参未使用**（`(void)attr;` `:247`）。
3. `apply_implicit_gp`（`:365-453`）——opcode 隐含的 GPR：`INC/DEC r`、`PUSH/POP r`、`XCHG r8,eAX`、`MOV r,Iv`、`ALU AL/eAX, Iv`、`BSWAP`，外加 `XA_PUSH`/`XA_POP` 的栈列（`stack_set`：16 → `XSET_SP`，32 → `XSET_ESP`，64 → `XSET_RSP`）。注意其 `rex` 判定只在 `:368` 用 `rex != 0`（与 `:253` 不同口径）。

**REX2 的 r/m 是 GPR，不是向量寄存器**：`parse_modrm` 的 SIB 分支（`:495-496`）与 `apply_modrm_usage` 的内存分支（`:360`）在判断「是否按向量寄存器记 `XSET_OTHER`」时都会排除 `XDE_ENC_REX2`；`mod == 3` 分支同样排除（`:324-325`）。另外 `parse_modrm` 的两个「无基址」判定——SIB 的 `mod == 0 && base == 5`（`:493`）与 `mod == 0 && rm == 5`（`:512`）——在 `rex_b4` 置位时不再成立，此时它指的是真寄存器 r21，不是 disp32。

`gp_set`（`:43-83`）的寄存器列映射：`reg > 31 → XSET_OTHER`（**解码路径不可达**，`reg` 由 5 位扩展位拼出，上限 31）；`reg >= 16` 交给其第 4 个形参 `uint64_t *egpr`——EGPR 写进**第二对象集字**（`*egpr |= XSET2_R16 << (reg - 16)`，`:68-72`），固定寄存器处传 `NULL`；`reg >= 8 → XSET_R8 << (reg-8)`；`sz <= 1` 时按 `rex` 选 `lo8_norex`（AL/CL/DL/BL/**AH/CH/DH/BH**）或 `lo8_rex`（AL/CL/DL/BL/**SPL/BPL/SIL/DIL**——独立的 `XSET_SPL/BPL/SIL/DIL` 位，不再复用 16 位那几位）；`sz` 2/4/8 → `w16`/`w32`/`w64`，**其他任何值静默落到 `w64`**。

### 编码方向（`xde_asm`）

**不做校验、不解码、无输出容量参数**。固定顺序拼接（`src/xde.c:1001-1036`）：

```
p_seg → p_lock → p_rep → p_67
  ├─ 若 nvex：vex[0..nvex-1] → opcode          （此路径不发 p_66 / rex）
  └─ 否则：p_66 → rex → opcode
...
| 路径 | 用途 | 关键符号 |
|------|------|----------|
| `include/xde.h` | 唯一公共头（292 行）：宏词汇表 + `struct xde_instr`（`:204-266`） + 7 个 API（`:272-282`） | `XDE_MODE_*`, `XDE_ENC_*`, `XDE_MAP_*`, `C_*`, `XSET_*`, `XSET2_*` |
| `src/xde.c` | 解码器 + 编码器（1036 行），全部核心逻辑 | `xde_disasm_buf:582`, `xde_asm:1001`, 12 个 static 助手 |
| `src/xdetbl.c` | **机器生成**的属性表（414 行） | `xde_attr:5`, `xde_group:382` |
| `src/xdetbl.h` | 手写的表层契约（86 行） | `XA_*`, `enum xde_group_id`, `XDE_MAP_COUNT 11` |
| `src/xde_text.c` | 调试打印（131 行，3 个函数） | `xde_sprintfl:7`, `xde_sprintset:26`, `xde_sprintset2:107` |
| `tools/gen_tables.py` | 表生成器（617 行），唯一写出 `src/xdetbl.c` 的地方 | `OUT:7-8`, `MAPS:530`, emit 块 `:592-617` |
| `tests/xde_test.c` | 唯一测试文件（470 行） | 5 个 `expect_*` 助手, `main:134` |
...
工具集 `PlatformToolset` **v145**（四个配置都是）；被提示时重定位工具集（`README.md:45`）。产物 `build\<Platform>\<Configuration>\xde_test.exe`，中间文件 `build\obj\<Platform>\<Configuration>\`。工程**无任何 PreBuild/PostBuild/CustomBuild/Exec 步骤**。
...
退出码 `0` = 全通过，`1` = 有失败。`main` 是 `int main(void)`，**没有任何 CLI 参数**（无 filter / verbose / 单用例选择），每次运行都执行全部 80 项检查。
...
- **语言**：纯 C11（`<stdint.h>`）。头文件带 `extern "C"` 守卫（`include/xde.h:11-12`、`:288-290`），可被 C++ 包含。为 nameless union 用 `#pragma warning(push/disable: 4201)` 包住（`:16-19`、`:284-286`）。
...
- **注释风格**：行内 `//`，多用于标注特例原因，例：`// 32-bit GP writes zero-extend in 64-bit mode.`（`:263`）、`// Vector encodings use OTHER for the reg field.`（`:267`）、`// segment override`（`:350`）。**全树没有任何 `TODO`/`FIXME`/`XXX`/`HACK`/`NOTE` 标记**（`src/` 已核）。
- **表纪律**：列宽固定，`xde_attr` 每行 8 个 `0x%08X`，每组带 `// legacy` / `// 0F` 之类行尾注释。
- **对象集的打印策略**（`xde_sprintset`）：按列从宽到窄取第一个命中——含有 `RAX` 就绝不打印 `EAX`/`AX`；16 位列之后还有 8 位扩展寄存器兜底（`SP`→`SPL`、`BP`→`BPL`、`SI`→`SIL`、`DI`→`DIL`，`:69-85`）；`XSET_UNDEF`（精确相等）特判为 `"???"`。第二对象集字由 `xde_sprintset2` 打印（`:107-131`）：`set2 == XSET2_ALL`（精确相等）特判为 `"???"`，否则逐位输出 `R16`…`R31`。
...
| 语义 | `flag` `src_set` `dst_set` `src_set2` `dst_set2` | 64 位；`flag`/`src_set`/`dst_set` 低 32 位兼容 1.02；第二字 `src_set2`/`dst_set2`（`:218-219`）只装 APX EGPR `r16-r31`（`XSET2_*`） |
...
| 寻址 | `modrm` `sib` | 原始字节 |
| 位展开 | `rex_w` `rex_r` `rex_x` `rex_b` `rex_r4` `rex_x4` `rex_b4` `vex_pp` `vex_l` `vex_vvvv` `evex_z` `evex_b` `evex_aaa` `evex_r2` | 已解码的位；`rex_r4`/`rex_x4`/`rex_b4`（`:237`）是 REX2 的 EGPR 扩展位（ModR/M.reg / SIB.index / r/m 的 bit 4），非 REX2 编码恒为 0；`vex_vvvv` 是**非取反**后的值（0-31） |
...
`flag` 低位（兼容 1.02，`include/xde.h:53-93`）：`C_ADDR1/2/4`、`C_MODRM`、`C_SIB`、`C_ADDR67`、`C_DATA66`、`C_UNDEF`、`C_DATA1/2/4`、`C_BAD`、`C_REL`、`C_STOP`、`C_OPSZ8`、`C_SRC_FL`/`C_DST_FL`、`C_SRC_REG`/`C_SRC_RM`/`C_DST_REG`/`C_DST_RM`、`C_SRC_ACC`/`C_DST_ACC`、`C_SRC_R0`/`C_DST_R0`、`C_PUSH`/`C_POP`，以及 `C_CMD_*`（`CALL`/`JMP`/`JCC`/`RET`，占 `0x08000000`-`0x80000000`，用 `XDE_CMD(fl)` 提取）。复合宏 `C_MOD_FL`/`C_MOD_REG`/`C_MOD_RM`/`C_MOD_ACC`/`C_MOD_R0`。
...
第二对象集字（`src_set2` / `dst_set2`，`include/xde.h:181-201`）：`XSET2_R16`…`XSET2_R31`（每寄存器一位，不区分宽度，同 `XSET_R8`…`XSET_R15` 的口径）与 `XSET2_ALL`（低 16 位全 1）。首字剩余位不足以再放 16 个寄存器，所以 EGPR 单独占一个字。

## Important Files

| 文件 | 说明 |
|------|------|
| `include/xde.h:272-282` | 7 个导出函数声明 + 语义注释 |
| `include/xde.h:204-266` | `struct xde_instr` |
| `include/xde.h:181-201` | `XSET2_*` 词汇（第二对象集字：APX `r16-r31`） |
| `src/xde.c:582-989` | `xde_disasm_buf` —— 改解码行为从这里读起 |
| `src/xde.c:1001-1036` | `xde_asm` —— 字节重组，不校验 |
| `src/xde.c:717-720` / `:768-771` / `:828-831` | 三处编码类歧义门槛 |
| `src/xde.c:116-137` | `XA_*` → `C_*` 映射表（新增属性位必改） |
| `src/xde.c:139` / `:250` / `:365` | 三趟对象集推导 |
...
| `tests/xde_test.c:134` | 测试 `main`，全部用例内联于此 |
...
**自研极简 harness，无第三方框架，无 `ASSERT`/`CHECK`/`TEST` 宏**。全部逻辑在 `tests/xde_test.c`（470 行）的单个 `int main(void)`（`:134-469`）中：65 个匿名 `{ }` 块，每块一个 `static const uint8_t` 向量紧跟 `expect_*` 调用。**零文件 IO、零 fixture、零 golden 文件**。
...
| `expect_roundtrip(name, mode, b, n)` | `:80-107` | disasm → `xde_asm` → 再 disasm，比较 `len` + `opcode` + `modrm` |
| `expect_set(name, mode, b, n, sel, bit, want)` | `:111-132` | 要求解码长度 == `n`，再断言单个对象集位的存在/不存在。`sel` 0/1/2/3 = `src_set`/`dst_set`/`src_set2`/`dst_set2`，`want` 1 = 置位、0 = 未置位；成功打 `set<n> 0x… set|clear` |

### 覆盖矩阵（80 项检查 / 79 个用例名）

| 区段注释 | 行 | 检查数 | 内容 |
|----------|-----|--------|------|
| `// 64-bit GP` | `:136` | 26 | nop、xor、`mov rax,imm64`、`mov r8,imm64`、RIP 相对、`call rel32`、`66 call rel32`、ret、push、`sub rsp,0x20`、SIB、`mov r8,[rsp+0x28]`、`nop dword [rax+rax]`、endbr64、syscall、movsxd、`moffs64` ×2、`test rax,imm32`、bt、`67` 前缀 |
| `// SSE / 0F38 / 0F3A` | `:249` | 6 | movups、palignr、pshufb、crc32、movbe、3DNow `pavgusb` |
| `// VEX` | `:275` | 6 | VEX2 `vaddps`（含往返）、VEX3 `andn` ×2、`rorx`、`vzeroupper` |
| `// EVEX` | `:298` | 4 | `vaddps zmm`（含往返）、EVEX 内存形式、`vrndscaless` |
| `// XOP` | `:317` | 4 | `vfrczpd`（含往返）、`vpcomb`、`bextr` |
| `// 32-bit mode: LES vs VEX, BOUND vs EVEX` | `:332` | 9 | LES/LDS、32 位 VEX2、BOUND、`inc eax`、truncated REX、`rex nop`、aaa 非法/合法、`push es` 非法 |
| `// REX2 (APX)` | `:364` | 2 | `rex2 lea`、`rex2 imul` |
| `// Object sets: 8-bit extension registers vs high bytes, and APX EGPRs` | `:374-433` | 16 | 13 项 `expect_set` + 3 项内联打印器检查：`mov spl,al`（`XSET_SPL` 置位且**不含** `XSET_SP`）、`mov ah,al`（`XSET_AH` 置位且**不含** `XSET_SPL`，钉住无 REX 时的差异）、`rex2 lea r16d,[rax]`（`dst_set2` 含 `XSET2_R16`、`dst_set` 不含 `XSET_OTHER`）、`rex2 lea r31d,[rax]`（`XSET2_R31`）、`rex2 push r16`（`src_set2` 含 `XSET2_R16`，钉住 opcode+r 的 B4）、`rex2 mov eax,[r16]`（SIB base 的 B4 → `src_set2` 含 R16，同时 `src_set` 含 `XSET_RAX`）、`rex2 add rax,r16`（`mod == 3` 下 reg 是 EGPR、r/m 是 legacy GPR，`:403-409`）；以及 `xde_sprintset2`/`xde_sprintset` 的输出串断言（`R16` / `SPL` / 解码后再打印 `R16`，`:410-433`） |
| （无区段注释） | `:435-451` | 4 | `pop rax (8F /0)`、`test al,0x12`、`not al`、`call rax` |
| `// 16-bit` | `:452` | 2 | `add ax,ax`、`mov ax,[moffs16]` |
| `// truncated` | `:462` | 1 | 截断的 `mov rax,imm64` |
...
1. **对象集覆盖仍然很浅**：现在有 13 项 `expect_set` 断言（见上表 Object sets 区段），但只走了少数几条编码路径（`mov spl,al`/`mov ah,al` 两个 8 位案例与 6 个 REX2/EGPR 案例）；`xde_sprintset`/`xde_sprintset2` 也仅被 3 项内联检查触碰。对象集是本库的一半价值，断言密度仍远低于长度/编码类。
2. **flag 只有 1 条断言**：`C_RIPREL`（`:170-172`，且它不在 `expect_*` 里，是唯一的内联 `fail`）。其余 `C_*` 无覆盖。
3. **往返是弱 oracle**：只比 `len`/`opcode`/`modrm`，**不比 `out[]` 与 `b[]` 的字节**；长度相同的错误重编码会通过。只有 4 个用例用它。
4. `expect_enc` 忽略 `n` 形参 → 该助手下从不测试截断行为。
5. 重复用例名：`"mov rax,[rip+0]"`（`:166` 与 `:171`，前者是 `expect_len`、后者是内联 `fail`/`printf`）与 `"rex2 lea r16d,[rax]"`（`:367` 的 `expect_enc` 与 `:387` 的 `expect_set`）——按名字 grep 输出会有歧义；缓冲区也被跨用例复用（`inc[]`、`aaa[]`）。
6. `:435-451` 四例缺区段注释，位置夹在 Object sets 块与 `// 16-bit` 之间，容易误归类。
7. `XDE_ENC_LEGACY` 从未被 `expect_enc` 断言。
...
5. 若新属性需要影响 `flag` 或对象集，还要改 `src/xde.c` 的 `apply_attr_flags`（`:116-137`）或三趟对象集推导（`:139` / `:250` / `:365`）；
6. 加测试用例并 `build.bat`。
...
1. `include/xde.h` 声明（放在 `:272-282` 区块内，保持 `extern "C"` 覆盖）；
...
| `build.bat` 成功但 `.sln` 打开报工具集 | 工程钉 `v145`，本地工具集不匹配 → 重定位（`README.md:45` 已预告）。 |
...
- **REX2 不清 `p_66`/`p_rep`**（对比 VEX `:784-785`/`:807-808`、EVEX `:747-748`、XOP `:851-852` 都清）。叠加 `xde_asm` 在 `nvex` 路径不发 `p_66`（`:1015-1018`），导致 `66 D5 …` 重新编码后**丢掉 `66`**，不字节级往返。
- **XOP 门槛缺 mod 测试**：`(b1 & 0x1F) >= 8`（`:831`）没有像 EVEX（`:720`）/VEX（`:771`）那样检查 16/32 位下的 `mod == 11b`，所以 `8F 08 …`（非法 POP 形式）会被归为 XOP 而非 POP。
- **XOP 不写 `opcode2`/`opcode3`**（`:856-857` 只写 `opcode` 和 `map`），而 VEX/EVEX 会写。
- **`mod == 3` 目标白名单漏了 `C6`/`C7`**：内存白名单含 `C6/C7`（`:358`），寄存器白名单不含（`:334-338`）→ `C6 /0 ib`、`C7 /0 id` 用寄存器操作数时 `dst_set` 拿不到目标寄存器。这是一处不对称，像是疏漏。
- **`MOV` 存储形式把目的也计入 `src_set` / `src_set2`**：`mod == 3` 分支无条件执行 `if (c != 0x8D) { src_set |= rset; src_set2 |= rset2; }`（`:329-332`），但 `0x88`/`0x89`（`MOV Eb,Gb` / `Ev,Gv`）的 r/m **只是目的**。实测：`40 88 C4`（`mov spl,al`）得 `src=AL|SPL`、`dst=SPL`（`src` 多算 SPL）；`D5 10 89 C0`（`mov r16d,eax`）得 `src2=R16` **且** `dst2=R16`（R16 多算进源）。对照装载形式 `D5 10 8B C0`（`mov eax,r16d`）→ `src2=R16`、`dst=RAX` 是正确的。ALU / 移位 / `F6` / `F7` 的 r/m 确实既读又写，所以只有 `MOV` 存储形式被过度近似；**属既有行为，不是本次 APX 改动引入**。
- **`C_ADDR8` 在 `parse_modrm` 不可达**：该处 `disp` 只会是 1/2/4（`:538-541`），`else` 分支永不触发；8 字节地址只经 MOFFS 路径（`:948`）。
- **`R8B-R15B` 仍报成宽度无关的位**：`gp_set` 的 `reg >= 8` 判定在尺寸判定之前（`:73-74`），所以 8 位扩展寄存器 `R8B-R15B` 与 `XSET_R8..XSET_R15` 共用一位，**没有按宽度区分的 8 位扩展寄存器集**。（注：`SPL/BPL/SIL/DIL` 已由 `lo8_rex` 用独立位区分，不再是这个问题的受害者。）
- **`gp_set` 对未知 `sz` 静默回落 64 位**（`:82`），`sz == 0` 或 3 不会报错。
- **`apply_modrm_usage` 与 `apply_implicit_gp` 的 `rex` 口径不一致**（`:253` 含非 legacy 编码，`:368` 只看 `rex != 0`）→ 对带 ModR/M 的 8 位形式的 VEX/EVEX/XOP 指令，两趟可能给出不同寄存器名（REX2 现在会置 `rex`，两趟已一致）。
- **`XA_GROUP` 强制 `XA_MODRM` 发生在 `apply_attr_flags` 之后**（`:879` vs `:882-885`），所以那个 `C_MODRM` 位要等 `parse_modrm`（`:465`）才置上。
- **`C_REL` 重复置位**：`apply_attr_flags`（`:120`）与收尾（`:986`）各一次，后者冗余。
- **3DNow 注释仅为描述**（`:980`）：尾随 opcode 实际靠表把它表达成 `XA_IMM_IB`，解码器并不特别识别 `0F 0F` 后缀。
- **`xde_asm` 无输出容量参数**（对比 `xde_disasm_buf` 的 `max_len`）：结构体的 `addrsize`/`datasize`/`nvex` 被写坏时无人保护；且它按 `p_67` → `p_66`/`rex` 的顺序输出（`:1013` vs `:1020-1021`），不还原规范前缀顺序（解码侧不限制顺序，所以能往返的输入未必是规范序）。
...
- **`xde_sprintset` 对 `XSET_UNDEF` 用精确相等判定**（`src/xde_text.c:30`）：若某集合只是"包含"undef 位，会走正常渲染而非 `"???"`。目前解码侧是整体赋值（`:958-963`），所以两者恰好一致——仅靠构造保证。
...
| 文档 | 内容 |
|------|------|
| `README.md`（92 行） | 唯一面向使用者的文档：`## What it does`（`:11`）、`## Layout`（`:22`）、`## Build (MSVC)`（`:35`）、`## API`（`:53`，示例 `:55-63`）、`## Notes`（`:78`，编码规则）。**无许可证段落、无外部链接。** |
