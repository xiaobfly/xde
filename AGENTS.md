...
| 函数 | 语义 |
|------|------|
| `int xde_disasm(const uint8_t *opcode, struct xde_instr *diza)` | 64 位模式解码（`src/xde.c:1671`） |
| `int xde_disasm_ex(const uint8_t *opcode, struct xde_instr *diza, unsigned mode)` | 指定 16/32/64（`src/xde.c:1666`） |
| `int xde_disasm_buf(const uint8_t *opcode, unsigned max_len, struct xde_instr *diza, unsigned mode)` | 额外限制读取上限（`src/xde.c:1050`） |
| `int xde_asm(uint8_t *opcode, const struct xde_instr *diza)` | 结构 → 字节；`return xde_asm_buf(opcode, XDE_MAXLEN, diza);` 的包装（`src/xde.c:1756`） |
| `int xde_asm_buf(uint8_t *opcode, unsigned max_len, const struct xde_instr *diza)` | 结构 → 字节，额外限制写入上限；装不下返回 0（`src/xde.c:1705`） |
...
**返回值契约**：解码返回指令长度，`0` = 失败（截断 / 该模式下非法 / undefined）。编码返回写入字节数，`xde_asm_buf` 在结构体所需字节数 `> max_len` 时返回 `0`（`xde_asm` 给的是 `XDE_MAXLEN`，所以除空指针外永远成功）。没有错误码枚举、没有 errno、没有 out-param 状态。**注意 `max_len == 0` 不是「零字节上限」而**是「用默认 `XDE_MAXLEN`(15)」（`src/xde.c:1152-1155`，`xde_asm_buf` 同构 `:1937-1938`）——传 0 不会失败，而是按 15 解码 / 编码；`max_len == 16` 也被夹到 15（同一处 `> XDE_MAXLEN` 判定，两边同构），所以「传 16 就多写一字节」的期待不成立；要限长必须传显式上界（批 B v2 的截断扫描因此在 `len == 1`（即 `len-1 == 0`）的 **8,437,552** 例上单列跳过）。
...
```
consumer
  └─ xde_disasm / xde_disasm_ex            (src/xde.c:1671 / :1666, 只差默认参数)
       └─ xde_disasm_buf(ptr, max_len, d, mode)   ← 唯一真正的入口 (src/xde.c:1050-1664)
            ├─ xde_attr[map][opcode]      (查表, :1355)  ← src/xdetbl.c 生成
            ├─ xde_group[gid][modrm.reg]  (二次查表, :1405)
            └─ parse_modrm (`:924`) → apply_modrm_usage (:430)
                                  → apply_usage_special (:146)
                                  → apply_implicit_gp (`:760`)
  └─ xde_asm(out, d) → xde_asm_buf(out, XDE_MAXLEN, d)   (src/xde.c:1981 / `:1930-1979`, 纯字节重组)
```
...
| # | 阶段 | 行 |
|---|------|-----|
| 1 | 入参守卫：空指针 → 0；`mode ∉ {16,32,64}` → 0；`max_len` 0/超限一律夹到 15 | `:1148-1155` |
| 2 | `memset` 清零 + 预置 `mode` / `defaddr` / `defdata` | `:1157-1160` |
| 3 | 游标初始化 `beg`/`p`/`end` | `:1162-1164` |
| 4 | `C_BAD` 启发式 | `:1166-1170` |
| 5 | 遗留前缀循环 | `:1172-1226` |
| 6 | 取下一个字节 | `:1229-1230` |
| 7 | REX（`40-4F`，仅 64 位） | `:1232-1245` |
| 8 | REX2（`D5`，仅 64 位） | `:1247-1278` |
| 9 | EVEX（`62`） | `:1280-1329` |
| 10 | VEX（`C4`/`C5`） | `:1331-1389` |
| 11 | XOP（`8F`） | `:1391-1424` |
| 12 | 遗留 opcode + `enc = XDE_ENC_LEGACY` | `:1426-1429` |
| 13 | 汇合点标签 `got_opcode:` | `:1431` |
| 14 | map 越界检查 + `attr = xde_attr[map][mop]` | `:1439-1441` |
| 15 | 三种拒绝：`XA_INVALID` → 0；`XA_I64 && mode==64` → 0；`XA_O64 && mode!=64` → 0 | `:1443-1448` |
| 16 | `apply_attr_flags`：`XA_*` → `C_*` 映射（**`C_REL` 的唯一来源**，收尾不再重复置位） | `:1450`（实现 `:123-144`） |
| 17 | `XA_GROUP` 强制 `XA_MODRM`，并就地补置 `C_MODRM`（`apply_attr_flags` 已经跑过） | `:1453-1458` |
| 18 | 编码类归因 `legacy_enc`（`LEGACY` 或 `REX2`），后面所有遗留形式判据的前置门 | `:1460-1464` |
| 19 | 强制前缀 / 白名单检查：`legacy_enc` 且 `map ∈ {0F, 0F38, 0F3A}` 时按 map 查 `xde_pfx_0f`/`xde_pfx_38`/`xde_pfx_3a`，当前前缀位不在表项里 → `C_BAD`（表项 0 的 `0F 7A`/`0F 7B`、以及只有 VEX/EVEX 形式的转义 opcode 在任何遗留前缀下都置位；见「`C_BAD` 的来源」第 20、21 条）；跑在 `XA_MODRM` 组**之前**——它不需要 r/m 字节，而 VEX/EVEX/XOP 的前缀在头里、从不到达这里 | `:1479-1488` |
| 20 | peek ModR/M，取 `reg = (mpeek >> 3) & 7` | `:1493-1495` |
| 21 | group 二次查表：`xde_group[gid][reg]` 再跑一次 `apply_attr_flags` | `:1496-1503` |
| 22 | 硬编码特例：移位组 `C0 C1 D0-D3` 写 FL（`RCL`/`RCR` 另读 FL、`D2`/`D3` 另读 CL）／`C6 C7 8F` 的 `C_BAD`（`F8` 例外 = `XBEGIN`/`XABORT`）／**`C_BAD` 普查的六个判据（`0F 00 /6 /7`、`0F 01` 内存 `/5` 与 mod=3 保留子槽、`FF /3`/`/5` mod=3、`62` mod=3、x87 `D8-DF`，见「`C_BAD` 的来源」第 8-13 条）**／**mod 结构性判据（`8D`、`0F 13`/`0F 17`/`0F 2B`、`0F 50` 与三组顺带 opcode，见第 14-19 条）**／**本轮操作数形式谓词群（组移位、VIA PadLock、PEXTRW/EXTRQ、MPX、group 9、Key Locker、HRESET 与 `0F AE` 的四条前缀/`rm` 规则，见第 23 条）**／`F6`/`F7` 的标志与 `MUL`/`DIV` 累加器规则 | `:1504-1809` |
| 23 | `parse_modrm` + `apply_modrm_usage` | `:1811-1822` |
| 24 | MOFFS 路径（非 ModR/M 的 `A0-A3` 等） | `:1823-1836` |
| 25 | `apply_usage_special` / `apply_implicit_gp` | `:1838-1839` |
| 26 | `XA_UNDEF` → `src_set = dst_set = XSET_UNDEF`、`src_set2 = dst_set2 = XSET2_ALL`（**赋值，非 OR**）；随后 `undef_sys_operands` 按 ModR/M 回填 `0F 00`/`0F 01`/`0F 02`/`0F 03` 已建模形式的真实集合并清 `C_UNDEF`，未回填者保持整集 + `C_UNDEF` | `:1841-1852`（实现 `:339-428`，`ea_set` 暂存 `:1817-1820`，调用 `:1850`） |
| 27 | `imm_bytes` 定长 → 拷贝到 `data_b` + `C_DATA*` | `:1854-1867` |
| 28 | **3DNow 选择子检查**：立刻数拷贝**之后**判 `0F 0F /r ib` 的尾随字节是不是 AMD 分配的选择子（`now_ok[4]` 位掩码，选择子即 `data_b[0]`、守卫 `datasize == 1`）→ 否则 `C_BAD`；放在这里而不是 ModR/M 组里，因为这个字节是立即数、前面刚被拷进 `data_b`（见「`C_BAD` 的来源」第 22 条） | `:1869-1878` |
| 29 | 收尾：`len = cur.p - opcode`；`0` 或 `>15` → 0；置 `len`；返回 | `:1882-1888` |

**3DNow 没有特例分支**：表把尾随 opcode 字节建模成 `XA_IMM_IB`，`XA_3DNOW` 只负责置 `C_3DNOW`（收尾注释 `:1880-1881`，现在是准确描述而非待办）；**选择子本身的合法性**由流水线第 28 步的 `now_ok[4]` 判据负责（`:1873-1878`），不在 `XA_3DNOW` 里。

**移位组与组 3 都写标志**：legacy map 的 `C0`/`C1`/`D0-D3`（八个操作）一律 `dst_set |= XSET_FL`；`reg == 2 || 3`（`RCL`/`RCR`）另加 `src_set |= XSET_FL`（读 CF），`D2`/`D3` 另加 `src_set |= XSET_CL`。`F6`/`F7` 在 `reg != 2` 时 `dst_set |= XSET_FL`（`NOT`(/2) 不写标志），`/4`-`/7` 的累加器规则不变（`:1504-1809`）。这与 ALU 路径一致（`:628`/`:632`/`:784`/`:818`/`:824`）：凡写标志都要在 `dst_set` 里出现 `XSET_FL`。
...
**`C_BAD` 的来源**（共 23 类；第 1-7 类是原有判据，第 8-13 类是 `C_BAD` 普查新增的六个 `0F`/`FF`/`62`/x87 判据，第 14-19 类是 mod 结构性判据——三个显式项加三组顺带 opcode，第 20 类是 `0F` map 的强制前缀判据，第 21 类是 `0F 38`/`0F 3A` 两个转义 map 的 legacy 白名单（P1），第 22 类是 3DNow 选择子（P4），第 23 类是操作数形式谓词群（P5）；除第 22 类在立即数拷贝**之后**外，全部落在 `xde_disasm_buf` 的 `got_opcode` 段）：

1. 首两字节构成的 16 位小端字等于 `0x0000` 或 `0xFFFF`（`:1166-1170`）；
2. 同一类遗留前缀**重复出现**（`66`/`67`/段/`F2F3`/`F0`，`:1187`/`:1200`/`:1208`/`:1215`/`:1222`）；
3. 表属性 `XA_BAD`（映射见 `:130`；语义 = 「**任何模式下都不可用的编码**」，模式受限的形式改由 `XA_I64`/`XA_O64`/`XA_F64`/`XA_D64` 表达，见 Known Gaps）；
4. `C6`/`C7`/`8F` 在 legacy map 下 `reg != 0` **且该 ModR/M 字节不等于 `0xF8`**（`:1519-1521`）——`C7 F8`/`C6 F8` 是合法的 `XBEGIN`/`XABORT`，其余 `/7` 编码（`C7 FA`/`C6 FA`/`C7 38`）才算坏。判据必须**字节精确**：写成 `reg != 0 && reg != 7` 会把所有非法 `/7` 一并豁免。
5. `0F AE` mod=3 的非法子形式：无 `F3` 的 `/0-/3` 与 `/4`（表里记的是内存形式，mod=3 时这两档没有定义；（`:1526-1529`）；
6. `0F 18 /0-/3` 与 `0F 0D`（全部 `/reg`，`/2-/7` 在 SDM 里同样空白）的 mod=3 **保留形式**（SDM 只定义 `m8`，`mod=11b` 不成立；（`:1559-1563`）；
7. **REX 紧邻 VEX/EVEX/XOP/REX2 引导字节**（SDM 视作 `#UD`；判据 `:1437-1438`，`rex_seen` 在 `:1241` 置位）——编码侧因 `nvex != 0` 永不发 REX，故这类输入不可能字节级往返（见 Known Gaps「编码器契约与「已判 `C_BAD` 不保证往返」边界」条）。
8. `0F 00` 组的 `/6`/`/7`（**全 mod** 共 64 槽；`:1569-1571`）——SDM 给该组只定义 `/0-/5`（SLDT/STR、LLDT/LTR、VERR/VERW），寄存器形式与内存形式都没有 `/6`/`/7`；REX2 也走这条（`legacy_enc` 在 `:1464`，覆盖写进 `opcode2` 的形式）。
9. `0F 01` **内存形式**的 `/5`（24 槽；`:1586-1587`）——内存表是 SGDT/SIDT、LGDT/LIDT、SMSW、LMSW、INVLPG，`/5` 空着；mod=3 的 `/5`（`E8`-`EF`）另有自己的表，不受影响。
10. `0F 01` **mod=3 的保留子槽**（12 槽；`:1588-1592`）——`C6 C7 CC CD CE D2 D3 E9 EA EB EC ED`；同一张表里 `C0-C5`（ENCLV…PCONFIG）、`C8-CB`/`CF`（MONITOR/MWAIT/CLAC/STAC/ENCLS）、`D0/D1/D4-D7`（XGETBV/XSETBV/VMFUNC/XEND/XTEST/ENCLU）、`D8-DF`（SVM）、`E0-E7`（SMSW）、`EE`/`EF`（RDPKRU/WRPKRU）、`F0-F7`（LMSW）、`F8-FF`（SWAPGS/RDTSCP/MONITORX…）都是真指令，**不得误标**。
11. `FF /3`/`/5` 的 **mod=3**（16 槽；`:1599-1601`）——far `CALL`/far `JMP` 只定义内存形式的 `m16:16/m16:32/m16:64`，mod=3 无编码；`/2`/`/4`（near `CALL`/`JMP r/m`）与 `/3`/`/5` 的内存形式保持干净，`/7` 本来就由表侧 `XA_BAD` 覆盖。
12. `62`（BOUND）的 **mod=3**（64 槽；`:1606-1608`）——BOUND 的两个操作数都从内存读；判据只看 `XDE_ENC_LEGACY`，所以 `62` 在 32 位被当 EVEX 引导时不受影响（`XDE_ENC_EVEX` 不置 `C_BAD`），64 位下 BOUND 早已被 `XA_I64` 拒。
13. **x87 逃逸表（`D8-DF`）的空白槽**（251 槽；`:1773-1781`）——两张掩码表：`x87_resm[8]`（`:872-874`，内存形式的 `/reg` 位掩码，SDM 空白的是 `D9 /1`、`DB /4`、`DB /6`、`DD /5`，共 96 槽）与 `x87_res3[8]`（`:866-871`，mod=3 的 `bit(reg*8+rm)` 掩码，155 槽），合计 96 + 155 = 251。**有意排除 12 槽**：`DB E0/E1/E4/E5`（8087/287 的 FENI/FDISI/FSETPM/FRSTPM）与 `DF C0-C7`（FFREEP）在 SDM 里同样空白，但 binutils 仍给它们助记符，标 `C_BAD` 会是假阳性 ⇒ 表里留 0，并有 `not bad` 对照断言钉住（见 Known Gaps「应标 `C_BAD` 却未标：431 槽」条的两条告诫）。

14. **`LEA`（`8D /r`）的 mod=3**（每模式 64 槽；`:1613-1615`）——SDM 把第二操作数写成内存操作数（算地址），mod=3 不是它的任何编码。判据只看 `map == XDE_MAP_LEGACY`，而 REX2 的 `8D` 也落在 legacy map（`legacy_enc` 在 `:1464` 覆盖 REX2），所以两个编码类一并命中；`lea [eax]` 的三种模式不误标。
15. **`0F 13`/`0F 17`/`0F 2B` 的 mod=3**（`:1622-1625`）——MOVLPS/MOVHPS 只定义存储进内存的 m64（`66` 的 MOVLPD/MOVHPD 与 REX2 形式同 r/m），MOVNTPS 只写 m128；**`0F 12`/`0F 16` 的载入形式不在此列**：它们的 mod=3 是 MOVHLPS/MOVLHPS，合法。
16. **`0F 50`（MOVMSKPS/MOVMSKPD）的 mod≠3**（`:1629-1631`）——r/m 必须取 xmm 寄存器，其它 mod 不是它的编码；`66` 形式同判据。
17. **第二组「r/m 只定义在内存」的 0F opcode 的 mod=3**（`:1646-1654`，7 条规则）——`66 0F 12`/`66 0F 16`（MOVLPD/MOVHPD，门 `p_66 && !p_rep`）、`0F B2`/`B4`/`B5`（LSS/LFS/LGS，**P5 起不再带 `!p_rep` 门**：这三个 opcode 没有强制前缀，`F2`/`F3` 不改变它们的 memory-only 形式）、`0F C3`（MOVNTI）、`0F E7`（MOVNTQ，`66` 形式为 MOVNTDQ，门 `!p_rep`）、`F2 0F F0`（LDDQU，门 `p_rep == 0xF2`）；门槛的作用是**保住合法形式**——`MOVHLPS`/`MOVLHPS`/`MOVSLDUP`/`MOVSHDUP`/`MOVDDUP` 都有寄存器 r/m，不得误标。
18. **第二组「r/m 只定义在寄存器」的 0F opcode 的 mod≠3**（`:1660-1664`，3 条规则）——`0F D7`/`66 0F D7`（PMOVMSKB，**P5 起 `F2`/`F3` 也保留同一寄存器形式，门已去掉**）、`0F F7`/`66 0F F7`（MASKMOVQ/MASKMOVDQU，门 `!p_rep`）、`F2 0F D6` 与 `F3 0F D6`（MOVDQ2Q / MOVQ2DQ，门 `p_rep ∈ {F2, F3}`）；**裸 `0F D6` 不是 MOVQ 的形式**——SDM 把 mm 存储写成 `0F 7F`、把 xmm 存储写成 `66 0F D6`，所以裸字节串不命名任何指令（现由第 20 条的前缀判据置位，`66 0F D6` 仍合法），而 `MOVQ r/m` 的寄存器 r/m 形式只在 `66`/`F3` 下存在，**不得误标**。
19. **`0F 38` 的 memory-only 组（mod=3）**（`:1670-1681`，10 条规则）——`66 0F 38 2A`（MOVNTDQA）、`66 0F 38 80/81/82`（INVEPT/INVVPID/INVPCID）、`0F 38 F0`/`F1`（MOVBE，门 `!p_rep`，从而 `F2` 读作 CRC32 时保持合法）、`66 0F 38 F5`（WRUSSD/WRUSSQ）、`0F 38 F6`（WRSSD/WRSSQ，门 `!p_66 && !p_rep`）、`66 0F 38 F8`（MOVDIR64B）与 `F2 0F 38 F8`（ENQCMD/ENQCMDS）、`0F 38 F9`（MOVDIRI）。

20. **`0F` map 的强制前缀合法性（P2，提交 `5aa9840`）**（`:1479-1488`；表 `xde_pfx_0f[256]` 在 `:900-933`，`enum { XDE_PFX_NONE = 0x01, XDE_PFX_66 = 0x02, XDE_PFX_F2 = 0x04, XDE_PFX_F3 = 0x08 }` 在 `:894-899`）——SSE 族把强制前缀当成 opcode 的一部分，所以同一个转义 opcode 在别的前缀下不命名任何指令：`66 0F 6C` 是 PUNPCKLQDQ、`F3 0F B8` 是 POPCNT，而裸 `0F 6C`/`F2 0F 6C`/`F3 0F 6C` 与裸 `0F B8`/`66 0F B8`/`F2 0F B8` 都不是指令；`0F 09` 在 `66`/`F2` 下无定义（`F3 0F 09` 是另一条 WBNOINVD）。表里每位一个前缀类、**0 表示该 opcode 在任何前缀下都没有 legacy 形式**（只有 `0F 7A`/`0F 7B`），判据是 `(esc & 实际前缀位) == 0`。VEX/EVEX/XOP 的头里自带前缀字节，从不到达这条判据。

21. **`0F 38` / `0F 3A` 的 legacy 白名单（P1，提交 `a69e0b9`）**（同一条 `:1479-1488` 判据的另外两个分支；表 `xde_pfx_38[256]` 在 `:941-974`、`xde_pfx_3a[256]` 在 `:975-1008`，注释 `:935-940`）——**`xde_attr` 的 `0F38`/`0F3A` 两行同时服务 legacy 与 VEX/EVEX 两条路径**（`parse_legacy_opcode` 把 `0F 38`/`0F 3A` 转成 `map = XDE_MAP_0F38`/`XDE_MAP_0F3A`（`:1100-1134`，调用 `:1426-1427`），VEX3 的 `b1 & 0x1F` 给出同样的 map 值）⇒ 那些**只有 VEX 形式**的 opcode（`0F 38 0C` = VPERMILPS、`0F 3A 08` = 无 `66` 的 ROUNDPS 等）在单字节属性表里本就有表项，legacy 路径照表放行、于是被当成合法指令；P1 新增的这两张白名单表正是为了拒掉它们。这两张白名单表里 **0 = 该 opcode 只有 VEX/EVEX 形式**（legacy 下不存在），非 0 = 它确实有 legacy 形式时允许的强制前缀集合；判据与第 20 条同一形状，`map ∈ {0F38, 0F3A}` 时索引的是 `opcode3`。**表本身不动**（它与 VEX 共用）：两个 map 的 MMX/`66`/`F2`/`F3` 形式（`0F 38 00` PSHUFB、`66 0F 38 00`、`F2 0F 38 F0` CRC32、`0F 38 C8` SHA、`0F 3A 0F` PALIGNR 等）保持合法；VEX/EVEX/XOP 路径自带前缀字节，不经过该判据。P1 用 **554 条逐 opcode 断言**（`tests/xde_test.c:1465-2034`）把白名单钉死，错一位即失败。

22. **3DNow 选择子（P4，提交 `99eb1f3`）**（`:1873-1878`；表 `now_ok[4]` 在 `:880-883`，注释 `:876-879`）——`0F 0F /r ib` 的尾随字节**就是 opcode**，AMD 只分配了 **24 个**值：`0C 0D 1C 1D 8A 8E 90 94 96 97 9A 9E A0 A4 A6 A7 AA AE B0 B4 B6 B7 BB BF`（`BF` = PAVGUSB，仓库既有用例 `0F 0F C1 BF`），其余 **232 个**选择子不命名任何指令。判据按位取 `now_ok[sel >> 6] & (1ULL << (sel & 63))`，`sel` = 尾随字节（`XA_IMM_IB` 已把它拷进 `data_b[0]`，守卫 `datasize == 1`），由此它**只能放在立即数拷贝之后**、不能并入 ModR/M 组。**与前缀无关**：`66`/`F2`/`F3` 都保留同一张选择子表（binutils 与 LLVM 一致），所以判据不做前缀门。

23. **操作数形式谓词群（P5，提交 `d7a5f03`）**——14 条子判据，回答「这个 opcode 存在，但这个 ModR/M 字节落在 SDM 没定义的那一类里」（寄存器形式给了内存操作数、内存形式给了寄存器，或 `/reg` 没有对应操作）：组移位 `0F 71`/`72`（`:1687-1691`：外 `if` 在前、`/reg ∈ {2,4,6}`）/ `0F 73`（`:1692-1697`：`/reg ∈ {2,6}`，`66` 另加 `/3`、`/7` 的 PSRLDQ/PSLLDQ）；VIA PadLock `0F A6`/`A7`（`:1701-1706`）；PEXTRW `0F C5` 与 AMD EXTRQ/INSERTQ `66`/`F2 0F 78`/`79`（`:1710-1714`）；`0F 18`/`0F 0D` 的 PREFETCH 形式（`:1559-1563`，第 6 条同一条，P5 起 `0F 0D` 的 **全部** `/reg` 都按 memory-only 判）；LSS/LFS/LGS 的 `F2`/`F3` 门去掉（`:1646-1654`，见第 17 条）与 `0F D7`/`0F D6` 的寄存器形式（`:1660-1664`，见第 18 条）；`0F AE` 的四条（`:1526-1529` 的 `/0-/4` 无 `F3`、`:1535-1538` 的 fence byte-exact、`:1539-1542` 的 `66`/`F2` `/5`、`:1547-1552` 的内存形式前缀限制）；group 9 `0F C7`（`:1741-1749`）；MPX `0F 1A`/`0F 1B`（`:1722-1735`，`mib` 操作数排除无基址形式）；Key Locker（`:1755-1763`：`F3 0F 38 D8` 的 `/4-/7`、`DD-DF`/`F8` 的 memory-only、`FA`/`FB` 的 register-only）；HRESET（`:1766-1769`：`F3 0F 3A F0 /0` 且 rm=0）。
    **两处有意取舍**：① MFENCE/SFENCE（`0F AE /6`/`/7`）、MONTMUL/XCRYPT（`0F A6`/`A7`）与 HRESET 都是 **byte-exact（rm 必须为 0）**，判据按此写；而 **LFENCE 的 rm≠0** 时 binutils 仍命名、LLVM 判非法 ⇒ **有意不标**；② `0F 18 /4-/7` 的 mod=3（NOP）同样**有意不标**，理由见 Known Gaps「P2 轮之后的有意残余」条。

第 14-16 条是三个显式判据，第 17-19 条是普查候选（44 条）里经 objdump 逐条复核（**合法**形式须被 objdump 命名助记符、**非法**形式须报 `(bad)`）后保留下来的顺带项，共 **21 条 (map, opcode[, 前缀]) 规则**；三组各自带一个 `not bad` 对照块，把守护的合法形式钉住（第 17-19 条的门就是为此存在）。第 21-23 条是 P1/P4/P5 三轮新增的三类，其中第 21 条复用第 20 条的判据形状、第 22 条独立成一条、第 23 条是 14 条子判据的集合。

**相邻发现（表级，本轮有意未做）**：`F2 0F 50`/`F3 0F 50` 在 SDM 里**根本没有定义**（objdump 对**所有** mod 都报 `(bad)`）⇒ 属 `src/xdetbl.c` 的 `XA_INVALID` 缺口，**不是** mod 规则问题；第 16 条的谓词**有意不设前缀门**，所以这两个前缀形式的 mod≠3 子集按相邻 `0F 18`/`0F 0D` 先例一并被标记。

**前缀语义**：`66` 翻转 `defdata` 2↔4（`:1184-1185`）；`67` 在 64 位翻转 `defaddr` 8↔4、其余模式 2↔4（`:1193-1198`）；段前缀存 `p_seg`、`F2/F3` 存 `p_rep`、`F0` 存 `p_lock`。**每类只保留最后见到的字节**，**尺寸效应与之一致：同组重复前缀只在最后一次生效**（`if (!twice)`，`:1184`/`:1193`）——否则 `p_66`/`p_67` 已置而 `defdata`/`defaddr` 退回默认值，struct 自相矛盾，`xde_asm` 写出的字节无法解码回自身长度；重复本身仍标 `C_BAD`。段前缀本来就是「后者替换前者」的口径（`2E 26 06` → `26 06`）。注意前缀循环在 REX 判定**之前**跑完且 REX 只判一次，所以 `48 66 90` 会把 `48` 当 REX、再把 `66` 当 opcode——解码侧不拒绝非规范前缀顺序，但**重编码会按 SDM 组序规范化**（见 Known Gaps）。
...
| 字节 | 两种解释 | 判定门槛 | 行 |
|------|----------|----------|-----|
| `62` | EVEX / BOUND | `peek(1..3)` 全成功 **且** `(b2 & 0x04)` **且**（`mode == 64` 或 `(b1 & 0xC0) == 0xC0`）→ 否则 BOUND（走 legacy 时 mod=3 一律 `C_BAD`，见「`C_BAD` 的来源」第 12 条；EVEX 分派不受影响） | `:1283` |
| `C4` `C5` | VEX2/VEX3 / LES/LDS | `mode == 64` 或 `(b1 & 0xC0) == 0xC0`（`C4` 还需第三字节）→ 否则 LES/LDS | `:1334` |
| `8F` | XOP / POP r/m | `(b1 & 0x1F) >= 8` **且**（`mode == 64` 或 `(b1 & 0xC0) == 0xC0`）→ 否则 `8F /0` = POP r/m | `:1395` |
| `D5` | REX2 (APX) / AAD | 仅 64 位且 `b == 0xD5` → 否则按 AAD 走 legacy | `:1247-1278` |
...
门槛失败即落到 `parse_legacy_opcode`（定义 `:1100-1134`，调用 `:1426-1427`）。这就是 README 那句「`C4`/`C5`/`62`/`8F` 只有在后随字节符合前缀形式时才是 VEX/EVEX/XOP」的实现。
...
| XOP | `XDE_ENC_XOP` | 3 | `8F, b1, b2` | `b1 & 0x1F`（→ map 8-10） | `C_XOP`；清 `p_66`/`p_rep`（`:1415-1416`）；只写 `opcode`/`map`（`:1420-1421`） |

**`opcode2`/`opcode3` 的写入规则**：只有 `map ∈ {XDE_MAP_0F, XDE_MAP_0F38, XDE_MAP_0F3A}` 时才写（VEX `:1376-1386`、EVEX `:1315-1325`）；EVEX 的 map 4-7、VEX 的 map 7（`VEX7`）、XOP 的 map 8/9/A 都不写。XOP 与 VEX/EVEX 同一规则，不是缺口——`opcode`/`opcode2`/`opcode3` 只描述 `0F`/`0F 38`/`0F 3A` 这条链。

**`opcode2`/`opcode3` 的语义（按 map 决定谁是真 opcode）**：`map == XDE_MAP_0F` 时 `opcode2` **就是**真 opcode（`0F xx` 里的 `xx`）；`map == XDE_MAP_0F38`/`XDE_MAP_0F3A` 时 `opcode2` 存的是**转义字节** `0x38`/`0x3A`，真 opcode 落在 `opcode3`（写入顺序见 `parse_legacy_opcode` `:1100-1134`，VEX/EVEX 路径同构）。所以判 0F38/0F3A 指令必须读 `opcode3`——读者若在 `apply_modrm_usage` 里照 `opcode2` 判断，永远只会看到 `0x38`/`0x3A`（上一轮 MOVBE 缺陷的一部分就是这么漏的）。
...
`XA_*` 到 `C_*` 的映射在 `apply_attr_flags`（`src/xde.c:123-144`）：17 条一对一 OR，**只增不减**（读-改-写 `diza->flag`）。`XA_VVVV_GPR` 与 group 位不经此函数（`XA_VVVV_GPR` 在 `:587`/`:599`/`:608`/`:646`/`:1051` 直接读，`XA_GRP_ID` 只在 `:1497` 读）。

**解码器自己的 `static const` 掩码表**（都在 `src/xde.c` 里，不进 `src/xdetbl.c`）：

| 表 | 行 | 形状 | 含义 |
|----|-----|------|------|
| `x87_res3[8]` | `:866-871` | `uint64_t` | x87 `D8-DF` 的 mod=3 保留槽，`bit(reg*8+rm)`（注释 `:850-865`） |
| `x87_resm[8]` | `:872-874` | `uint8_t` | 同组的**内存形式**保留槽，`bit(reg)` |
| `now_ok[4]` | `:880-883` | `uint64_t` 256 位 | 3DNow `0F 0F /r ib` 的合法选择子，`bit(sel & 63)`（注释 `:876-879`） |
| `xde_pfx_0f[256]` | `:900-933` | `uint8_t` | `0F xx` 的强制前缀集合，位 = `XDE_PFX_*`（注释 `:885-893`） |
| `xde_pfx_38[256]` | `:941-974` | `uint8_t` | `0F 38 xx` 的 legacy 前缀集合（注释 `:935-940`） |
| `xde_pfx_3a[256]` | `:975-1008` | `uint8_t` | `0F 3A xx` 的 legacy 前缀集合 |

前缀位定义：`enum { XDE_PFX_NONE = 0x01, XDE_PFX_66 = 0x02, XDE_PFX_F2 = 0x04, XDE_PFX_F3 = 0x08 }`（`:894-899`，`NONE` = 无 `66`/`F2`/`F3`）。

两种「0」语义**不同，别混**：

- `xde_pfx_0f[opcode] == 0` = 该 `0F` opcode 在**任何前缀下都没有 legacy 形式**（只有 `0F 7A`/`0F 7B`，以及 P2 轮补的 `0F 0C`、`0F 24`/`26`、`0F 38` 那一格等）；
- `xde_pfx_38[opcode3] == 0` / `xde_pfx_3a[opcode3] == 0` = 该转义 opcode **只有 VEX/EVEX 形式**，legacy 编码不命名任何指令（P1 的白名单就是这个意思）；非 0 才是「它有 legacy 形式，且允许这些强制前缀」。

判「这个 `0F xx` / `0F 38 xx` / `0F 3A xx` + 前缀组合 xde 会不会拒」先查这三张表（索引分别是 `opcode2`、`opcode3`、`opcode3`），别从判据反推。
...
尺寸→标志：1 → `C_DATA1`，2 → `C_DATA2`，**3 → `C_DATA1 \| C_DATA2`**，4 → `C_DATA4`，8 → `C_DATA8`，**6 → `C_DATA4 \| C_DATA2`**（`:1861-1866`）。
...
7. **`C7 F8`（XBEGIN）/ `C6 F8`（XABORT）被误标 `C_BAD`**（两者都是合法指令）：成因是 `C6`/`C7`/`8F` 的 `reg != 0` 判据，现改用**字节精确**判据 `mpeek != 0xF8`（`:1519-1521`）；`reg != 0 && reg != 7` 那种写法会把 `C7 FA`/`C6 FA`/`C7 38` 等非法 `/7` 编码一并豁免（mutation M10b：`C7 FA`/`C6 FA` 的「仍应 bad」断言失败）。mutation M10（恢复 `reg != 0`）→ 2 条失败。
...
1. **`0F 1E`（无 `F3`）是 NOP Ev**（与 `0F 1F` 同类）：mod≠3 与 mod=3 都**不访问操作数**。新增 `nop1e`（判据 `map0F && LEGACY && c2 == 0x1E && p_rep == 0`，`:524-525`），加入 `reg_not_dst`（`:555-556`）、mod=3 的空操作数分支与内存分支的 `src` 排除（`!nop1e`，`:718`）。mutation → 6 条失败（断言在 `tests/xde_test.c:3073-3083`）。
...
5. **`0F AE` mod=3 的非法子形式 → `C_BAD`**：无 `F3` 的 `/0-/3` 与 `/4` 在表里仍是内存形式的编码，mod=3 时没有任何定义，原先当普通 `OTHER|OTHER` 指令报出；`xde_disasm_buf` 新增**字节精确**判据 `opcode == 0x0F && map == XDE_MAP_0F && opcode2 == 0xAE && (mpeek >> 6) == 3 && reg <= 4 && p_rep != 0xF3`（其中的 `p_rep!=0xF3` 就是 `F3` 豁免条件；`:1526-1529`）。mutation → 5 条失败；另加 9 条「不得误伤」对照（`lfence`/`mfence`/`sfence`/四个 FS/GS base 形式/`incsspd`/`fxsave` 内存形式都不带 `C_BAD`，`tests/xde_test.c:2590-2603`）。
6. **NOP Ev 的 mod=3 形式**（`{0F,1F,C0}`/`{0F,19,C0}`/`{0F,1D,C0}`/`{0F,18,E0}`）与 mod≠3 一致清空：`nop_ea`（`:533-535`）去掉 `mod != 3`，mod=3 分支与 `fence`/`endbr`/`nop1e` 共用空操作数分支。mutation（重新插回 `mod != 3 &&`）→ 8 条失败（断言在 `tests/xde_test.c:2831-2838`）。
7. **`F3 0F AE /5`（INCSSPD/INCSSPQ）**：读 r/m GPR、写 SSP（折 `OTHER`）。新增 `incssp`（`:483-484`，`map0F && LEGACY && c2 == 0xAE && mod == 3 && reg == 5 && p_rep == 0xF3`），`fence` 判据改成 `reg >= 5 && !incssp`（`:499-500`），加入 `reg_not_dst`（`:555-556`）与 mod=3 分支（`:680-685`）；`C_BAD` 保持 0。mutation A（去掉 `!incssp`）→ 4 条、B（`dst |= OTHER|rset`）→ 2 条；断言在 `tests/xde_test.c:2682-2695`。
...
4. **`0F 18 /0-/3` 与 `0F 0D /0-/1` 的 mod=3 是保留形式**（SDM 只定义 `m8`）→ `C_BAD`（`:1559-1563`）；`/4-/7` 的 mod=3 NOP 形式与内存形式保持干净。mutation：范围收窄 → 2 条失败。
...
5. **重复 `66`/`67` 的尺寸效应与自身字节不自洽**：原实现遇同组重复前缀时置 `C_BAD` 并**取消**该前缀的尺寸效应（留默认 `datasize`/`defaddr`）**却仍填 `p_66`/`p_67`** ⇒ struct 自相矛盾（`67 67 00 06` 写出 3 字节、重解码 5）。改为**最后者生效**（SDM：同组只有最后一个前缀有效；**段前缀本就是该口径**，`2E 26 06` → `26 06`），判据形如 `if (!twice) diza->defdata = …` / `if (!twice) { … defaddr … }`（`:1184-1185` / `:1193-1198`），`C_BAD` 保留。**有意后果**：部分重复前缀输入的 `len` 会变（如 `66` 生效后 Jcc 走 rel16 而非 rel32）——实测**没有任何既有断言因此变化**（新增恰为该轮的 22 条）。
6. **REX 紧邻 VEX/EVEX/XOP/REX2 引导字节**（`40 C5 04 08` 这类）：SDM 视作非法 `#UD`，此前被**静默接受**；而编码侧因 `nvex != 0` 永不发 REX ⇒ 丢字节（`66 48 D5 …` 还会变立即数宽度）。按已定口径置 `C_BAD`（判据在 `got_opcode`：`rex_seen && enc != XDE_ENC_LEGACY`，`:1437-1438`；`rex_seen` 在 `:1241` 置位），并配 3 条「不得误伤」对照（合法 `48 31 C0`、无 REX 的 `C5 04 08`、`vaddps`）。**全空间往返扫描与残差口径**见 Known Gaps「编码器契约与「已判 `C_BAD` 不保证往返」边界」条。
...
1. **`F3 0F AE /4` = PTWRITE**（r/m 是**只读** GPR 源或**只读内存**，**无目的操作数**）：新增 `ptwrite` 谓词（`:506-507`）、加进 `reg_not_dst`（`:555-556`）——`mod == 3` 形式因此走通用 r/m 读路径（不另设分支）、reg 字段不再冒充目的；内存形式另从 `mem_store`（`:549` 的 `(reg == 4 && !ptwrite)`）与 `xsave_mask`（`:572`）**排除**（不是写，也不读 `EDX:EAX`）。编码 `{F3,0F,AE,E0}`、`{F3,48,0F,AE,E0}`、`{F3,0F,AE,20}`，断言在 `tests/xde_test.c:2606` 起的块。
2. **`F3 0F AE /6` mod=3 = UMONITOR**（WAITPKG；r/m 只读 GPR）：新增 `umonitor`（`:491-492`），与同字段同 mod 的 `umwait`（`F2`，`:493-494`）、`tpause`（`66`，`:495-497`）折成 `waitpkg`（`:498`），`fence` 判据加 `&& !waitpkg`（`:499-500`）——否则 WAITPKG 三个会被**无操作数**的 MFENCE 吞掉；`waitpkg` 与 `ptwrite` 一并进 `reg_not_dst`。断言在 `tests/xde_test.c:2631` 起的块。
3. **`F2 0F AE /6` = UMWAIT、`66 0F AE /6` = TPAUSE**：除 r/m 只读外还读 `EDX:EAX`（唤醒期限）、写 `FL`（CF 报告唤醒原因）——`:750-753`。
4. **`F3 0F AE /6` mod≠3 = CLRSSBSY**（CET；清 shadow stack token 的 busy 标志）：`clrssbsy`（`:512-513`）——m64 读且写（由 `mem_store` 的 `reg == 6` 覆盖）、`dst_set |= XSET_FL`（`:756-757`）、`xsave_mask` 用 `!clrssbsy` 排除（`:573`）。断言在 `tests/xde_test.c:2668-2669` 起的块。
...
**不得误伤（最新一轮新加对照）**：`{F3,0F,AE,F8}` 的冗余 `F3` 仍是**无操作数**的 SFENCE（`tests/xde_test.c:2562-2568`）；既有的 `{0F,AE,E8/F0/F8}`（LFENCE/MFENCE/SFENCE）、`{F3,0F,AE,C0}`（RDFSBASE）、`{F3,0F,AE,E8}`（INCSSPD）对照（`tests/xde_test.c:2590-2603`）保持不动。

**本轮（mod 结构性违规普查）——批 A 已改 `src/xde.c` + `tests/xde_test.c`（未提交），批 B 只读、未改仓库任何文件：99 条新断言（全是 `expect_flag`）+ 28 组 mutation 对照**——同样按「先写断言看到失败 → 改 → 通过 → mutation 回退即失败」走：

批 A（**未动表**，全部落在 `xde_disasm_buf` 的 `got_opcode` 段，即「`C_BAD` 的来源」第 14-19 条）：

1. **`8D`（LEA）的 mod=3 → `C_BAD`**（`:1613-1615`）：10 条断言——`8D C0` 在 16/32/64、`8D FF`、`48 8D C0` 与 REX2 的 `D5 40 8D C0` **置位**，`8D 00`/`48 8D 00` 的内存形式三种模式**不置位**；断言在 `tests/xde_test.c:2857-2925`。mutation（删该判据）→ **恰 6 条**失败。
2. **`0F 13`/`0F 17`/`0F 2B` 的 mod=3 → `C_BAD`**（`:1622-1625`）：16 条断言——三个 opcode 各三模式、`66 0F 13`、REX2 的 `D5 80 13 C0` 置位，五个内存形式不置位。mutation（删该判据）→ **该组 11 条**失败。
3. **`0F 50`（MOVMSKPS/MOVMSKPD）的 mod≠3 → `C_BAD`**（`:1629-1631`）：9 条断言——裸/`66` 的 mod=0 与 `/7` mod=0 置位，`0F 50 C0`/`66 0F 50 C0` 的寄存器形式不置位。
4. **三组顺带 opcode**（`:1646-1654`「r/m 只在内存」、`:1660-1664`「r/m 只在寄存器」、`:1670-1681` 的 `0F 38` 组）：64 条断言——`66 0F 12/16` 11、LSS/LFS/LGS 与 MOVNTI/MOVNTQ/MOVNTDQ/LDDQU 16、PMOVMSKB/MASKMOVQ/MASKMOVDQU/MOVDQ2Q 18、`0F 38` 组 19，含守护合法形式的门（MOVHLPS/MOVLHPS/MOVSLDUP/MOVSHDUP/MOVDDUP、`crc32 r/m8`/`r/m32`、`0F D6` 的裸/`66` MOVQ r/m）；断言在 `tests/xde_test.c:2926-3063`。

合计 99 条（置位 63 / 未置位 36）。**mutation 对照**：28 组逐条回退，每组只失败对应断言（上引两组为例：删 LEA 判据 → 6 条；删 `0F 13`/`0F 17`/`0F 2B` 判据 → 11 条）。

批 B（**只读普查，未改仓库任何文件**）：带「必需前缀」重跑 objdump 普查并给残余分歧分类，分类表、P1-P5 优先修清单与「不要动」清单见 Known Gaps「objdump↔xde 分歧普查（带必需前缀重跑）」条。
...
**REX2 的 r/m 是 GPR，不是向量寄存器**：`parse_modrm` 的 SIB 分支（`:964-966`）与 `apply_modrm_usage` 的内存分支（`:732-733`）在判断「是否按向量寄存器记 `XSET_OTHER`」时都会排除 `XDE_ENC_REX2`；`mod == 3` 分支同样排除（`:644-649`）。另外 `parse_modrm` 的两个「无基址」判定——SIB 的 `mod == 0 && base == 5`（`:962`）与 `mod == 0 && rm == 5`（`:981`）——在 `rex_b4` 置位时不再成立，此时它指的是真寄存器 r21，不是 disp32。
...
**不做校验、不解码**；`xde_asm_buf`（`src/xde.c:1930-1979`）只多做一次容量检查，`xde_asm`（`:1981-1984`）是 `return xde_asm_buf(opcode, XDE_MAXLEN, diza);` 的薄包装。固定顺序拼接：

```
max_len 夹取：0 或 > XDE_MAXLEN 一律按 15                        (:1937-1938)
→ 计数夹到数组容量：nvex ≤ 4 / naddr ≤ 8 / ndata ≤ 8            (:1941-1943)
→ asm_size() 算所需字节数，> max_len 则 return 0                 (:1902-1928, :1945-1946)
p_lock → p_rep → p_seg → p_66 → p_67    （SDM 组序 1→2→3→4(:1953-1957)
...
**容量安全**：`addrsize`/`datasize`/`nvex` 是 `uint8_t`，被写坏时解码侧的计数会越过 `addr_b[8]`/`data_b[8]`/`vex[4]`（真 UB）。`xde_asm_buf` 先把这三个计数夹到数组容量（`:1941-1943`），再用 `asm_size()`（`:1902-1928`）算出所需字节数，`> max_len` 就返回 `0`（`:1945-1946`）；`max_len == 0` 或 `> XDE_MAXLEN` 一律按 `XDE_MAXLEN` 处理（`:1937-1938`），与 `xde_disasm_buf` 的 `max_len` 处理同构。所以解码成功的指令（`len ≤ 15`）经 `xde_asm` 永远编得出来（不变量与 13 项同步清单见 Known Gaps「`asm_size()` 是写出序列的第二份文本实现」条）。

`p_66` 现在与其它遗留前缀一起在 `:1956` 统一发射（没有 nvex 专属分支）：这是为 REX2 准备的——REX2 是唯一**保留** `p_66`/`p_rep` 的编码类（VEX/EVEX/XOP 在解码时就清掉，`:1347`/`:1370`/`:1310`/`:1415-1416`），而它们的 `vex[]` 头自带 pp 字段，不走 `p_66`；所以 `66 D5 …` 现在能字节级往返。
...
- 返回 `0` 只有两种情形：`!opcode || !diza`（`:1935-1936`），或所需字节数 `> max_len`（`:1945-1946`）；解码成功的指令不可能编出 0 字节；
...
| `src/xde.c` | 解码器 + 编码器（1984 行），全部核心逻辑 | `xde_disasm_buf:1136`, `xde_asm_buf:1930`, `xde_asm:1981`, `asm_size:1902`, `undef_sys_operands:339`, 14 个 static 助手 + 六张 `static const` 掩码表：`x87_res3`/`x87_resm`（`:866-874`）、`now_ok`（`:880-883`）、`xde_pfx_0f`/`xde_pfx_38`/`xde_pfx_3a`（`:900-1008`） |
...
| `tests/xde_test.c` | 唯一测试文件（3679 行），1956 项检查 | 9 个 `expect_*` 助手（含 `expect_seteq:194`）, `main:237-3679` |
...
退出码 `0` = 全通过，`1` = 有失败。`main` 是 `int main(void)`，**没有任何 CLI 参数**（无 filter / verbose / 单用例选择），每次运行都执行全部 1956 项检查。
...
- **注释风格**：行内 `//`，多用于标注特例原因，例：`// 32-bit GP writes zero-extend in 64-bit mode.`（`:576`）、`// NOT (/2) writes no flags`（`:1571/1586`）、`// prefetches do read memory`（`:723`）、`// segment override`（`:719`）。**全树没有任何 `TODO`/`FIXME`/`XXX`/`HACK`/`NOTE` 标记**（`src/` 已核）。
...
| `src/xde.c:1136-1889` | `xde_disasm_buf` —— 改解码行为从这里读起 |
| `src/xde.c:1930-1979` | `xde_asm_buf` —— 字节重组 + 容量检查（`xde_asm` 是它的包装，`:1981-1984`） |
| `src/xde.c:1902-1928` | `asm_size` —— 估算编码所需字节数，与 `xde_asm_buf` 的落地循环必须同步 |
| `src/xde.c:1283` / `:1334` / `:1395` | 三处编码类歧义门槛 |
...
**自研极简 harness，无第三方框架，无 `ASSERT`/`CHECK`/`TEST` 宏**。全部逻辑在 `tests/xde_test.c`（3679 行）的单个 `int main(void)`（`:237-3679`）中：132 个匿名 `{ }` 块，每块一个 `static const uint8_t` 向量紧跟 `expect_*` 调用。**零文件 IO、零 fixture、零 golden 文件**。
...
| `expect_fail(name, mode, b, n)` | `:67-78` | 要求返回 `0`（34 次） |
...
| `expect_flag(name, mode, b, n, bit, want)` | `:217-235` | 要求解码长度 == `n`，再断言单个 flag 位。`bit` 取单个 `C_*` 常量，`want` 1 = 置位、0 = 未置位；成功打 `flag 0x… set|clear`（1343 次） |

### 覆盖矩阵（1956 项检查）

1956 = 1931 次助手调用（`expect_len` 46 + `expect_enc` 16 + `expect_fail` 34 + `expect_roundtrip` 5 + `expect_set` 416 + `expect_seteq` 65 + `expect_flag` 1343 + `expect_sizes` 2 + `expect_selflen` 4）+ 25 项其余检查（25 项内联/打印器块——`C_RIPREL`、Object sets 打印器、闸门/asm 区段、规范前缀、`sete al` 非 undef、打印器最坏情况、Coverage 区段 ud2、Jcc/LOOP 打印器、`0F C7`/`0F AE` 保留编码等，逐项见覆盖矩阵各行）。**上一轮（批 A + 批 B）新增 61 项**：批 A 的 39 项落在 `// System groups…` 区段，批 B 的 22 项落在 `// Canonical…` 区段的两个新块（重复前缀折叠 14 + REX 紧邻引导字节 8）；**最新一轮新增 44 项**（批 A：PTWRITE 12 + WAITPKG 23 + CLRSSBSY 6 + SFENCE 对照 3，全在 `// System groups…` 区段；批 B 是只读扫描、不加断言）；**UNDEF 回填那轮新增 69 条断言、删旧锚点 2 条**——62 条 `expect_seteq` + 7 条 `C_UNDEF` 的 `expect_flag`，全在 `// System groups…` 区段的新块（`:3121-3248`），删掉的是旧的 `0F 00`/`0F 01` 整集 `C_UNDEF` 锚点 ⇒ 净增 67 项；**`C_BAD` 普查 + `CPUID`/`XABORT` 那轮再新增 124 项**——119 条 `expect_flag`（`62`(BOUND) 7 + `0F 00`/`0F 01` 保留槽 41 + x87 62 + `FF /3`/`/5` 9；其中 `C_BAD` 置位 68 条、未置位 51 条）+ 3 条 `expect_seteq`（`C6 F8` 的 `dst`/`src` + `C6 C0` 的 `dst`）+ 1 条 `expect_set`（`CPUID` 的 `ECX`）+ 1 条 `expect_enc`（32 位 EVEX 对照，证明 BOUND 的 `C_BAD` 不影响 `62` 的 EVEX 分派）⇒ 该轮净增 124 项；**随后那轮（mod 结构性判据）再净增 99 条 `expect_flag`**；**P2 轮（强制前缀判据）再新增 69 条 `expect_flag`、删 2 条**——新块 67 条全在 `// SSE mandatory prefixes…` 区段（`tests/xde_test.c:1326-1463`，29 条置位 / 38 条未置位），`// System groups…` 区段另加 2 条 `0F D6` 置位断言、同时删掉 2 条把裸 `0F D6` 当作合法 MOVQ 的旧断言 ⇒ 该区段计数不变 ⇒ **1044**（`expect_flag` 375 → 442，其余助手调用数不变）；**P1/P3/P4/P5 四轮再新增 912 项**——P1 白名单 554 条逐 opcode `expect_flag`、P3 未定义 0F 槽 18 条（11 条 `expect_fail` + 7 条 `expect_flag`）、P4 3DNow 选择子 37 条、P5 操作数形式 303 条（生成块 286 + 细节块 17），全部落在 `// SSE mandatory prefixes…` 之后的五个新区段，`expect_fail` 23 → 34、`expect_flag` 442 → 1343 ⇒ **1956**。以上计数都是对 **3679** 行文件的实测值（助手自身的定义行不计入调用）。
...
| `// SSE mandatory prefixes: the prefix byte is part of the opcode, so the same escaped opcode under any other prefix names no instruction.` | `:1326-1463` | 67 | 67 条 `expect_flag`（**29 条 `C_BAD` 置位 / 38 条未置位**，一个 `static const` 向量一条断言）：每族一对「合法前缀 → 不置位」+「裸/错前缀 → 置位」——`66 0F 6C`/`6D`（PUNPCKLQDQ/HQDQ）与裸/`F2`/`F3`、`F3 0F B8`（POPCNT）与裸/`66`/`F2`、`F2 0F F0`（LDDQU）与裸/`F3`、`66`+`F2 0F 7C`/`7D`（HADD/HSUBPD·PS）与裸/`F3`、`66`+`F2 0F D0`（ADDSUBPD/PS）与裸/`F3`、`66`/`F2`/`F3 0F D6`（MOVQ 存储 / MOVDQ2Q / MOVQ2DQ）与**裸 `0F D6`**、`66`/`F2`/`F3 0F E6`（CVTTPD2DQ / CVTPD2DQ / CVTDQ2PD）与裸、`0F 09`（WBINVD）与 `F3`（WBNOINVD）的裸形式 ⇒ `66`/`F2` 置位、`0F 52`（RSQRTPS）/`F3 0F 52`（RSQRTSS）与 `66`/`F2`、`0F 5B`（CVTDQ2PS）/`66`（CVTPS2DQ）/`F3`（CVTTPS2DQ）⇒ `F2` 置位、`0F 78`/`79` 的裸（VMREAD/VMWRITE）/`66`（EXTRQ）/`F2`（INSERTQ）与 `F3`、`0F 6F`/`7E`/`7F` 的裸（MMX）/`66`（MOVDQA）/`F3`（MOVDQU·MOVQ xmm）与 `F2`（`0F 6F`）、`0F 71`/`72`/`73` 的裸与 `66`（`F2`/`F3` 置位）、`0F C3`（MOVNTI）的裸形式、`0F 58` 的裸/`66`/`F3`/`F2`（ADDPS/ADDPD/ADDSS/ADDSD 四种前缀都合法 ⇒ 4 条不置位对照）、`0F 7A`/`7B` 各两种前缀全置位；末尾 3 条是**不得误伤**对照（`C5 F8 58 C1` vaddps、`C4 E2 78 F2 C1` andn、`62 F1 7C 48 58 C1` EVEX vaddps 都不置位，钉住 VEX/EVEX 不经这条判据） |
| `// The escaped-map whitelists, one assertion per opcode.`（P1，`a69e0b9`） | `:1465-2034` | 554 | 554 条 `expect_flag`，**逐 opcode 一条**（`0F 38`/`0F 3A` 各 256 个 opcode 里的白名单项与排除项），每条断 `C_BAD` 置位/未置位——`ok` 用 objdump 命名的形式、`bad` 用 objdump 判 `(bad)` 的形式，断言由普查数据生成而不是手写；错一位白名单即失败 |
| `// The 0F slots the SDM leaves blank under every prefix`（P3，`d519779`） | `:2036-2079` | 18 | 11 条 `expect_fail` + 7 条 `expect_flag`：`0F 7A`/`0F 7B` 的裸/`66`/`F2`/`F3` 形式、`0F 77`（EMMS）**不得**被标、冗余前缀族（`F2 0F 13` 等）由 mod 规则而非前缀规则处置的对照 |
| `// 3DNow! names its opcode in the trailing immediate byte`（P4，`99eb1f3`） | `:2081-2162` | 37 | 37 条 `expect_flag`：24 个合法选择子各一条（`C_BAD` 未置位，含 `0F 0F C1 BF` PAVGUSB）+ 13 条非法选择子（置位）+ 前缀无关对照（`66`/`F2`/`F3` 保留同一形式） |
| `// The operand forms the SDM gives one class only, checked across both classes`（P5 生成块，`d7a5f03`） | `:2164-2458` | 286 | 286 条 `expect_flag`，同样由普查数据生成：每个「只定义寄存器」/「只定义内存」的 opcode 各一条合法形式 + 每条相邻的 `/reg`、mod 取值 |
| `// The /reg fields and rm values the class sweep above cannot reach`（P5 细节块） | `:2460-2502` | 17 | 17 条 `expect_flag`：`66` 组移位 `/3`/`/7` 的 PSRLDQ/PSLLDQ、fence 的 byte-exact（MFENCE/SFENCE 的 rm≠0 置位、**LFENCE 的 rm≠0 不置位**）、`66`/`F2 0F AE /5` 置位而 `/6` 保留等类扫描覆盖不到的子句 |
| `// System groups: CR/DR moves, RDRAND/RDSEED, fences, FS/GS base, save/restore, prefetch.` | `:2504-3470` | 488 | 181 项 `expect_set` + 242 项 `expect_flag` + 65 项 `expect_seteq`（**本轮新增 69 项断言、删 2 条**：`0F 00`/`0F 01`/`0F 02`/`0F 03` 系统组的 62 条精确集合断言 + 7 条 `C_UNDEF`，见「0F 00 / 0F 01 / 0F 02 / 0F 03」条；**上一轮批 A 新增 39 项**：`RDRAND`/`RDSEED` 的 CF 3 条、`CMPXCHG8B`/`CMPXCHG16B` 的 `EDX:EAX`（`REX.W` 下 `RDX:RAX`）读写对 + ZF 4 条、`XSAVE`/`XSAVEOPT`/`XSAVEC`/`XSAVES`/`XRSTOR`/`XRSTORS` 的状态掩码 `EDX:EAX` 6 条、`0F 18 /0-/3` 与 `0F 0D /0-/1` 的 mod=3 保留编码 2 条，其余为 FXSAVE/FXRSTOR、LDMXCSR/STMXCSR、CLFLUSH/**CLWB**、VMPTRLD/VMPTRST 等**不得置位**对照；区段共 19 个匿名块（4 空格缩进的 `{` 起首行）、119 行子注释（8 空格缩进的 `//`），均为实测；**最新一轮批 A 再加 44 项**：`F3 0F AE /4` PTWRITE 12（r/m 只读 GPR 或只读内存、**无目的操作数**、`m` 形式无 `dst M`、不读 `EAX`/`EDX`、无 `C_BAD`）、`F3`/`F2`/`66 0F AE /6` 的 UMONITOR/UMWAIT/TPAUSE 23（r/m 只读 GPR；UMWAIT/TPAUSE 另读 `EDX:EAX` 并写 `FL`；三者两侧无 `OTHER`、均无 `C_BAD`）、`F3 0F AE /6` mod≠3 的 CLRSSBSY 6（`src`/`dst` 均含 `M`、`dst` 含 `FL`、不读 `EDX:EAX`、无 `C_BAD`）、`{F3,0F,AE,F8}` 的 SFENCE 对照 3（仍**无操作数**、无 `C_BAD`））：MOV CR/DR 12（`mov eax,cr0`/`mov eax,dr0`/`mov rax,cr0`：`src` 含 `OTHER` 且**无** `RAX`、`dst` 含 `RAX`；`mov cr0,eax`/`mov dr0,eax`：`src` 含 `RAX`、`dst` 含 `OTHER` 且**无** `RAX`）、`rdrand`/`rdseed`/`rdrand rax` 8（`dst` 含 GPR，两侧无 `OTHER`）、`lfence`/`mfence`/`sfence` 6 + FS/GS base 10（`rdfsbase`/`rdgsbase` 的 `dst` 含 `RAX`；`wrfsbase`/`wrgsbase` 的 `src` 含 `RAX`、`dst` 含 `OTHER`）+ 14 条 `C_BAD` 边界（5 条 `0F AE` mod=3 非法子形式 + 9 条「不得误伤」对照）、`F3 0F AE /5` INCSSPD/INCSSPQ 7（`src` 含 `RAX` 且无 `OTHER`、`dst` 含 `OTHER` 且无 `RAX`）、`0F AE`/`0F C7` 内存 20（11 条写形式 `dst` 含 `M`、4 条只读对照无 `dst M`，另有 `fxsave`/`cmpxchg8b`/`vmptrst`/`vmptrld` 的 `src` 含 `M` 与 `vmptrst dst other`）、prefetch/NOP/ENDBR 32（PREFETCHNTA/T0/T1/T2、PREFETCHW 与 PREFETCHWT1：`src` 含 `M`、无 `OTHER`；NOP Ev/`0F 19`/`0F 1D`/`0F 18 /4` 的 mod≠3 与 mod=3 两组：两侧无 `M`/`OTHER`；`endbr64`：两侧无 `OTHER`）、`0F 1E` NOP Ev / RDSSPD 11（无前缀的 `0F 1E`（mod≠3 与 mod=3）两侧无 `M`/`OTHER`；`F3 0F 1E /0` 的 RDSSPD/RDSSPQ：`src` 含 `OTHER`、`dst` 含 `RAX` 且无 `OTHER`）、`lss`/`lfs`/`lgs`/`movzx` 6（`dst` 含 `RAX`）、`C7 F8`/`C6 F8`（无 `C_BAD`）与 `C7 FA`/`C6 FA`（仍有 `C_BAD`）5 条 + `sldt`/`smsw` 的 `C_UNDEF` 2 条（**本轮已删除，换成下层那批**）+ **最新一轮批 A 的 44 条**（PTWRITE/WAITPKG/CLRSSBSY/SFENCE 对照，见上）+ **本轮的 69 条**（62 条 `expect_seteq` 精确集合断言 + 7 条 `C_UNDEF`，覆盖 `0F 00`/`0F 01`/`0F 02`/`0F 03` 的已建模形式与未回填面）+ **`C_BAD` 普查那轮的 106 条**（3 条 `expect_seteq`：`C6 F8` 的 `dst EAX` 与空 `src`、`C6 C0` 的 `dst AL`，`:3115-3119`；41 条 `C_BAD` 断言——`0F 00 /6`/`/7` 全 mod 与 REX2 形式、`0F 01` 内存 `/5` 的四种寻址、mod=3 保留子槽 `C6 C7 CC CD CE D2 D3 E9 EA EB EC ED` 12 槽，`:3249-3338`；62 条 x87——`D8-DF` 逃逸表空白槽（内存 `D9 /1`、`DB /4`、`DB /6`、`DD /5` 与 mod=3 的 27 槽）加 24 条「槽位相邻的真实形式」与 12 槽 alias 对照，`:3340-3470`；**本轮（mod 结构性判据）再加 99 条 `C_BAD` 断言**（见「`C_BAD` 的来源」第 14-19 条，共 6 个新判据块）：LEA 10 + `0F 13`/`0F 17`/`0F 2B` 16 + `0F 50` 9 + 三组顺带 opcode 64＝置位 63 / 未置位 36，分两个新块 `:2857-2925` / `:2926-3063`；**P2 轮再加 2 条 `0F D6` 置位断言、并删掉 2 条把裸 `0F D6` 当合法 MOVQ 的旧断言**（`movq mm not bad (64)`/`movq mm m64 not bad (64)`，连同随之无用的 `movq_r`/`movq_m` 两个向量），净计数不变 ⇒ 本区段仍 488） |
| `// XA_BAD means "not a usable encoding in any mode", so the legacy forms below, legal in 16/32/64-bit alike, must not carry it.` | `:3472-3659` | 99 | 66 项 `expect_flag(…, C_BAD, 0)` + 7 项 `expect_flag(…, C_I64, 1)` + 19 项 `expect_fail`（64 位拒绝）+ 7 项 `expect_flag(…, C_BAD, 1)`（`FF /7`、`0F B9` UD1 参照，加**本轮** `FF /3`/`/5` 的 mod=3 5 条），4 个匿名块（子注释 `:3472-3473`、`:3547`、`:3556-3558`、`:3633`；最后一块的 `FF /3`/`/5` 断言在 `:3648-3658`）：全合法 legacy 35（`6C-6F` INS/OUTS、`70/71/7A/7B`、`8C/8E`、`9C-9F`、`AD/AF`、`CA/CB/CC/CF`、`D7`、`E0/E1`、`E4-E7`、`EC-EF`、`F4/F5`、`FA/FB`）、`0F B2/B4/B5` 3、`XA_I64` 形式在 16/32 位的合法性 24（22 个 opcode，`push cs`/`aaa` 各两种模式）、64 位 `expect_fail` 19、仍非法 group 2 |
| `// 16-bit` | `:3661` | 2 | `add ax,ax`、`mov ax,[moffs16]` |
| `// truncated` | `:3671` | 1 | 截断的 `mov rax,imm64` |
...
本轮（分组 opcode 口径）**没有动任何表**：改动全在 `src/xde.c` 的 `apply_modrm_usage`（新增 `mov_crdr`/`rdrand`/`fence`/`fsgsbase`/`endbr`/`nop_ea`/`prefetch_ea`/`mem_store` 八个 opcode+mod+reg 判定，并让通用规则让位）与 `xde_disasm_buf` 的 `C6`/`C7`/`8F` `C_BAD` 判据（加 `mpeek != 0xF8`），所以 `tools/gen_tables.py`、`src/xdetbl.c`、`src/xdetbl.h` 三者**逐字节未变**；`src/xde.c` 里带 `XA_VVVV_GPR` 的分支与 `simd_0f` 名单也一并保持原样。改这类「reg 字段是选择器而非寄存器」的组时，**三层都要看**：表属性、`apply_modrm_usage` 的 `mod == 3` / 内存两条分支、以及 `xde_disasm_buf` 里的组特例。此后两批同样**没有动任何表**：新增 `nop1e`/`incssp` 两个判定、去掉 `nop_ea` 的 mod 条件、给 `mem_store` 的 `0F C7` 列表加上 `/7`，并在 `xde_disasm_buf` 新增 `0F AE` mod=3 的 `C_BAD` 判据（`:1526-1529`）——`tools/gen_tables.py`、`src/xdetbl.c`、`src/xdetbl.h` 仍**逐字节未变**。**上一轮（批 A 提交 `fdce0f3` + 批 B）同样零表改动**：全部落在 `src/xde.c` 的 `apply_modrm_usage` 谓词表与 `xde_disasm_buf`（前缀循环的 `if (!twice)` + `got_opcode` 的 `rex_seen` 判据），`src/xdetbl.c`/`src/xdetbl.h`/`tools/gen_tables.py` 仍逐字节未变。**最新一轮（批 A）同样零表改动**：仍只动 `src/xde.c` 的 `apply_modrm_usage` 谓词表（`umonitor`/`umwait`/`tpause`/`waitpkg`/`ptwrite`/`clrssbsy`），`src/xdetbl.c`/`src/xdetbl.h`/`tools/gen_tables.py` 继续逐字节未变。**本轮（`0F 00`/`0F 01`/`0F 02`/`0F 03` 系统组回填）同样零表改动**：新增 `src/xde.c` 的 `undef_sys_operands`（`:339-428`）、它的调用与 `C_UNDEF` 清理（`:1841-1852`）以及寻址寄存器的 `ea_set` 暂存（`:1146`、`:1817-1820`）——表上的 `XA_UNDEF` 位**原样保留**，回填只在解码侧做，所以 `src/xdetbl.c`/`src/xdetbl.h`/`tools/gen_tables.py` 仍逐字节未变。

**本轮（`C_BAD` 定口径 + 表侧清理）是第一次按「`XA_BAD` = 该编码在任何模式下都不可用」改 legacy / 0F map**：`tools/gen_tables.py` 删掉 60 个 `XA_BAD` 槽（35 个三模式全合法的 legacy 槽 + 22 个带 `XA_I64` 的 legacy 槽 + `m1` 的 `0F B2`/`B4`/`B5`），并在 `:20-21` 给 `XA_BAD` 补注释「not a usable encoding in any mode; mode-limited forms use XA_I64/XA_O64/XA_F64/XA_D64 instead」；`src/xdetbl.c` 由重跑生成器产出（24 行改动、24 插 24 删，**字节数 40140 不变** ⇒ 只翻转属性位，`src/xdetbl.c` **未手工编辑**）；`src/xdetbl.h` 未动（无新常量，`check_header()` 通过）；`src/xde.c` 未动（64 位拒绝仍走 `:1445-1446` 的 `XA_I64` 分支，与 `XA_BAD` 无关）。删后 `XA_BAD` 余 **28 槽** = `m0[0xD6]`（SALC，`XA_BAD|XA_OPSZ8`）+ `m0[0xF1]`（INT1/ICEBP，`XA_BAD|XA_UNDEF`）+ 26 个 group 槽。生成器幂等实测：连跑两次逐字节一致（md5 `3fc495d3a3ce7180179bd5e8ddd261f0`、40140 字节、退出码 0）。双向 mutation 实测：把 `XA_BAD` 加回 `m0[0xCC]`（INT3）→ 重跑生成器后**恰好 1 条**失败（`int3 not bad`）；删掉 `group[XG_5][7]` 的 `XA_BAD` → **恰好 1 条**失败（`FF /7 still bad`）；恢复后 md5 复原、`0 failure(s)`。测试侧同步加 90 条断言（见「覆盖缺口」与覆盖矩阵里 `tests/xde_test.c:3472-3659` 那一行）。
...
| `xde_disasm_buf(ptr, 0, …)` 没报错却按 15 字节解码 | `max_len == 0` 是「用默认 `XDE_MAXLEN`」而不是「零字节上限」（`src/xde.c:1152-1155`；`xde_asm_buf` 同构，`:1937-1938`）。要限长请传显式上界；批 B v2 的截断扫描因此把 `len == 1` 的 **8,437,552** 例单列跳过。 |

## Known Gaps / Cautions

以下是**代码可证**的已知缺口（作者未用 TODO 标注）：

- **REX2 保留 `p_66`/`p_rep`（刻意行为，不是缺口）**（对比 VEX `:1347`/`:1370`、EVEX `:1310`、XOP `:1415-1416` 都清）——`66` 是 REX2 合法的遗留前缀，不是多余前缀。编码侧 `p_66` 与其它遗留前缀一起在 `:1956` 统一发射（无 nvex 专属分支），所以 `66 D5 …` 现在能字节级往返；改这里要解码/编码两侧一起动。
- **`C_ADDR8` 只来自 MOFFS（原先那条不可达分支已删）**：`parse_modrm` 里 `disp` 只会是 1/2/4（`:1081-1095`，末行注释写明），原来永不触发的 `else → C_ADDR8` 已删除；8 字节地址只经 MOFFS 路径（`:1831`）。新增位移宽度前先确认这条不变量还成立。
...
- **`0F 18 /0-/3` 与 `0F 0D`（全部 `/reg`）的 mod=3 是保留形式，不是 NOP**：SDM 里 PREFETCHh/PREFETCHW/PREFETCHWT1 只定义 `m8`（Mod≠11），所以 `{0F,18,C0}`、`{0F,0D,C0}` 这类 mod=11 编码**不并入** `nop_ea`（`:533-535`）——**P5 起它们按「保留」标 `C_BAD`**（`:1559-1563`，第 6 条；`0F 0D` 的 `/2-/7` 在 SDM 里同样空白，因此一并标）。**`0F 18 /4-/7` 仍不标**（第二/第三解码器冲突，见「P2 轮之后的有意残余」条）。（`0F 1E`（无 `F3`）的 NOP Ev 此后那批已并入 `nop1e`。）
- **未定义编码只记录、不建模**：`F3 0F 1E /1..6`、`F3 0F 1E /0` 的 mod≠3、`F2 0F 1E C0`——UD/未定义编码，无可辩驳的目标语义 → 不写断言（`F3 0F AE /4` 的 mod=3 形式**最新一轮已按 PTWRITE 建模**，不再算「未定义」，见上条；`F3 0F AE /4` 因 `F3` 豁免而无 `C_BAD`；`0F 0D /2-/7` 与 `0F 18 /0-/3` 的 mod=3 已按「SDM 保留」标 `C_BAD`（`:1559-1563`），不再算「只记录」；`0F AE` mod=3 的其余非法子形式此后那批已加 `C_BAD`，见 `src/xde.c:1526-1529`）。
...
- **`asm_size()` 是写出序列的第二份文本实现：13 项同步清单 + 容量不变量零违例（上一轮批 B v2，只读扫描、未改仓库文件）**：`asm_size()`（`src/xde.c:1902-1928`）**不是**从 `xde_asm_buf` 的落地循环推导出来的，而是同一套发射规则的**独立复刻**——两者唯一的共享「接缝」是 `xde_asm_buf` 先算好**夹取后**的 `nvex`/`naddr`/`ndata` 并按值传给 `asm_size()`（`:1941-1945`），而夹取（`nvex ≤ 4` / `naddr ≤ 8` / `ndata ≤ 8`）与 `max_len` 归一（0 或 >15 → 15）**只存在于 `xde_asm_buf`**（`:1941-1943`/`:1937-1938`），`asm_size()` 无条件信任入参。必须同步的 **13 项**：① `p_lock`、② `p_rep`、③ `p_seg`、④ `p_66`、⑤ `p_67`、⑥ `nvex > 0 → nvex + 1`（该分支**不含** `rex`/`opcode2`/`opcode3`）、⑦ `else` 分支的 `rex`、⑧ `opcode`、⑨ `opcode == 0x0F → opcode2`、⑩ `opcode2 ∈ {0x38, 0x3A} → opcode3`、⑪ `flag & C_MODRM → modrm`、⑫ `flag & C_SIB → sib`、⑬ `naddr`/`ndata` 两项（唯一「传参而非自推导」的项）；另需同步的常量是夹取上限 4/8/8 与 `max_len` 归一（二者只在 `xde_asm_buf`）。
  **实测（上一轮批 B v2）**：**170,675,981** 例解码（mode16 64,665,149 / mode32 64,665,149 / mode64 41,345,683；候选 175,270,944，解码成功率 97.4%）中 `asm_size() == 实际写出字节数` **零违例**，同时 `S > 15` = 0、溢出写 = 0、`ret == 0` 却写入 = 0、`K == 0` = 0。**容量夹取**：13 个病态结构（`addrsize`/`datasize`/`nvex` = 255/200；`flag |= C_MODRM|C_SIB`）× 11 个 `max_len`（含 0、16、17、100、`0xFFFFFFFF`）＋ 2 次显式调用，共 **169 次**，canary（`0xA5`）与 `VirtualAlloc` `PAGE_NOACCESS` 保护页 + 向量化异常处理器 + `longjmp` 验证**零违例**；夹取实测生效（raw 255/255/255 → 按 8/8/4 计，`K` 最大 28）。**4 个变异全部被捕获**：m1 删 `asm_size()` 的 `C_SIB` 项 → `S != K` **987,834**；m2 写循环改用未夹取计数 → **14 例**容量违例且**全部命中保护页**；m3 删 `p_seg` 项 → **185,016**；m4 容量判定 `>` → `>=` → **240** 例「装得下却报 0」。最小复现各一：m1 mode64 `00 04 00` → `K=3 S=2`；m4 `62 00 A5 …` → `len=15 S=15 K=0`。
...
- **objdump↔xde 分歧普查（P5 之后重跑；快照 HEAD `d7a5f03`）**：普查把「必需前缀」当成 opcode 选择子的 **80 个** opcode 规则补进来（`66`/`F2`/`F3` 作选择子者，如 `66 0F 6C/6D` 族、`F2 0F F0`、`F3 0F B8`、`66`+`F2 0F 7C/7D/D0`、`F3 0F 3A F0` 等），规范化形式两边即一致，残余只剩**裸前缀/错前缀**形式被 xde 接受。分类（槽数，三列 = m64 / m32 / m16）：

| 类别 | 含义 | m64 | m32 | m16 |
|------|------|-----|-----|-----|
| `OK` | 两边一致接受 | 1,131,020 | 834,514 | 833,544 |
| `B` | objdump 判 `(bad)`、xde 干净接受 | 770,216 | 547,614 | 548,584 |
| `B2` | objdump 的裸前缀行伪影 | 1,409 | 535 | 535 |
| `C` | objdump 合法、xde `C_BAD`（**有意**） | 8,695 | 7,338 | 7,338 |
| `A` | objdump 合法、xde 拒绝（32/16 位的 SYSCALL/SYSRET、MOV TR） | 0 | 7,196 | 7,196 |
| `D` | 两边都非法 | 17,981 | 12,526 | 12,526 |
| `E` | 两边都拒绝 | 692,119 | 425,285 | 425,285 |

  `B` 的子类（m64）：操作数形式违规 **36,275**、必需前缀缺失/错 **180,992**、legacy `0F 38`/`0F 3A` **501,504**、其它未定义被接受 52,854、裸前缀伪影 1,409（子类数字是 `1d7d37b` 快照值；P2 轮在另一套枚举上重跑的前后计数见下面的 P2 条）。**3DNow**：objdump 认可 24 个选择子，xde 接受 **232** 个未定义选择子（复现 `0F 0F C0 00`）。**截断**：3,267 例中 2,890 例两边一致（**91%**），残余是 objdump 工具缺口（APX REX2、ICEBP、`0F 05`/`0F 07` 模式不敏感）。
- **优先修清单（按槽数降序；P1-P5 五类全部已完成）**：
  - **P1 ✅ 已完成（提交 `a69e0b9`）**：legacy `0F 38`/`0F 3A` 白名单掩码（起点 m64 **501,504** / m32·16 355,584，复现 `0F 38 0C C0`）——实现为 `xde_pfx_38[256]`（`src/xde.c:941-974`）与 `xde_pfx_3a[256]`（`:975-1008`），判据复用 P2 那条 `:1479-1488`；**表未动**（与 VEX/EVEX 共用）。P1 轮后 `B` **691,747 → 25,593**；断言 `tests/xde_test.c:1465-2034` 554 条；mutation 两个：翻转一个表位 → 1 条失败，禁用整条判据 → **401** 条失败。
  - **P2 ✅ 已完成（提交 `5aa9840`）**：`0F` map 的必需前缀检查——判据在 `src/xde.c:1479-1488`、表是 `xde_pfx_0f[256]`（`:900-933`），见「`C_BAD` 的来源」第 20 条。**只覆盖 `0F` map**：原清单里那 80 个 opcode 中属于 `0F 38`/`0F 3A` 的部分没进这条判据（那两张转义表与 VEX/EVEX 共用，属 **P1** 的白名单范围）。**P2 轮重跑（64 位；10 种前缀 prologue × 4 张 map × 256 opcode × 256 ModR/M + 2,560 个 3DNow = 2,624,000 个候选，去重后 2,614,006 个唯一字节串；第二解码器 objdump 2.36.1，第三解码器 capstone 5.0.7 作旁证）**：`B`（objdump 判非法、xde 干净接受）**760,174 → 691,747**（修掉 **68,427**），`C` 8,695、`A` 21,573、`OK` 1,132,140、`E` 670,546 **均不变**，`D`（两边都非法）30,872 → 99,299；残余 `B` 691,747 按 map 分：**0F38 484,799** / **0F3A 182,469** / 0F 23,471 / legacy 单字节 952（全是 REX2/APX）/ XOP 9 56 ⇒ 剩余优先项是 **P1**（legacy `0F 38`/`0F 3A` 白名单，槽数最大）与 **P5**（操作数形式）。复现：`0F 6C C0`（缺 `66`）现 `C_BAD=1`，`66 0F 6C C0`（PUNPCKLQDQ）`C_BAD=0`；
  - **P3 ✅ 已完成（提交 `d519779`）**：其它未定义编码（起点 52,854）——`0F 7A`/`0F 7B` 的裸/`66`/`F2`/`F3` 四种形式由 P2 的 `xde_pfx_0f` **表项 0** 覆盖（任何前缀下都置 `C_BAD`，复现 `0F 7A C0`）；`0F 77`（EMMS，合法指令）**不置位**；「冗余前缀族」（如 `F2 0F 13 C0`）按 SDM「与指令无关的冗余前缀被忽略」的措辞**有意不新增判据**——`F2 0F 13 C0` 之所以非法，是因为 `0F 13` 在 mod=3 下没有定义（`:1622-1625` 的 mod 规则），而不是因为那个 `F2`。断言 18 条（11 条 `expect_fail` + 7 条 `expect_flag`，`tests/xde_test.c:2036-2079`）；mutation 一个：放开 `0F 13` 的 `F2` 门 → 2 条失败。**这一类在验收里单独计数**（见下）
  - **P4 ✅ 已完成（提交 `99eb1f3`）**：3DNow 选择子掩码（起点 **232** 个未定义选择子 vs AMD 分配的 24 个，复现 `0F 0F C0 00`）——`now_ok[4]`（`src/xde.c:880-883`，照 `x87_res3` 先例的 256 位掩码），判据 `:1873-1878`；P4 轮后 `B` 25,593 → **23,273**。断言 37 条（`tests/xde_test.c:2081-2162`）；mutation 两个：清掉 `BF` 位 → 3 条失败，禁用整条判据 → 10 条失败。
  - **P5 ✅ 已完成（提交 `d7a5f03`）**：操作数形式谓词的 48 组（起点 36,275；头部 `0F 73`/`72`/`71` 12,624、`0F A6`/`A7` 12,072、`0F D7` 4,608、`0F F7`/`C5`/`50` 各 3,456、`0F 1A`/`1B` 6,588、`0F C7` 2,440、`8D`/`0F B2`/`B4`/`B5`/`0F 2B` 各 1,536、`0F 38 F0`/`F1` 2,304、`0F 38 F6`/`F9` 1,920、`0F E7`/`D6`/`78`/`79`/`13`/`17`/`0D` 各 1,152、`0F C3` 960、`0F AE` 897、`0F 18` 768、`0F 3A F0` 765）。判据共 **14 条**，逐条见「`C_BAD` 的来源」第 23 条（组移位、VIA PadLock、PEXTRW/EXTRQ、`0F 18`/`0D`、LSS/LFS/LGS 的前缀门、`0F D7`/`D6`、`0F AE` 四条、`0F C7` group 9、MPX、Key Locker、HRESET）；P5 轮后 `B` 23,273 → **4,257**。断言 303 条（生成块 `tests/xde_test.c:2164-2458` 286 + 细节块 `:2460-2502` 17）；mutation **18 个**（逐条禁用或改动 `/reg`、`rm` 子句），每个失败 **2–34** 条 ⇒ 每条 P5 规则都有断言覆盖。
- **P5 之后的残余 4,257 槽：三类，全部有最小复现与判定**：
  1. **legacy 335 槽** = `0F 18 /4-/7` 的 mod=3（**320** 槽：224 裸 + 32×3 带 `66`/`F2`/`F3`；binutils 判 `(bad)`、capstone/LLVM 解作 `nop eax`、SDM 表空白 ⇒ **有意不标**，复现 `0F 18 E0`）+ `66`/`F2`/`F3 0F 01` 的 mod=3（**15** 槽；binutils 判 `(bad)`、capstone 判合法 ⇒ **有意不标**，复现 `66 0F 01 C0`）；
  2. **无效 VEX opcode 2,066 槽**（`2E`/`66`/`67`/`F0`/`F2`/`F3` 前缀 + `C5`/`C4` 头的字节串里，xde 接受了一个**不存在**的 VEX opcode，而 objdump 与 capstone 都判非法）⇒ 属 **VEX opcode 白名单**问题，**不在 P1-P5 范围内**，记为「**新发现的未修类**」：要修需要一个同形的 VEX 白名单 + 专门的 VEX 全空间枚举（本次枚举的前缀 prologue 只覆盖 legacy，这些槽是被顺带命中的）；
  3. **REX2/APX 1,856 槽** ⇒ objdump 2.36.1 的工具缺口（不认识 REX2 编码，一律报 `(bad)`）。
  335 + 2,066 + 1,856 = **4,257** ✓
- **普查口径与断言来源（P5 轮）**：枚举 2,624,000 个候选（10 种前缀 prologue × 4 张 map × 256 opcode × 256 ModR/M + 2,560 个 3DNow），去重后 2,614,006 个唯一字节串；第二解码器 GNU objdump 2.36.1，第三解码器 capstone 5.0.7（LLVM 表）用于裁决争议形式。**P1/P3/P4/P5 的断言由普查数据生成，不是手写**：`ok` = objdump 命名的形式 → 断 `C_BAD=0`，`bad` = objdump 判 `(bad)` 的形式 → 断 `C_BAD=1`。**前后计数（m64）**：`B`（objdump 判非法 / xde 干净接受）**760,174 → 4,257**（P2 后 691,747 → P1 后 25,593 → P4 后 23,273 → P5 后 4,257）；`C`（objdump 合法 / xde 置 `C_BAD`）**8,695 不变**；`A`（objdump 合法 / xde 拒绝）**21,573 不变**；`OK` **1,132,140 不变**；`E` **670,546 不变**；`D`（两边都非法）**30,872 → 786,789**（判据修好的槽从 `B` 落进 `D`）。**3DNow**：除 objdump 认可的 24 个选择子，xde 从接受 **232** 个未定义选择子降到 **0**。
- **P2 轮之后的有意残余（第二/第三解码器互相冲突 ⇒ 不标 `C_BAD`）**：`0F 18 /4-/7` 的 mod=3（objdump 2.36.1 判 `(bad)`，capstone 5.0.7 / LLVM 解作 `nop eax`，**224 例**；P5 轮的全枚举把带 `66`/`F2`/`F3` 的形式也算进来，因此记作 **320 槽** = 224 裸 + 32×3 带前缀）与 `66`/`F2`/`F3 0F 01` 的 mod=3（objdump 判 `(bad)`，capstone 判合法，**15 例**）——SDM 对这两个形状没有定义，两个解码器各执一词，按「不制造假阳性」的口径**有意保留**（`0F 18 /4-/7` 的 mod=3 是 NOP，见上文「`0F 18 /0-/3` 与 `0F 0D /0-/1` 的 mod=3 是保留形式，不是 NOP」条）。
- **objdump 2.36.1 的工具缺口（APX / REX2）**：残余 `B` 里那 **1,856** 槽全是 REX2/APX 形式，objdump 2.36.1 不认识这些编码、一律报 `(bad)` ⇒ 这是**工具缺口**，不计入 P1-P5，也不据此给它们标 `C_BAD`。
- **不要动清单（普查发现但按现有口径不修）**：`B2` 的裸前缀行伪影；`C` 类（`0F B9` UD1、`F1` ICEBP、重复前缀、REX 紧邻引导字节——已在本文件声明为**有意**）；`A` 类（32/16 位的 SYSCALL/SYSRET、MOV TR）；`D5` REX2 与合法的 `0F 38 F6 00`、`F3 0F 3A F0 C0`；截断残余（3,267 例中 2,890 一致）属 objdump 工具缺口；`0F 18 /4-/7` mod=3 与 `66`/`F2`/`F3 0F 01` mod=3 属上面两条记的第二解码器冲突；**P5 之后新发现的「无效 VEX opcode」2,066 槽**同属「不动」类（要动需要专门的 VEX 全空间枚举 + 同形白名单，本轮没做）。
- **更早一轮普查的副产物（已收尾）**：那一轮记的「前后两次字节计数差值 **296**、`LEA 8D` 的 mod=3 是明确的下一候选」已由 mod 结构性判据处理（`8D`、`0F 13`/`17`/`2B`、`0F 50` 与三组顺带 opcode，见上文「本轮（mod 结构性违规普查）」段与「`C_BAD` 的来源」第 14-19 条）；其余候选即 P1-P5，现已全部完成（P2 `5aa9840` / P1 `a69e0b9` / P3 `d519779` / P4 `99eb1f3` / P5 `d7a5f03`），残余 4,257 槽的三类归属见上。
