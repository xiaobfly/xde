...
| 函数 | 语义 |
|------|------|
| `int xde_disasm(const uint8_t *opcode, struct xde_instr *diza)` | 64 位模式解码（`src/xde.c:1025`） |
| `int xde_disasm_ex(const uint8_t *opcode, struct xde_instr *diza, unsigned mode)` | 指定 16/32/64（`src/xde.c:1020`） |
| `int xde_disasm_buf(const uint8_t *opcode, unsigned max_len, struct xde_instr *diza, unsigned mode)` | 额外限制读取上限（`src/xde.c:608`） |
| `int xde_asm(uint8_t *opcode, const struct xde_instr *diza)` | 结构 → 字节；`return xde_asm_buf(opcode, XDE_MAXLEN, diza);` 的包装（`src/xde.c:1110`） |
| `int xde_asm_buf(uint8_t *opcode, unsigned max_len, const struct xde_instr *diza)` | 结构 → 字节，额外限制写入上限；装不下返回 0（`src/xde.c:1059`） |
...
```
consumer
  └─ xde_disasm / xde_disasm_ex            (src/xde.c:1025 / :1020, 只差默认参数)
       └─ xde_disasm_buf(ptr, max_len, d, mode)   ← 唯一真正的入口 (src/xde.c:608-1018)
            ├─ xde_attr[map][opcode]      (查表, :897)  ← src/xdetbl.c 生成
            ├─ xde_group[gid][modrm.reg]  (二次查表, :925)
            └─ parse_modrm (:482) → apply_modrm_usage (:269)
                                  → apply_usage_special (:146)
                                  → apply_implicit_gp (:392)
  └─ xde_asm(out, d) → xde_asm_buf(out, XDE_MAXLEN, d)   (src/xde.c:1110 / :1059-1108, 纯字节重组)
```
...
| # | 阶段 | 行 |
|---|------|-----|
| 1 | 入参守卫：空指针 → 0；`mode ∉ {16,32,64}` → 0；`max_len` 0/超限一律夹到 15 | `:619-626` |
| 2 | `memset` 清零 + 预置 `mode` / `defaddr` / `defdata` | `:628-631` |
| 3 | 游标初始化 `beg`/`p`/`end` | `:633-635` |
| 4 | `C_BAD` 启发式 | `:637-641` |
| 5 | 遗留前缀循环 | `:643-690` |
| 6 | 取下一个字节 | `:693-694` |
| 7 | REX（`40-4F`，仅 64 位） | `:696-708` |
| 8 | REX2（`D5`，仅 64 位） | `:710-741` |
| 9 | EVEX（`62`） | `:743-792` |
| 10 | VEX（`C4`/`C5`） | `:794-852` |
| 11 | XOP（`8F`） | `:854-887` |
| 12 | 遗留 opcode + `enc = XDE_ENC_LEGACY` | `:889-892` |
| 13 | 汇合点标签 `got_opcode:` | `:894` |
| 14 | map 越界检查 + `attr = xde_attr[map][mop]` | `:895-897` |
| 15 | 三种拒绝：`XA_INVALID` → 0；`XA_I64 && mode==64` → 0；`XA_O64 && mode!=64` → 0 | `:899-904` |
| 16 | `apply_attr_flags`：`XA_*` → `C_*` 映射（**`C_REL` 的唯一来源**，收尾不再重复置位） | `:906`（实现 `:123-144`） |
| 17 | `XA_GROUP` 强制 `XA_MODRM`，并就地补置 `C_MODRM`（`apply_attr_flags` 已经跑过） | `:909-914` |
| 18 | peek ModR/M，取 `reg = (mpeek >> 3) & 7` | `:916-921` |
| 19 | group 二次查表：`xde_group[gid][reg]` 再跑一次 `apply_attr_flags` | `:922-929` |
| 20 | 硬编码特例（移位计数 / `C6 C7 8F` / `F6` / `F7`） | `:930-963` |
| 21 | `parse_modrm` + `apply_modrm_usage` | `:965-968` |
| 22 | MOFFS 路径（非 ModR/M 的 `A0-A3` 等） | `:969-982` |
| 23 | `apply_usage_special` / `apply_implicit_gp` | `:984-985` |
| 24 | `XA_UNDEF` → `src_set = dst_set = XSET_UNDEF`、`src_set2 = dst_set2 = XSET2_ALL`（**赋值，非 OR**） | `:987-992` |
| 25 | `imm_bytes` 定长 → 拷贝到 `data_b` + `C_DATA*` | `:994-1007` |
| 26 | 收尾：`len = cur.p - opcode`；`0` 或 `>15` → 0；置 `len`；返回 | `:1011-1017` |

**3DNow 没有特例分支**：表把尾随 opcode 字节建模成 `XA_IMM_IB`，`XA_3DNOW` 只负责置 `C_3DNOW`（收尾注释 `:1009-1010`，现在是准确描述而非待办）。

**`rex` 口径统一**：`apply_modrm_usage`（`:272`）与 `apply_implicit_gp`（`:395`）都用 `(diza->rex != 0) || (diza->enc != XDE_ENC_LEGACY)`，8 位寄存器命名不会因走哪一趟而不同。`gp_set` 对 `sz ∉ {1,2,4,8}` 返回 `XSET_OTHER`（`:89`），不会冒充 64 位。
...
1. 首两字节构成的 16 位小端字等于 `0x0000` 或 `0xFFFF`（`:637-641`）；
2. 同一类遗留前缀**重复出现**（`66`/`67`/段/`F2F3`/`F0`，`:653`/`:664`/`:672`/`:679`/`:686`）；
3. 表属性 `XA_BAD`（映射见 `:130`）；
4. `C6`/`C7`/`8F` 在 legacy map 下 `reg != 0`（`:938-940`）。

**前缀语义**：`66` 翻转 `defdata` 2↔4（`:651`）；`67` 在 64 位翻转 `defaddr` 8↔4、其余模式 2↔4（`:659-662`）；段前缀存 `p_seg`、`F2/F3` 存 `p_rep`、`F0` 存 `p_lock`。**每类只保留最后见到的字节**。注意前缀循环在 REX 判定**之前**跑完且 REX 只判一次，所以 `48 66 90` 会把 `48` 当 REX、再把 `66` 当 opcode——解码侧不拒绝非规范前缀顺序，但**重编码会按 SDM 组序规范化**（见 Known Gaps）。
...
| 字节 | 两种解释 | 判定门槛 | 行 |
|------|----------|----------|-----|
| `62` | EVEX / BOUND | `peek(1..3)` 全成功 **且** `(b2 & 0x04)` **且**（`mode == 64` 或 `(b1 & 0xC0) == 0xC0`）→ 否则 BOUND | `:743-746` |
| `C4` `C5` | VEX2/VEX3 / LES/LDS | `mode == 64` 或 `(b1 & 0xC0) == 0xC0`（`C4` 还需第三字节）→ 否则 LES/LDS | `:794-797` |
| `8F` | XOP / POP r/m | `(b1 & 0x1F) >= 8` **且**（`mode == 64` 或 `(b1 & 0xC0) == 0xC0`）→ 否则 `8F /0` = POP r/m | `:858` |
| `D5` | REX2 (APX) / AAD | 仅 64 位且 `b == 0xD5` → 否则按 AAD 走 legacy | `:710-741` |
...
门槛失败即落到 `parse_legacy_opcode`（定义 `:572`，调用 `:889-891`）。这就是 README 那句「`C4`/`C5`/`62`/`8F` 只有在后随字节符合前缀形式时才是 VEX/EVEX/XOP」的实现。
...
| XOP | `XDE_ENC_XOP` | 3 | `8F, b1, b2` | `b1 & 0x1F`（→ map 8-10） | `C_XOP`；清 `p_66`/`p_rep`（`:878-879`）；只写 `opcode`/`map`（`:883-884`） |

**`opcode2`/`opcode3` 的写入规则**：只有 `map ∈ {XDE_MAP_0F, XDE_MAP_0F38, XDE_MAP_0F3A}` 时才写（VEX `:841-849`、EVEX `:780-788`）；EVEX 的 map 4-7、VEX 的 map 7（`VEX7`）、XOP 的 map 8/9/A 都不写。XOP 与 VEX/EVEX 同一规则，不是缺口——`opcode`/`opcode2`/`opcode3` 只描述 `0F`/`0F 38`/`0F 3A` 这条链。
...
`XA_*` 到 `C_*` 的映射在 `apply_attr_flags`（`src/xde.c:123-144`）：17 条一对一 OR，**只增不减**（读-改-写 `diza->flag`）。`XA_VVVV_GPR` 与 group 位不经此函数（`XA_VVVV_GPR` 在 `:294`/`:309`/`:346`/`:523` 直接读，`XA_GRP_ID` 只在 `:923` 读）。
...
尺寸→标志：1 → `C_DATA1`，2 → `C_DATA2`，**3 → `C_DATA1 \| C_DATA2`**，4 → `C_DATA4`，8 → `C_DATA8`，**6 → `C_DATA4 \| C_DATA2`**（`:1001-1006`）。

### 对象集推导（三趟）

1. `apply_modrm_usage`（`:269-390`）——ModR/M 的 reg/r/m 字段。`mod == 3` 走寄存器分支，否则内存分支。`rex` 启发式在 `:272`：`rex != 0 || enc != LEGACY`（即 VEX/EVEX/XOP/REX2 一律当作「有 REX」，影响 8 位寄存器命名）。reg 字段带扩展位组成 `regx = rex_r4<<4 | rex_r<<3 | reg`（`:279-280`），r/m 侧同理用 `rex_b4`（`:339-340`）；`int setcc = (map == 0F && c == 0x0F && c2 ∈ 90..9F)` 在 `:281-282`。`mod == 3` 的寄存器分支里，`dst` 白名单 = ALU / `MOV r/m` / 移位 / `F6`/`F7` / `FE`/`FF` / `80-83` / **`C6`/`C7`** / **`SETcc`**（`:358-364`），而 `src` 赋值排除 `0x8D` **以及 MOV 存储形式 `0x88`/`0x89`/`0xC6`/`0xC7` 与 `SETcc`**（`:352-353`）——这几类只写 r/m，不读；内存分支同样处理（`src` 排除 `:375-378`、`dst` 白名单 `:379-386`）。
2. `apply_usage_special`（`:146-267`）——隐式操作数：REP/串操作（`A4-A7`/`AA-AF`/`6C-6F`/`AC-AD`）、IO（`E4-E7`/`EC-EF`）、`SAHF/LAHF`、`CBW/CWD`、`AAA/AAS`、`AAM/AAD`、`PUSHA/POPA`、`PUSH/POP sreg`、`XLAT`、`ENTER/LEAVE`、`MOV` 段寄存器，以及 0F map 的 `CPUID`/`SHLD/SHRD`/`LSS` 等。**其 `attr` 形参未使用**（`(void)attr;` `:266`）。标志规则**按指令区分**（`:239-248`）：`A6/A7`（CMPS）、`AE/AF`（SCAS）、`FC/FD`（CLD/STD）→ `dst_set |= XSET_FL`；`p_rep` 且 `A6/A7/AE/AF` → `src_set |= XSET_FL`（REP 循环测 ZF）；`MOVS/STOS/LODS/INS/OUTS` 完全不动标志，**DF 有意不作为源**。`REP` 本身只把 `CX/ECX/RCX` 计入 `src`+`dst`（`:155-161`），不再无条件置 `XSET_FL`。0F 段的 `SETcc`（`c2 ∈ 90..9F`）读标志：`src_set |= XSET_FL`（`:261-263`）。
3. `apply_implicit_gp`（`:392-480`）——opcode 隐含的 GPR：`INC/DEC r`、`PUSH/POP r`、`XCHG r8,eAX`、`MOV r,Iv`、`ALU AL/eAX, Iv`、`BSWAP`，外加 `XA_PUSH`/`XA_POP` 的栈列（`stack_set`：16 → `XSET_SP`，32 → `XSET_ESP`，64 → `XSET_RSP`，`:471-479`）。其 `rex` 判定在 `:395`，用的是与 `:272` **相同的** `(diza->rex != 0) || (diza->enc != XDE_ENC_LEGACY)`。

**REX2 的 r/m 是 GPR，不是向量寄存器**：`parse_modrm` 的 SIB 分支（`:522-523`）与 `apply_modrm_usage` 的内存分支（`:387-388`）在判断「是否按向量寄存器记 `XSET_OTHER`」时都会排除 `XDE_ENC_REX2`；`mod == 3` 分支同样排除（`:345-349`）。另外 `parse_modrm` 的两个「无基址」判定——SIB 的 `mod == 0 && base == 5`（`:520`）与 `mod == 0 && rm == 5`（`:539`）——在 `rex_b4` 置位时不再成立，此时它指的是真寄存器 r21，不是 disp32。
...
**不做校验、不解码**；`xde_asm_buf`（`src/xde.c:1059-1108`）只多做一次容量检查，`xde_asm`（`:1110-1113`）是 `return xde_asm_buf(opcode, XDE_MAXLEN, diza);` 的薄包装。固定顺序拼接：

```
max_len 夹取：0 或 > XDE_MAXLEN 一律按 15                        (:1066-1067)
→ 计数夹到数组容量：nvex ≤ 4 / naddr ≤ 8 / ndata ≤ 8            (:1070-1072)
→ asm_size() 算所需字节数，> max_len 则 return 0                 (:1031-1057, :1074-1075)
p_lock → p_rep → p_seg → p_66 → p_67    （SDM 组序 1→2→3→4，:1082-1086）
...
**容量安全**：`addrsize`/`datasize`/`nvex` 是 `uint8_t`，被写坏时解码侧的计数会越过 `addr_b[8]`/`data_b[8]`/`vex[4]`（真 UB）。`xde_asm_buf` 先把这三个计数夹到数组容量（`:1070-1072`），再用 `asm_size()`（`:1031-1057`）算出所需字节数，`> max_len` 就返回 `0`（`:1074-1075`）；`max_len == 0` 或 `> XDE_MAXLEN` 一律按 `XDE_MAXLEN` 处理（`:1066-1067`），与 `xde_disasm_buf` 的 `max_len` 处理同构。所以解码成功的指令（`len ≤ 15`）经 `xde_asm` 永远编得出来。

`p_66` 现在与其它遗留前缀一起在 `:1085` 统一发射（没有 nvex 专属分支）：这是为 REX2 准备的——REX2 是唯一**保留** `p_66`/`p_rep` 的编码类（VEX/EVEX/XOP 在解码时就清掉，`:810-811`/`:833-834`/`:773-774`/`:878-879`），而它们的 `vex[]` 头自带 pp 字段，不走 `p_66`；所以 `66 D5 …` 现在能字节级往返。
...
- 返回 `0` 只有两种情形：`!opcode || !diza`（`:1064-1065`），或所需字节数 `> max_len`（`:1074-1075`）；解码成功的指令不可能编出 0 字节。
...
| `src/xde.c` | 解码器 + 编码器（1113 行），全部核心逻辑 | `xde_disasm_buf:608`, `xde_asm_buf:1059`, `xde_asm:1110`, 13 个 static 助手 |
...
| `tools/gen_tables.py` | 表生成器（617 行），唯一写出 `src/xdetbl.c` 的地方 | `OUT:8`, `MAPS:530`, emit 块 `:592-617` |
| `tests/xde_test.c` | 唯一测试文件（688 行） | 6 个 `expect_*` 助手, `main:161` |
...
退出码 `0` = 全通过，`1` = 有失败。`main` 是 `int main(void)`，**没有任何 CLI 参数**（无 filter / verbose / 单用例选择），每次运行都执行全部 124 项检查。
...
只写 `src/xdetbl.c`（`OUT` 定义在 `tools/gen_tables.py:8`），**不生成 `src/xdetbl.h`**——头文件是手写的，改常量要两边同步。脚本无参数、无 `--check`、无外部输入文件（全部表数据以 Python 字面量内联在 `:74-528`），仅依赖标准库（`import os`），需要 Python 3.7+（用了 `from __future__ import annotations` 与 f-string）。以 `newline="\n"` 写文件（`:615-616`），保持这一点以免重新生成时行尾抖动。
...
| `src/xde.c:608-1018` | `xde_disasm_buf` —— 改解码行为从这里读起 |
| `src/xde.c:1059-1108` | `xde_asm_buf` —— 字节重组 + 容量检查（`xde_asm` 是它的包装，`:1110-1113`） |
| `src/xde.c:1031-1057` | `asm_size` —— 估算编码所需字节数，与 `xde_asm_buf` 的落地循环必须同步 |
| `src/xde.c:743-746` / `:794-797` / `:858` | 三处编码类歧义门槛 |
| `src/xde.c:123-144` | `XA_*` → `C_*` 映射表（新增属性位必改） |
| `src/xde.c:146` / `:269` / `:392` | 三趟对象集推导 |
...
| `tools/gen_tables.py:8` / `:74-528` / `:592-617` | 输出路径 / 全部表数据 / emit 块 |
...
**自研极简 harness，无第三方框架，无 `ASSERT`/`CHECK`/`TEST` 宏**。全部逻辑在 `tests/xde_test.c`（688 行）的单个 `int main(void)`（`:161-688`）中：81 个匿名 `{ }` 块，每块一个 `static const uint8_t` 向量紧跟 `expect_*` 调用。**零文件 IO、零 fixture、零 golden 文件**。
...
| `expect_set(name, mode, b, n, sel, bit, want)` | `:117-138` | 要求解码长度 == `n`，再断言单个对象集位的存在/不存在。`sel` 0/1/2/3 = `src_set`/`dst_set`/`src_set2`/`dst_set2`，`want` 1 = 置位、0 = 未置位；成功打 `set<n> 0x… set|clear`（40 次） |
| `expect_flag(name, mode, b, n, bit, want)` | `:141-159` | 要求解码长度 == `n`，再断言单个 flag 位。`bit` 取单个 `C_*` 常量，`want` 1 = 置位、0 = 未置位；成功打 `flag 0x… set|clear`（3 次） |

### 覆盖矩阵（124 项检查 / 108 个用例名）

124 = 108 次助手调用（`expect_len` 42 + `expect_enc` 14 + `expect_fail` 4 + `expect_roundtrip` 5 + `expect_set` 40 + `expect_flag` 3）+ 16 项内联检查（`C_RIPREL` 1 + 打印器 4 + 闸门区段 8 + 规范前缀 2 + `sete al` 非 undef 1）。
...
| `// Canonical prefix order, and the flag intents recorded in xde102/todo.` | `:592-651` | 16 | 13 项 `expect_set` + 3 项内联检查：规范前缀顺序 2 项（`:592-613`，`67 66 90` → `66 67 90`、`64 F3 A4` → `F3 64 A4`）；REP/CMPS 标志 7 项（`:614-626`：`rep movsb` src/dst 均无 FL 且有 RCX；`rep cmpsb` src+dst 有 FL；`cmpsb` dst 有 FL、src 无 FL）；`cld`/`std` dst 有 FL 各 1 项（`:627-632`）；`SETcc` 4 项（`:633-651`：`sete al` dst 有 AL、src 有 FL、src 无 AL；`sete r8b` dst2 有 `R8B`）+ 1 项内联「`sete al` 的 `xde_sprintset(src_set)` 不是 `???`」 |
| （无区段注释） | `:653-668` | 4 | `pop rax (8F /0)`、`test al,0x12`、`not al`、`call rax` |
| `// 16-bit` | `:670` | 2 | `add ax,ax`、`mov ax,[moffs16]` |
| `// truncated` | `:680` | 1 | 截断的 `mov rax,imm64` |
...
1. **对象集覆盖仍然很浅**：现在有 40 项 `expect_set` 断言，但分散在少数几条编码路径上——Object sets 区段 27 项（`mov spl,al`/`mov ah,al` 两条高字节/扩展字节案例、5 个 REX2/EGPR 向量共 9 条断言，以及 `C6`/`MOV` 存储形式与 8 位 r8b-r15b 几条），本轮新增区段 13 项（`XSET_FL` 9 项 + `SETcc` 4 项）。位掩码里绝大多数 `XSET_*` 仍无断言；`xde_sprintset`/`xde_sprintset2`/`xde_sprintfl` 三个打印器合计也只被 8 项内联检查触碰（Object sets 区段 4 + 闸门区段 3 + 新增 `sete al` 非 undef 1）。对象集是本库的一半价值，断言密度仍远低于长度/编码类。
2. **`flag` 位只有 4 条断言**：3 条 `expect_flag`（`C_BAD` 置位 / `C_REL` 置位 / `C_BAD` 未置位，`:510-515`）+ 1 条内联 `C_RIPREL`（`:194-201`，唯一还留在 `expect_*` 之外的内联 `fail`）。其余 `C_*` 无覆盖。（`XSET_FL` 是对象集位、不是 `flag` 位，本轮新增的 9 项断言记在 `expect_set` 名下。）
...
5. `:653-668` 四例缺区段注释，位置夹在「规范前缀 / 标志」区段与 `// 16-bit` 之间，容易误归类。
...
5. 若新属性需要影响 `flag` 或对象集，还要改 `src/xde.c` 的 `apply_attr_flags`（`:123-144`）或三趟对象集推导（`:146` / `:269` / `:392`）；
6. 加测试用例并 `build.bat`。

本轮示例：`m1[0x90..0x9F]`（SETcc）在 `gen_tables.py:314-315` 从 `XA_MODRM | XA_OPSZ8 | XA_UNDEF` 改成 `XA_MODRM | XA_OPSZ8`（去掉 `XA_UNDEF`），重跑生成器后 `src/xdetbl.c` 的行数与结构不变；FL 读取改在 `apply_usage_special`/`apply_modrm_usage` 里按 opcode 硬编码（`XA_*` 位空间已满，没有对应属性位）。
...
- **REX2 保留 `p_66`/`p_rep`（刻意行为，不是缺口）**（对比 VEX `:810-811`/`:833-834`、EVEX `:773-774`、XOP `:878-879` 都清）——`66` 是 REX2 合法的遗留前缀，不是多余前缀。编码侧 `p_66` 与其它遗留前缀一起在 `:1085` 统一发射（无 nvex 专属分支），所以 `66 D5 …` 现在能字节级往返；改这里要解码/编码两侧一起动。
- **`C_ADDR8` 只来自 MOFFS（原先那条不可达分支已删）**：`parse_modrm` 里 `disp` 只会是 1/2/4（`:559-567`，末行注释写明），原来永不触发的 `else → C_ADDR8` 已删除；8 字节地址只经 MOFFS 路径（`:977`）。新增位移宽度前先确认这条不变量还成立。
...
- **编码侧已按 SDM 组序规范化前缀**：`xde_asm_buf` 现在按 SDM 组序发射遗留前缀——lock/rep（组1）→ segment（组2）→ `66`（组3）→ `67`（组4）（`:1082-1086`），随后才是 REX 或 `vex[]` + opcode；`asm_size()`（`:1031-1057`）的计数顺序同步调整（字节总数不变）。解码侧仍不限制前缀顺序，所以非规范序输入（如 `67 66 90`、`64 F3 A4`）能解码，但**重编码会规范化**为 `66 67 90`、`F3 64 A4`——`xde_asm` 的输出不再逐字节等于非规范输入，往返测试因此只用规范序输入，或显式断言规范化结果。
- **`xde102/todo` 的 4 条现已全部处理**（1.02 时代的留档，不再有未决项）：① `REP` 对不同串指令的标志差异 → 已修：`REP` 只把 `CX/ECX/RCX` 计入 src+dst，不再无条件置 FL；`CMPS`/`SCAS` 写 FL、带 `REP` 时再读 FL（`src/xde.c:155-161`、`:239-248`）；② `setxx` → 已修：`0F 90-9F` 读 FL 且 r/m 只写不读（`:261-263`、`:281-282`）；③ `cld/std/cmpsb` 的 DF 源集 → 按「DF 不作为源」处理（`CLD`/`STD` 只置 `dst_set |= XSET_FL`）；④ `PUSH` 的栈宽不受 `67` 影响 → 2.00 早已由 `XA_PUSH` → `stack_set(mode)` 处理（`:471-479`）。被要求处理这些行为前先确认是否仍适用。
- **`SAHF`/`LAHF` 的标志读写未反映到对象集**（本轮发现、未处理）：`apply_usage_special` 里 `9E`（SAHF）只置 `src_set |= XSET_AH`、`9F`（LAHF）只置 `dst_set |= XSET_AH`（`:191-192`），缺 `dst_set |= XSET_FL` / `src_set |= XSET_FL`；`xde102/todo` 未涵盖它们。
...
| 文档 | 内容 |
|------|------|
| `README.md`（100 行） | 唯一面向使用者的文档：`## What it does`（`:11`）、`## Layout`（`:23`）、`## Build (MSVC)`（`:36`）、`## API`（`:54`，示例 `:57-64`，含 `xde_asm_buf`）、`## Notes`（`:82`，编码规则）。**无许可证段落、无外部链接。** |
