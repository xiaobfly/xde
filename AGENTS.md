...
`XA_*` 到 `C_*` 的映射在 `apply_attr_flags`（`src/xde.c:123-144`）：17 条一对一 OR，**只增不减**（读-改-写 `diza->flag`）。`XA_VVVV_GPR` 与 group 位不经此函数（`XA_VVVV_GPR` 在 `:340`/`:354`/`:362`/`:400`/`:577` 直接读，`XA_GRP_ID` 只在 `:977` 读）。
...
**VEX/XOP 的 GPR 路径（早前一轮修掉两处缺陷）**：表里带 `XA_VVVV_GPR` 的条目表示 **`vvvv` 本身就是 GPR 操作数**，所以 `0` = `EAX` 合法；只有**向量**操作数的 `vvvv` 才把 `0`/`0xF` 当作「无 vvvv」哨兵。原实现把整个分支包在 `if (vex_vvvv != 0 && vex_vvvv != 0xF)` 里，`andn eax,eax,ecx`（`C4 E2 78 F2 C1`，vvvv=EAX）因此丢掉 EAX 源；现在 `XA_VVVV_GPR` → **无条件** `gp_set(dsz, vex_vvvv, 1)`（含 0 与 15），否则才用 `!= 0 && != 0xF` 跳过再置 `XSET_OTHER`（`:340-349`）。第二处同在这一趟：load-form 的「reg 字段是目的寄存器」白名单只枚举了 0F map 的 `0F 40-4F`/`AF`/`BC`/`BD`/`B8`（及 `8A`/`8B`/`8D`），而 BMI/BMI2 经 **VEX 0F38/0F3A 与 XOP** 到达解码器，`andn`/`bextr`/`sarx`/`bzhi`/`mulx`/`rorx` 等的目的寄存器只报成 `XSET_OTHER`；现在条件加了 `(attr & XA_VVVV_GPR)`（该属性本就表示「reg 字段是目的寄存器」，注释见 `:352-353`），并在 `:361-362` 的编码类判定里同样放行。两处都做过 mutation 验证：回退任一处后 `andn`/`bextr`/`sarx` 各 2 条断言失败（共 6 条）。
...
| `tests/xde_test.c` | 唯一测试文件（1120 行） | 6 个 `expect_*` 助手, `main:161` |
...
退出码 `0` = 全通过，`1` = 有失败。`main` 是 `int main(void)`，**没有任何 CLI 参数**（无 filter / verbose / 单用例选择），每次运行都执行全部 310 项检查。
...
**自研极简 harness，无第三方框架，无 `ASSERT`/`CHECK`/`TEST` 宏**。全部逻辑在 `tests/xde_test.c`（1120 行）的单个 `int main(void)`（`:161-1120`）中：101 个匿名 `{ }` 块，每块一个 `static const uint8_t` 向量紧跟 `expect_*` 调用。**零文件 IO、零 fixture、零 golden 文件**。
...
| `expect_set(name, mode, b, n, sel, bit, want)` | `:117-138` | 要求解码长度 == `n`，再断言单个对象集位的存在/不存在。`sel` 0/1/2/3 = `src_set`/`dst_set`/`src_set2`/`dst_set2`，`want` 1 = 置位、0 = 未置位；成功打 `set<n> 0x… set|clear`（185 次） |
...
310 = 285 次助手调用（`expect_len` 42 + `expect_enc` 15 + `expect_fail` 4 + `expect_roundtrip` 5 + `expect_set` 185 + `expect_flag` 34）+ 25 项内联检查（`C_RIPREL` 1 + Object sets 打印器 4 + 闸门/asm 区段 10 + 规范前缀 2 + `sete al` 非 undef 1 + 打印器最坏情况 3 + Coverage 区段 ud2 1 + Jcc/LOOP 区段打印器 3）。
...
1. **对象集覆盖仍偏浅，但已铺到更多指令族**：现在对象集断言共 185 项（全部是 `expect_set`；34 项 `expect_flag` 单列于下一条），分布为 Object sets 区段 27 项（`mov spl,al`/`mov ah,al` 两条高字节/扩展字节案例、5 个 REX2/EGPR 向量共 9 条断言，以及 `C6`/`MOV` 存储形式与 8 位 r8b-r15b 几条），Canonical 区段 17 项（`XSET_FL` 9 项 + `SETcc` 4 项 + `SAHF`/`LAHF` 4 项），Coverage 区段 39 项（模式相关栈集、16 位串操作与寻址、`PUSHA`、I/O、`CPUID`、段寄存器、`LEAVE`、移位与组 3 的标志、若干 `C_*` 位、`UD2`），Jcc/LOOP 区段 8 项（`JZ` 读 FL、`LOOP`/`LOOPE`/`JCXZ` 的计数寄存器），以及前两轮新增的五个区段 94 项——`// Implicit operands…` 31 项（字符串 `LODS`/`STOS`/`INS`/`OUTS`、转换 `CBW`/`CWDE`/`CDQE`/`CWD`/`CQO`、BCD `AAA`/`AAM`/`AAD`、栈 `POPA`/`ENTER`、段寄存器 `PUSH ES`/`POP ES`、`XLAT`）、`// 0F-map implicit operands…` 12 项（0F 隐式操作数 + BMI 的 `andn`/`bextr`/`sarx`）、`// Addressing forms…` 6 项（SIB 带 index / SIB 不带 index / disp32 无基址）、`// Opcode-embedded registers…` 17 项（`BSWAP`/`XCHG`/`MOV r,I`/`ADD AL|EAX,Iv`/`F7 /4 MUL`/`INC eax`）、`// MOVBE / CRC32…` 28 项（本轮：MOVBE 载入/存储各 2、`CRC32` 2、XOP map 9/8 的 `vfrczpd`/`vpcomb` 各 2、EVEX 内存形式 3、`endbr64`+`movss` 4、legacy SSE/MMX 6、3DNow `pavgusb` 1、GPR 对照 4）。**本轮从「仍未覆盖」里移出**：MOVBE、CRC32、3DNow（现有 `XSET` 断言）、legacy SSE/MMX（`movups`/`paddb` 已覆盖寄存器与内存形式）。**仍缺**：向量操作数的 `vvvv`（`XSET_OTHER` 分支，如 `vaddps xmm0,xmm1,xmm2` 的 vvvv）、EVEX 的 `aaa`/`z`/`b` 语义（本引擎不建模 → 只能断 `OTHER`）、XOP map 8/9 的更多 opcode、`0F 00`/`0F 01` 系统组（带 `XA_UNDEF`，全有或全无）、`CALL`/`RET`（同理）、`0F AE` 系列（fences/fxsave）、`0F C7`（CMPXCHG8B/16B，仅内存），以及 `LOOP`/`Jcc` 之外的控制转移。位掩码里绝大多数 `XSET_*` 仍无断言；`xde_sprintset`/`xde_sprintset2`/`xde_sprintfl` 三个打印器合计也只被 16 项内联检查触碰（Object sets 区段 4 + 闸门区段 5 + `sete al` 非 undef 1 + 打印器最坏情况 3 + Jcc/LOOP 区段 3）。对象集是本库的一半价值，断言密度仍低于长度/编码类。
