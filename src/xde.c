...
got_opcode:
    ...
    // the table. The VEX / EVEX encodings carry their own prefix byte in the
    // header and never reach this test, and it needs no ModR/M byte either,
    // which is why it runs before the group below.
    if (legacy_enc && diza->map == XDE_MAP_0F) {
        unsigned pfx = diza->p_66 ? XDE_PFX_66
                     : (diza->p_rep == 0xF2 ? XDE_PFX_F2
                     : (diza->p_rep == 0xF3 ? XDE_PFX_F3 : XDE_PFX_NONE));
        if (!(xde_pfx_0f[diza->opcode2] & pfx))
            diza->flag |= C_BAD;
    }
